# HOLY Beverage — Product Rd — Ai Reviewer Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **ai-reviewer** in **product-rd** uses.
...
- **Portal**: every report retrievable at `/holy/product-rd/reports?role=ai-reviewer&id=<report_id>`
...
- `request_id`, `actor_role=ai-reviewer`, `dept=product-rd`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/product-rd/ai-reviewer
  → list of standard reports

POST /api/v1/holy/reports/product-rd/ai-reviewer/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/product-rd/ai-reviewer/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/product-rd/reports?role=ai-reviewer`
