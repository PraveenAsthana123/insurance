# Pipeline Checklist · reserve-tier-recommendation
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `reserve_tier_recommendation_raw` |
| Cleaned | Postgres | `reserve_tier_recommendation_clean` |
| Features | Feast / Postgres | `reserve_tier_recommendation_features` |
| Predictions | Postgres | `reserve_tier_recommendation_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/reserve-tier-recommendation/` |
...
1. Input → `reserve_tier_recommendation_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `reserve_tier_recommendation_predictions`
4. Log → ELK / Loki
