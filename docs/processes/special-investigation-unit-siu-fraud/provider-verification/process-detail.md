...
> Department 8 · Special Investigation Unit (SIU) / Fraud · slug `provider-verification` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/provider-verification/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/8/<domain>/provider-verification?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/8/<domain>/provider-verification?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/provider-verification.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/special-investigation-unit-siu-fraud/provider-verification/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PROVIDER_VERIFICATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PROVIDER_VERIFICATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PROVIDER_VERIFICATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PROVIDER_VERIFICATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PROVIDER_VERIFICATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PROVIDER_VERIFICATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PROVIDER_VERIFICATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/special-investigation-unit-siu-fraud/provider-verification/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PROVIDER_VERIFICATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/special-investigation-unit-siu-fraud/provider-verification/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/special-investigation-unit-siu-fraud/provider-verification/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/special-investigation-unit-siu-fraud/provider-verification/stats/`
