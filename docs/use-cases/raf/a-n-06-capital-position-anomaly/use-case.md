# Anomaly-Numerical-6 · capital-position-anomaly
...
Dept 15 ERM · multivariate z-score
...
graph LR
    ...
    Feature --> Model[Model · A-specific]
...
|---|---|---|---|
| `INSUR-CAPITAL_POSITION_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CAPITAL_POSITION_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CAPITAL_POSITION_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CAPITAL_POSITION_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-CAPITAL_POSITION_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-CAPITAL_POSITION_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
