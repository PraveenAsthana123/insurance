# Contact Center AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `contact-center-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for contact center ai tasks in
...
**FR-1**: System MUST accept input matching `contact-center-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/contact-center-ai/predict
        │
        ▼
...
- `predictions_contact-center-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/contact-center-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/contact-center-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/contact-center-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/contact-center-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `contact-center-ai_embeddings`
...
**Fairness metrics** (from data/fairness/contact-center-ai.json):
