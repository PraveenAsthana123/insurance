# Pipeline Checklist · claim-narrative-fraud-signal
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `claim_narrative_fraud_signal_raw` |
| Cleaned | Postgres | `claim_narrative_fraud_signal_clean` |
| Features | Feast / Postgres | `claim_narrative_fraud_signal_features` |
| Predictions | Postgres | `claim_narrative_fraud_signal_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/claim-narrative-fraud-signal/` |
...
1. Input → `claim_narrative_fraud_signal_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `claim_narrative_fraud_signal_predictions`
4. Log → ELK / Loki
