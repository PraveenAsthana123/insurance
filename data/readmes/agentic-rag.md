# Agentic RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `agentic-rag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for agentic rag tasks in
...
**FR-1**: System MUST accept input matching `agentic-rag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/agentic-rag/predict
        │
        ▼
...
- `predictions_agentic-rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/agentic-rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/agentic-rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/agentic-rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/agentic-rag/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `agentic-rag_embeddings`
...
**Fairness metrics** (from data/fairness/agentic-rag.json):
