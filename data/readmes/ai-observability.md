# AI Observability · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-observability` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for ai observability tasks in
...
**FR-1**: System MUST accept input matching `ai-observability` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-observability/predict
        │
        ▼
...
- `predictions_ai-observability`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-observability/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-observability/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-observability/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-observability/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-observability_embeddings`
...
**Fairness metrics** (from data/fairness/ai-observability.json):
