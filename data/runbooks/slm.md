...
WHAT broke?      Check `/data/metrics/slm.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/slm.json`
WHO touched?     `git log --since=<date> data/ai_types/slm.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/slm/model.joblib.backup models/slm/model.joblib`
