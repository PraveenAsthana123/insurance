# Pipeline Checklist · sensor-telemetry-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `sensor_telemetry_anomaly_raw` |
| Cleaned | Postgres | `sensor_telemetry_anomaly_clean` |
| Features | Feast / Postgres | `sensor_telemetry_anomaly_features` |
| Predictions | Postgres | `sensor_telemetry_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/sensor-telemetry-anomaly/` |
...
1. Input → `sensor_telemetry_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `sensor_telemetry_anomaly_predictions`
4. Log → ELK / Loki
