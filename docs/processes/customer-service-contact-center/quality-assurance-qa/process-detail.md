...
> Department 9 · Customer Service / Contact Center · slug `quality-assurance-qa` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/quality-assurance-qa/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/9/<domain>/quality-assurance-qa?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/9/<domain>/quality-assurance-qa?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/quality-assurance-qa.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/customer-service-contact-center/quality-assurance-qa/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-QUALITY_ASSURANCE_QA-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-QUALITY_ASSURANCE_QA-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-QUALITY_ASSURANCE_QA-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-QUALITY_ASSURANCE_QA-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-QUALITY_ASSURANCE_QA-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-QUALITY_ASSURANCE_QA-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-QUALITY_ASSURANCE_QA-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/customer-service-contact-center/quality-assurance-qa/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-QUALITY_ASSURANCE_QA-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/customer-service-contact-center/quality-assurance-qa/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/customer-service-contact-center/quality-assurance-qa/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/customer-service-contact-center/quality-assurance-qa/stats/`
