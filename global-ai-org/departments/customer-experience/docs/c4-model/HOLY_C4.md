# HOLY Beverage — Customer Experience C4 Model
...
graph TB
    ...
    holy[HOLY Customer Experience Platform]
...
graph LR
    ...
    features --> model[NLP, RAG, Sentiment AI, Conversational AI, Predictive ML]
...
- Decision lineage: every API call writes a row to `customer_experience_audit` (see LLD §2)

## L6 — Observability

- Logs: structured JSON with `correlation_id`, `tenant_id`, `user_id`, `model_version`, `prompt_version`
- Metrics: `customer-experience_request_count`, `customer-experience_latency_p95`, `customer-experience_model_confidence`, `customer-experience_hallucination_rate`
