# Recommender-Numerical-7 · vendor-payment-terms-rec
...
Dept 17 Procurement · MAB Thompson
...
|---|---|---|---|
| `INSUR-VENDOR_PAYMENT_TERMS_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VENDOR_PAYMENT_TERMS_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VENDOR_PAYMENT_TERMS_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VENDOR_PAYMENT_TERMS_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-VENDOR_PAYMENT_TERMS_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-VENDOR_PAYMENT_TERMS_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
