# Autonomous Vehicle AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `autonomous-vehicle-ai` · **Kind**: `spec_only`
**Accuracy**: 0.9333 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for autonomous vehicle ai tasks in
...
**FR-1**: System MUST accept input matching `autonomous-vehicle-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/autonomous-vehicle-ai/predict
        │
        ▼
...
- `predictions_autonomous-vehicle-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/autonomous-vehicle-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/autonomous-vehicle-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/autonomous-vehicle-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/autonomous-vehicle-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `autonomous-vehicle-ai_embeddings`
...
**Fairness metrics** (from data/fairness/autonomous-vehicle-ai.json):
