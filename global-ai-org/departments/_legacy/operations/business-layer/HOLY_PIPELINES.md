# HOLY Beverage — Operations — Automated Pipelines
...
|---|---|---|---|
| 1 | Process Anomaly | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Process Anomaly + RCA Generation + Workflow Optimization**._
...
**Audit prefix:** `pipeline.operations.process_anomaly`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream operations systems | `outcome=data_received` |
...
| 5 Report | Operations manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/operations` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/operations/<process_id>` | Full 5-phase spec for one process |
