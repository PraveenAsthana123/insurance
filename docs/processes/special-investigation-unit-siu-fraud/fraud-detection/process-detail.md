...
> Department 8 · Special Investigation Unit (SIU) / Fraud · slug `fraud-detection` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/fraud-detection/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/8/<domain>/fraud-detection?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/8/<domain>/fraud-detection?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/fraud-detection.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/special-investigation-unit-siu-fraud/fraud-detection/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-FRAUD_DETECTION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-FRAUD_DETECTION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-FRAUD_DETECTION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-FRAUD_DETECTION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-FRAUD_DETECTION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-FRAUD_DETECTION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-FRAUD_DETECTION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/special-investigation-unit-siu-fraud/fraud-detection/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-FRAUD_DETECTION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/special-investigation-unit-siu-fraud/fraud-detection/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/special-investigation-unit-siu-fraud/fraud-detection/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/special-investigation-unit-siu-fraud/fraud-detection/stats/`
