# HOLY Beverage — Legal Low-Level Design (LLD)
...
|---|---|---|---|
| `/api/v1/legal/list` | GET | List entities | RBAC |
| `/api/v1/legal/<id>` | GET | Get entity by id | RBAC |
| `/api/v1/legal/<id>` | PUT | Update entity | RBAC + ABAC |
| `/api/v1/legal/forecast` | POST | Trigger forecast/recommendation | RBAC |
| `/api/v1/legal/approve/<id>` | POST | Human approval step | RBAC + audit |
...
CREATE INDEX idx_legal_audit_ts ON legal_audit(timestamp);
CREATE INDEX idx_legal_audit_tenant ON legal_audit(tenant_id, timestamp);
```
...
- Metrics (Prometheus): `legal_request_count`, `legal_request_latency_ms`, `legal_model_confidence`
