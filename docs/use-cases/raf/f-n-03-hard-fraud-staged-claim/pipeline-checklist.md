# Pipeline Checklist · hard-fraud-staged-claim
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `hard_fraud_staged_claim_raw` |
| Cleaned | Postgres | `hard_fraud_staged_claim_clean` |
| Features | Feast / Postgres | `hard_fraud_staged_claim_features` |
| Predictions | Postgres | `hard_fraud_staged_claim_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/hard-fraud-staged-claim/` |
...
1. Input → `hard_fraud_staged_claim_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `hard_fraud_staged_claim_predictions`
4. Log → ELK / Loki
