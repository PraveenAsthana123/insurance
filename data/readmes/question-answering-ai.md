# Question Answering AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `question-answering-ai` · **Kind**: `nlp_baseline`
...
**Business goal**: Reduce manual effort + improve accuracy for question answering ai tasks in
...
**FR-1**: System MUST accept input matching `question-answering-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/question-answering-ai/predict
        │
        ▼
...
- `predictions_question-answering-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/question-answering-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/question-answering-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/question-answering-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/question-answering-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `question-answering-ai_embeddings`
...
**Fairness metrics** (from data/fairness/question-answering-ai.json):
