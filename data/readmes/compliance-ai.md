# Compliance AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `compliance-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for compliance ai tasks in
...
**FR-1**: System MUST accept input matching `compliance-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/compliance-ai/predict
        │
        ▼
...
- `predictions_compliance-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/compliance-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/compliance-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/compliance-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/compliance-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `compliance-ai_embeddings`
...
**Fairness metrics** (from data/fairness/compliance-ai.json):
