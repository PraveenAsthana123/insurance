# Autonomous AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `autonomous-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for autonomous ai tasks in
...
**FR-1**: System MUST accept input matching `autonomous-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/autonomous-ai/predict
        │
        ▼
...
- `predictions_autonomous-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/autonomous-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/autonomous-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/autonomous-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/autonomous-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `autonomous-ai_embeddings`
...
**Fairness metrics** (from data/fairness/autonomous-ai.json):
