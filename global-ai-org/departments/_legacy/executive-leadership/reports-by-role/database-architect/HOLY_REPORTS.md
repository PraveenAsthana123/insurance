# HOLY Beverage — Executive Leadership — Database Architect Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **database-architect** in **executive-leadership** uses.

## Standard Reports

| # | Report Name | Cadence | Format | Recipients |
|---|---|---|---|---|
| 1 | Weekly DB health | weekly | PDF | DBA team + manager |
| 2 | Monthly query optimization | monthly | Excel | eng + DBA |
| 3 | Quarterly capacity + schema review | quarterly | PDF | manager + system-architect |
...
- **Portal**: every report retrievable at `/holy/executive-leadership/reports?role=database-architect&id=<report_id>`
...
- `request_id`, `actor_role=database-architect`, `dept=executive-leadership`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/executive-leadership/database-architect
  → list of standard reports

POST /api/v1/holy/reports/executive-leadership/database-architect/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/executive-leadership/database-architect/<report_id>/runs/<run_id>
  → status + signed download URL
```

## Related Artifacts

- `../../dashboards-by-role/database-architect/HOLY_DASHBOARD.md` — paired dashboard
- `../../roles/database-architect/HOLY_README.md` — role definition
- Frontend route: `/holy/executive-leadership/reports?role=database-architect`
