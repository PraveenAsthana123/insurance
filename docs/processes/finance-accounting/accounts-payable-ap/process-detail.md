...
> Department 14 · Finance & Accounting · slug `accounts-payable-ap` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/accounts-payable-ap/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/14/<domain>/accounts-payable-ap?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/14/<domain>/accounts-payable-ap?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/accounts-payable-ap.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/finance-accounting/accounts-payable-ap/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-ACCOUNTS_PAYABLE_AP-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-ACCOUNTS_PAYABLE_AP-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-ACCOUNTS_PAYABLE_AP-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-ACCOUNTS_PAYABLE_AP-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-ACCOUNTS_PAYABLE_AP-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-ACCOUNTS_PAYABLE_AP-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-ACCOUNTS_PAYABLE_AP-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/finance-accounting/accounts-payable-ap/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-ACCOUNTS_PAYABLE_AP-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/finance-accounting/accounts-payable-ap/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/finance-accounting/accounts-payable-ap/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/finance-accounting/accounts-payable-ap/stats/`
