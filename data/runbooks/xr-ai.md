...
WHAT broke?      Check `/data/metrics/xr-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/xr-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/xr-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/xr-ai/model.joblib.backup models/xr-ai/model.joblib`
