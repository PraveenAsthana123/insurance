...
WHAT broke?      Check `/data/metrics/what-if-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/what-if-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/what-if-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/what-if-ai/model.joblib.backup models/what-if-ai/model.joblib`
