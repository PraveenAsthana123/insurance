# Anomaly-Numerical-5 · loss-ratio-drift-anomaly
...
Dept 11 Reinsurance · TFT residual + SPC
...
|---|---|---|---|
| `INSUR-LOSS_RATIO_DRIFT_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-LOSS_RATIO_DRIFT_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-LOSS_RATIO_DRIFT_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-LOSS_RATIO_DRIFT_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-LOSS_RATIO_DRIFT_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-LOSS_RATIO_DRIFT_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
