...
> Department 10 · Actuarial · slug `ibnr-estimation` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/ibnr-estimation/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/10/<domain>/ibnr-estimation?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/10/<domain>/ibnr-estimation?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/ibnr-estimation.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/actuarial/ibnr-estimation/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-IBNR_ESTIMATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-IBNR_ESTIMATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-IBNR_ESTIMATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-IBNR_ESTIMATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-IBNR_ESTIMATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-IBNR_ESTIMATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-IBNR_ESTIMATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/actuarial/ibnr-estimation/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-IBNR_ESTIMATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/actuarial/ibnr-estimation/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/actuarial/ibnr-estimation/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/actuarial/ibnr-estimation/stats/`
