...
WHAT broke?      Check `/data/metrics/iot-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/iot-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/iot-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/iot-ai/model.joblib.backup models/iot-ai/model.joblib`
