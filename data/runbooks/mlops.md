...
WHAT broke?      Check `/data/metrics/mlops.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/mlops.json`
WHO touched?     `git log --since=<date> data/ai_types/mlops.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/mlops/model.joblib.backup models/mlops/model.joblib`
