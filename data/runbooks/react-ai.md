...
WHAT broke?      Check `/data/metrics/react-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/react-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/react-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/react-ai/model.joblib.backup models/react-ai/model.joblib`
