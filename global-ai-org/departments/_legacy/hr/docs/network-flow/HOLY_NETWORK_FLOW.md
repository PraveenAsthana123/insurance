...
```
Hr API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Hr API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          hr_user_events       analytics-svc
Backend API       ──►          hr_audit              audit-store
ML Worker         ──►          hr_predictions        prediction-store
Drift detector    ──►          hr_alerts             on-call PagerDuty
...
|---|---|---|---|
| hr-api | postgresql | 5432 | TCP |
| hr-api | redis | 6379 | TCP |
| hr-api | ollama | 11434 | TCP |
| hr-api | otel-collector | 4317 | gRPC |
| hr-api | (other dept APIs) | 8000 | HTTP via service mesh |
