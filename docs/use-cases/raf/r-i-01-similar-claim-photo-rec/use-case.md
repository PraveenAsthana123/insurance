# Recommender-Image-1 · similar-claim-photo-rec
...
|---|---|---|---|
| `INSUR-SIMILAR_CLAIM_PHOTO_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SIMILAR_CLAIM_PHOTO_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SIMILAR_CLAIM_PHOTO_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SIMILAR_CLAIM_PHOTO_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SIMILAR_CLAIM_PHOTO_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SIMILAR_CLAIM_PHOTO_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
