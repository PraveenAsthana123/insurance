# Semantic AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `semantic-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for semantic ai tasks in
...
**FR-1**: System MUST accept input matching `semantic-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/semantic-ai/predict
        │
        ▼
...
- `predictions_semantic-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/semantic-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/semantic-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/semantic-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/semantic-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `semantic-ai_embeddings`
...
**Fairness metrics** (from data/fairness/semantic-ai.json):
