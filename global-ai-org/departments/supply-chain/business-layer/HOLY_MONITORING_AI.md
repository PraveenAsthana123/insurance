# HOLY Beverage — Supply Chain — Monitoring AI
...
> Persona: **Supply-Chain Manager**. Focus: **demand-forecast MAPE + reorder-point ML drift**.
...
Tab: `/holy/supply-chain/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/supply-chain` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/supply-chain/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/supply-chain/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #supply-chain-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #supply-chain-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/supply-chain` returns 200 + job list
