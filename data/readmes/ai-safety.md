# AI Safety · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-safety` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for ai safety tasks in
...
**FR-1**: System MUST accept input matching `ai-safety` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-safety/predict
        │
        ▼
...
- `predictions_ai-safety`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-safety/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-safety/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-safety/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-safety/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-safety_embeddings`
...
**Fairness metrics** (from data/fairness/ai-safety.json):
