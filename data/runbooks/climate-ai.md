...
WHAT broke?      Check `/data/metrics/climate-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/climate-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/climate-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/climate-ai/model.joblib.backup models/climate-ai/model.joblib`
