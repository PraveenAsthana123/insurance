# Physical AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `physical-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for physical ai tasks in
...
**FR-1**: System MUST accept input matching `physical-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/physical-ai/predict
        │
        ▼
...
- `predictions_physical-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/physical-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/physical-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/physical-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/physical-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `physical-ai_embeddings`
...
**Fairness metrics** (from data/fairness/physical-ai.json):
