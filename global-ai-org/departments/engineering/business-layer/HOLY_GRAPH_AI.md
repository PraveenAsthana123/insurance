# HOLY Beverage — Engineering — Graph AI
...
> Anchor flow: **PR ─ reviewed by code_llm ─ deployed via pipeline**.
...
```
PR ─ reviewed by code_llm ─ deployed via pipeline
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in engineering touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/engineering` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/engineering/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/engineering/neighbors/<node_id>` | 1-hop neighbors of a node |
