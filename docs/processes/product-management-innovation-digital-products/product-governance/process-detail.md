...
> Department 22 · Product Management, Innovation & Digital Products · slug `product-governance` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/product-governance/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/22/<domain>/product-governance?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/22/<domain>/product-governance?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/product-governance.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/product-management-innovation-digital-products/product-governance/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRODUCT_GOVERNANCE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRODUCT_GOVERNANCE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRODUCT_GOVERNANCE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRODUCT_GOVERNANCE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRODUCT_GOVERNANCE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRODUCT_GOVERNANCE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRODUCT_GOVERNANCE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/product-management-innovation-digital-products/product-governance/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRODUCT_GOVERNANCE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/product-management-innovation-digital-products/product-governance/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/product-management-innovation-digital-products/product-governance/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/product-management-innovation-digital-products/product-governance/stats/`
