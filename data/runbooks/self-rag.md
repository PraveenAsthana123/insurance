...
WHAT broke?      Check `/data/metrics/self-rag.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/self-rag.json`
WHO touched?     `git log --since=<date> data/ai_types/self-rag.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/self-rag/model.joblib.backup models/self-rag/model.joblib`
