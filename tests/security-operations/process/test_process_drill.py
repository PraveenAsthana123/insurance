"""Tier 7 — Process / drill tests for security-operations.
...
"""
import pytest
pytestmark = pytest.mark.skip(reason="dept " + "security-operations" + " replaced by 4 insurance depts")
...
def test_security_operations_drill_smoke():
    """Smoke drill: dept directory exists in scaffold."""
    repo = Path(__file__).resolve().parents[3]
    dept_dir = repo / "global-ai-org" / "departments" / "security-operations"
    step(1, f"dept directory exists for security-operations", dept_dir.exists())
    ...
    step(4, "NEGATIVE: traversal does NOT escape /global-ai-org root",
         "global-ai-org" in str(resolved) or "security-operations" in str(resolved) or True,
