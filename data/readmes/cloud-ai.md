# Cloud AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `cloud-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for cloud ai tasks in
...
**FR-1**: System MUST accept input matching `cloud-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/cloud-ai/predict
        │
        ▼
...
- `predictions_cloud-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/cloud-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/cloud-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/cloud-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/cloud-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `cloud-ai_embeddings`
...
**Fairness metrics** (from data/fairness/cloud-ai.json):
