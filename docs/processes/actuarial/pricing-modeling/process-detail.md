...
> Department 10 · Actuarial · slug `pricing-modeling` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/pricing-modeling/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/10/<domain>/pricing-modeling?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/10/<domain>/pricing-modeling?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/pricing-modeling.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/actuarial/pricing-modeling/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRICING_MODELING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRICING_MODELING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRICING_MODELING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRICING_MODELING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRICING_MODELING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRICING_MODELING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRICING_MODELING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/actuarial/pricing-modeling/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRICING_MODELING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/actuarial/pricing-modeling/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/actuarial/pricing-modeling/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/actuarial/pricing-modeling/stats/`
