...
any 3-5 to assemble a Security Operations board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Security Operations
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is Admin (System)'s view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: Dept Manager in Security Operations
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is Dept Manager's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: Team Member in Security Operations
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is Team Member's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Security Operations
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is QA / Tester's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: SecOps Engineer in Security Operations
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is SecOps Engineer's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Security Operations
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is DevOps's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: AI Reviewer in Security Operations
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is AI Reviewer's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: DT Lead in Security Operations
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is DT Lead's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: System Architect in Security Operations
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is System Architect's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: Test Architect in Security Operations
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is Test Architect's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: DB Architect in Security Operations
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is DB Architect's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: API Architect in Security Operations
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is API Architect's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: Data Owner in Security Operations
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is Data Owner's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: AI Strategy Lead in Security Operations
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is AI Strategy Lead's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
- **Persona**: InfoSec / CISO Office in Security Operations
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Security Operations dashboards
- **KPI moved**: MTTD + false-positive rate
...
- **Talking points**: "This is InfoSec / CISO Office's view of Security Operations. In 90 seconds we'll move MTTD + false-positive rate by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/security-operations` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/security-operations` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/security-operations/<role>` | Single role demo detail |
