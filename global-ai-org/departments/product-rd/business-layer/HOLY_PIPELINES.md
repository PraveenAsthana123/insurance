# HOLY Beverage — Product Rd — Automated Pipelines
...
|---|---|---|---|
| 1 | Concept Scoring | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Concept Scoring + Feature Extraction + Patent Similarity**._
...
**Audit prefix:** `pipeline.product_rd.concept_scoring`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream product-rd systems | `outcome=data_received` |
...
| 5 Report | Product Rd manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/product-rd` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/product-rd/<process_id>` | Full 5-phase spec for one process |
