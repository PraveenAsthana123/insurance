# HOLY Beverage — Supply Chain — Team Member Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **team-member** in **supply-chain** uses.
...
- **Portal**: every report retrievable at `/holy/supply-chain/reports?role=team-member&id=<report_id>`
...
- `request_id`, `actor_role=team-member`, `dept=supply-chain`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/supply-chain/team-member
  → list of standard reports

POST /api/v1/holy/reports/supply-chain/team-member/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/supply-chain/team-member/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/supply-chain/reports?role=team-member`
