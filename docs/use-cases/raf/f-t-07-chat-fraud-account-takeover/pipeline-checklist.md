# Pipeline Checklist · chat-fraud-account-takeover
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `chat_fraud_account_takeover_raw` |
| Cleaned | Postgres | `chat_fraud_account_takeover_clean` |
| Features | Feast / Postgres | `chat_fraud_account_takeover_features` |
| Predictions | Postgres | `chat_fraud_account_takeover_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/chat-fraud-account-takeover/` |
...
1. Input → `chat_fraud_account_takeover_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `chat_fraud_account_takeover_predictions`
4. Log → ELK / Loki
