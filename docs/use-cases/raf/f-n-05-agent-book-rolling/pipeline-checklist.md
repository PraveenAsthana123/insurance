# Pipeline Checklist · agent-book-rolling
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `agent_book_rolling_raw` |
| Cleaned | Postgres | `agent_book_rolling_clean` |
| Features | Feast / Postgres | `agent_book_rolling_features` |
| Predictions | Postgres | `agent_book_rolling_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/agent-book-rolling/` |
...
1. Input → `agent_book_rolling_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `agent_book_rolling_predictions`
4. Log → ELK / Loki
