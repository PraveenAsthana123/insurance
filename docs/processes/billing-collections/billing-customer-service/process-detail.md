...
> Department 6 · Billing & Collections · slug `billing-customer-service` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/billing-customer-service/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/6/<domain>/billing-customer-service?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/6/<domain>/billing-customer-service?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/billing-collections/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/billing-customer-service.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/billing-collections/billing-customer-service/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-BILLING_CUSTOMER_SERVICE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-BILLING_CUSTOMER_SERVICE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-BILLING_CUSTOMER_SERVICE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-BILLING_CUSTOMER_SERVICE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-BILLING_CUSTOMER_SERVICE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-BILLING_CUSTOMER_SERVICE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-BILLING_CUSTOMER_SERVICE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/billing-collections/billing-customer-service/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-BILLING_CUSTOMER_SERVICE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/billing-collections/billing-customer-service/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/billing-collections/billing-customer-service/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/billing-collections/billing-customer-service/stats/`
