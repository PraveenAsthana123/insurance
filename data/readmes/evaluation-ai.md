# Evaluation AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `evaluation-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for evaluation ai tasks in
...
**FR-1**: System MUST accept input matching `evaluation-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/evaluation-ai/predict
        │
        ▼
...
- `predictions_evaluation-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/evaluation-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/evaluation-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/evaluation-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/evaluation-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `evaluation-ai_embeddings`
...
**Fairness metrics** (from data/fairness/evaluation-ai.json):
