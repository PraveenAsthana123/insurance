# Fraud-Text-4 · vendor-form-fraud
...
Dept 17 Procurement · LayoutLM + handwriting
...
|---|---|---|---|
| `INSUR-VENDOR_FORM_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VENDOR_FORM_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VENDOR_FORM_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VENDOR_FORM_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-VENDOR_FORM_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-VENDOR_FORM_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
