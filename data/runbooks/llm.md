...
WHAT broke?      Check `/data/metrics/llm.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/llm.json`
WHO touched?     `git log --since=<date> data/ai_types/llm.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/llm/model.joblib.backup models/llm/model.joblib`
