# Pipeline Checklist · policy-clause-recommender
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `policy_clause_recommender_raw` |
| Cleaned | Postgres | `policy_clause_recommender_clean` |
| Features | Feast / Postgres | `policy_clause_recommender_features` |
| Predictions | Postgres | `policy_clause_recommender_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/policy-clause-recommender/` |
...
1. Input → `policy_clause_recommender_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `policy_clause_recommender_predictions`
4. Log → ELK / Loki
