# Taxonomy AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `taxonomy-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for taxonomy ai tasks in
...
**FR-1**: System MUST accept input matching `taxonomy-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/taxonomy-ai/predict
        │
        ▼
...
- `predictions_taxonomy-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/taxonomy-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/taxonomy-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/taxonomy-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/taxonomy-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `taxonomy-ai_embeddings`
...
**Fairness metrics** (from data/fairness/taxonomy-ai.json):
