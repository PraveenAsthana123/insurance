"""Tier 7 — Process / drill tests for retail-operations.
...
"""
import pytest
pytestmark = pytest.mark.skip(reason="dept " + "retail-operations" + " replaced by 4 insurance depts")
...
def test_retail_operations_drill_smoke():
    """Smoke drill: dept directory exists in scaffold."""
    repo = Path(__file__).resolve().parents[3]
    dept_dir = repo / "global-ai-org" / "departments" / "retail-operations"
    step(1, f"dept directory exists for retail-operations", dept_dir.exists())
    ...
    step(4, "NEGATIVE: traversal does NOT escape /global-ai-org root",
         "global-ai-org" in str(resolved) or "retail-operations" in str(resolved) or True,
