# Humanoid AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `humanoid-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for humanoid ai tasks in
...
**FR-1**: System MUST accept input matching `humanoid-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/humanoid-ai/predict
        │
        ▼
...
- `predictions_humanoid-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/humanoid-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/humanoid-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/humanoid-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/humanoid-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `humanoid-ai_embeddings`
...
**Fairness metrics** (from data/fairness/humanoid-ai.json):
