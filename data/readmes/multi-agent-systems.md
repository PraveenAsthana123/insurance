# Multi-Agent Systems · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `multi-agent-systems` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for multi-agent systems tasks in
...
**FR-1**: System MUST accept input matching `multi-agent-systems` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/multi-agent-systems/predict
        │
        ▼
...
- `predictions_multi-agent-systems`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/multi-agent-systems/predict` · body: features → prediction
- `GET /api/v1/ai-types/multi-agent-systems/metrics` → live metrics.json
- `GET /api/v1/ai-types/multi-agent-systems/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/multi-agent-systems/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `multi-agent-systems_embeddings`
...
**Fairness metrics** (from data/fairness/multi-agent-systems.json):
