...
any 3-5 to assemble a Engineering board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Engineering
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is Admin (System)'s view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: Dept Manager in Engineering
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is Dept Manager's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: Team Member in Engineering
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is Team Member's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Engineering
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is QA / Tester's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: SecOps Engineer in Engineering
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is SecOps Engineer's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Engineering
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is DevOps's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: AI Reviewer in Engineering
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is AI Reviewer's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: DT Lead in Engineering
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is DT Lead's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: System Architect in Engineering
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is System Architect's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: Test Architect in Engineering
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is Test Architect's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: DB Architect in Engineering
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is DB Architect's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: API Architect in Engineering
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is API Architect's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: Data Owner in Engineering
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is Data Owner's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: AI Strategy Lead in Engineering
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is AI Strategy Lead's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
- **Persona**: InfoSec / CISO Office in Engineering
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Engineering dashboards
- **KPI moved**: Deploy frequency + MTTR
...
- **Talking points**: "This is InfoSec / CISO Office's view of Engineering. In 90 seconds we'll move Deploy frequency + MTTR by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/engineering` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/engineering` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/engineering/<role>` | Single role demo detail |
