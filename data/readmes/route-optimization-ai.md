# Route Optimization AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `route-optimization-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for route optimization ai tasks in
...
**FR-1**: System MUST accept input matching `route-optimization-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/route-optimization-ai/predict
        │
        ▼
...
- `predictions_route-optimization-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/route-optimization-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/route-optimization-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/route-optimization-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/route-optimization-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `route-optimization-ai_embeddings`
...
**Fairness metrics** (from data/fairness/route-optimization-ai.json):
