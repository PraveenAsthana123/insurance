...
WHAT broke?      Check `/data/metrics/lineage-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/lineage-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/lineage-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/lineage-ai/model.joblib.backup models/lineage-ai/model.joblib`
