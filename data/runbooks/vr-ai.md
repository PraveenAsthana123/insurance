...
WHAT broke?      Check `/data/metrics/vr-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/vr-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/vr-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/vr-ai/model.joblib.backup models/vr-ai/model.joblib`
