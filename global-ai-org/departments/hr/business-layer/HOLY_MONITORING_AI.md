# HOLY Beverage — Hr — Monitoring AI
...
> Persona: **HR Manager**. Focus: **attrition-prediction drift + resume-parsing NLP accuracy**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/hr` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/hr/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/hr/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #hr-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #hr-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/hr` returns 200 + job list
