# Anomaly-Image-6 · satellite-imagery-anomaly
...
Dept 11 Reinsurance · Siamese before/after
...
|---|---|---|---|
| `INSUR-SATELLITE_IMAGERY_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SATELLITE_IMAGERY_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SATELLITE_IMAGERY_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SATELLITE_IMAGERY_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SATELLITE_IMAGERY_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SATELLITE_IMAGERY_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
