# Pipeline Checklist · doc-format-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `doc_format_anomaly_raw` |
| Cleaned | Postgres | `doc_format_anomaly_clean` |
| Features | Feast / Postgres | `doc_format_anomaly_features` |
| Predictions | Postgres | `doc_format_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/doc-format-anomaly/` |
...
1. Input → `doc_format_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `doc_format_anomaly_predictions`
4. Log → ELK / Loki
