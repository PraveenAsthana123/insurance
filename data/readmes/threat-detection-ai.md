# Threat Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `threat-detection-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for threat detection ai tasks in
...
**FR-1**: System MUST accept input matching `threat-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/threat-detection-ai/predict
        │
        ▼
...
- `predictions_threat-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/threat-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/threat-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/threat-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/threat-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `threat-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/threat-detection-ai.json):
