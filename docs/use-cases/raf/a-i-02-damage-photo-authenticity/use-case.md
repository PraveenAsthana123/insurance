# Anomaly-Image-2 · damage-photo-authenticity
...
|---|---|---|---|
| `INSUR-DAMAGE_PHOTO_AUTHENTICITY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DAMAGE_PHOTO_AUTHENTICITY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DAMAGE_PHOTO_AUTHENTICITY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DAMAGE_PHOTO_AUTHENTICITY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-DAMAGE_PHOTO_AUTHENTICITY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-DAMAGE_PHOTO_AUTHENTICITY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
