# Pipeline Checklist · capital-position-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `capital_position_anomaly_raw` |
| Cleaned | Postgres | `capital_position_anomaly_clean` |
| Features | Feast / Postgres | `capital_position_anomaly_features` |
| Predictions | Postgres | `capital_position_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/capital-position-anomaly/` |
...
1. Input → `capital_position_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `capital_position_anomaly_predictions`
4. Log → ELK / Loki
