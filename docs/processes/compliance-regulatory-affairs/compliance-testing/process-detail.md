...
> Department 12 · Compliance & Regulatory Affairs · slug `compliance-testing` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/compliance-testing/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/12/<domain>/compliance-testing?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/12/<domain>/compliance-testing?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/compliance-testing.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/compliance-regulatory-affairs/compliance-testing/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-COMPLIANCE_TESTING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-COMPLIANCE_TESTING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-COMPLIANCE_TESTING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-COMPLIANCE_TESTING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-COMPLIANCE_TESTING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-COMPLIANCE_TESTING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-COMPLIANCE_TESTING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/compliance-regulatory-affairs/compliance-testing/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-COMPLIANCE_TESTING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/compliance-regulatory-affairs/compliance-testing/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/compliance-regulatory-affairs/compliance-testing/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/compliance-regulatory-affairs/compliance-testing/stats/`
