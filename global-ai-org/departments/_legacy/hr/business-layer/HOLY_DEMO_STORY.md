# HOLY Beverage — Hr — Demo User Story
...
The persona needs to: **cut time-to-fill from 65 days to 30, reduce attrition 25%, raise engagement 15%**

Main KPI being moved: **Time-to-fill, attrition rate, employee NPS, internal mobility rate**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/hr` (the HOLY navigation page for this dept).
...
> "Today Rachel spends 80% of their week on manual data
...
> counterfactual. We don't replace Rachel — we replace
...
|---|---|---|---|
| 1 | Open `/holy/hr` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/hr` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_hr.py` (planned).
