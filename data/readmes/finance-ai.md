# Finance AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `finance-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for finance ai tasks in
...
**FR-1**: System MUST accept input matching `finance-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/finance-ai/predict
        │
        ▼
...
- `predictions_finance-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/finance-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/finance-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/finance-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/finance-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `finance-ai_embeddings`
...
**Fairness metrics** (from data/fairness/finance-ai.json):
