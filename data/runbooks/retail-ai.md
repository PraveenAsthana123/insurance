...
WHAT broke?      Check `/data/metrics/retail-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/retail-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/retail-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/retail-ai/model.joblib.backup models/retail-ai/model.joblib`
