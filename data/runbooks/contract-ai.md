...
WHAT broke?      Check `/data/metrics/contract-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/contract-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/contract-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/contract-ai/model.joblib.backup models/contract-ai/model.joblib`
