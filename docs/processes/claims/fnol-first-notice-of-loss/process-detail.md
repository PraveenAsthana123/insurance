...
> Department 7 · Claims · slug `fnol-first-notice-of-loss` · AI items: 6
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/fnol-first-notice-of-loss/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/7/<domain>/fnol-first-notice-of-loss?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/7/<domain>/fnol-first-notice-of-loss?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/fnol-first-notice-of-loss.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/claims/fnol-first-notice-of-loss/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/claims/fnol-first-notice-of-loss/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-FNOL_FIRST_NOTICE_OF_LOSS-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/claims/fnol-first-notice-of-loss/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/claims/fnol-first-notice-of-loss/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/claims/fnol-first-notice-of-loss/stats/`
