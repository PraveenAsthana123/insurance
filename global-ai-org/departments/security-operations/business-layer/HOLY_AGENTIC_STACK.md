# HOLY Beverage — Security Operations — Agentic Stack
...
> actions against and the required scope for each. The Security Operations manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Security Operations)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Security Operations agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Security Operations agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Security Operations and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/security-operations.yaml](../../../../config/scopes/security-operations.yaml)
...
| Action irreversible (deletion, signing) | Manager (Security Operations) approval |
| Confidence < 0.6 | Manager (Security Operations) review queue |
...
`/holy/security-operations/agentic` (per §64.40.5).

## Rollback plan

Every Layer-10 write action MUST have a tested rollback before scope grant.
Per-app rollback specs live in
[ops/runbook/security-operations-agentic-rollback.md](../../../../ops/runbook/).
...
will enforce every grant in `config/scopes/security-operations.yaml` has a matching audit
