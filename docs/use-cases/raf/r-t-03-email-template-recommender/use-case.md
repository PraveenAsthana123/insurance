# Recommender-Text-3 · email-template-recommender
...
|---|---|---|---|
| `INSUR-EMAIL_TEMPLATE_RECOMMENDER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-EMAIL_TEMPLATE_RECOMMENDER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-EMAIL_TEMPLATE_RECOMMENDER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-EMAIL_TEMPLATE_RECOMMENDER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-EMAIL_TEMPLATE_RECOMMENDER-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-EMAIL_TEMPLATE_RECOMMENDER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
