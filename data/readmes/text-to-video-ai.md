# Text-to-Video AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `text-to-video-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for text-to-video ai tasks in
...
**FR-1**: System MUST accept input matching `text-to-video-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/text-to-video-ai/predict
        │
        ▼
...
- `predictions_text-to-video-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/text-to-video-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/text-to-video-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/text-to-video-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/text-to-video-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `text-to-video-ai_embeddings`
...
**Fairness metrics** (from data/fairness/text-to-video-ai.json):
