...
WHAT broke?      Check `/data/metrics/audit-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/audit-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/audit-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/audit-ai/model.joblib.backup models/audit-ai/model.joblib`
