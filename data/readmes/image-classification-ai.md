# Image Classification AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `image-classification-ai` · **Kind**: `cv_pretrained`
**Accuracy**: 1.0 · **Algorithm**: RandomForest on synthetic ResNet50-like features

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for image classification ai tasks in
...
**FR-1**: System MUST accept input matching `image-classification-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/image-classification-ai/predict
        │
        ▼
...
- `predictions_image-classification-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/image-classification-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/image-classification-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/image-classification-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/image-classification-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Used**: yes (cv_pretrained)

**Schema** (Qdrant collection · §123 stack):
- collection: `image-classification-ai_embeddings`
...
**Fairness metrics** (from data/fairness/image-classification-ai.json):
