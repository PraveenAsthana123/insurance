...
WHAT broke?      Check `/data/metrics/taxonomy-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/taxonomy-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/taxonomy-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/taxonomy-ai/model.joblib.backup models/taxonomy-ai/model.joblib`
