# Pipeline Checklist · training-content-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `training_content_rec_raw` |
| Cleaned | Postgres | `training_content_rec_clean` |
| Features | Feast / Postgres | `training_content_rec_features` |
| Predictions | Postgres | `training_content_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/training-content-rec/` |
...
1. Input → `training_content_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `training_content_rec_predictions`
4. Log → ELK / Loki
