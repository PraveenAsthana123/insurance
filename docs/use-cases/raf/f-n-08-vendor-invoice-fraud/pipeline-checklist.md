# Pipeline Checklist · vendor-invoice-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `vendor_invoice_fraud_raw` |
| Cleaned | Postgres | `vendor_invoice_fraud_clean` |
| Features | Feast / Postgres | `vendor_invoice_fraud_features` |
| Predictions | Postgres | `vendor_invoice_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/vendor-invoice-fraud/` |
...
1. Input → `vendor_invoice_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `vendor_invoice_fraud_predictions`
4. Log → ELK / Loki
