# Graph-of-Thought AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `graph-of-thought-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for graph-of-thought ai tasks in
...
**FR-1**: System MUST accept input matching `graph-of-thought-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/graph-of-thought-ai/predict
        │
        ▼
...
- `predictions_graph-of-thought-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/graph-of-thought-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/graph-of-thought-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/graph-of-thought-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/graph-of-thought-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `graph-of-thought-ai_embeddings`
...
**Fairness metrics** (from data/fairness/graph-of-thought-ai.json):
