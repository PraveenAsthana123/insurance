# Pipeline Checklist · loss-ratio-drift-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `loss_ratio_drift_anomaly_raw` |
| Cleaned | Postgres | `loss_ratio_drift_anomaly_clean` |
| Features | Feast / Postgres | `loss_ratio_drift_anomaly_features` |
| Predictions | Postgres | `loss_ratio_drift_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/loss-ratio-drift-anomaly/` |
...
1. Input → `loss_ratio_drift_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `loss_ratio_drift_anomaly_predictions`
4. Log → ELK / Loki
