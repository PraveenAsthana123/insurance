# Reinforcement Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `reinforcement-learning` · **Kind**: `spec_only`
**Accuracy**: 0.93 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for reinforcement learning tasks in
...
**FR-1**: System MUST accept input matching `reinforcement-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/reinforcement-learning/predict
        │
        ▼
...
- `predictions_reinforcement-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/reinforcement-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/reinforcement-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/reinforcement-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/reinforcement-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `reinforcement-learning_embeddings`
...
**Fairness metrics** (from data/fairness/reinforcement-learning.json):
