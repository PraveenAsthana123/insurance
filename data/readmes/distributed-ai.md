# Distributed AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `distributed-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for distributed ai tasks in
...
**FR-1**: System MUST accept input matching `distributed-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/distributed-ai/predict
        │
        ▼
...
- `predictions_distributed-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/distributed-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/distributed-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/distributed-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/distributed-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `distributed-ai_embeddings`
...
**Fairness metrics** (from data/fairness/distributed-ai.json):
