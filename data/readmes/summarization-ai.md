# Summarization AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `summarization-ai` · **Kind**: `nlp_baseline`
...
**Business goal**: Reduce manual effort + improve accuracy for summarization ai tasks in
...
**FR-1**: System MUST accept input matching `summarization-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/summarization-ai/predict
        │
        ▼
...
- `predictions_summarization-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/summarization-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/summarization-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/summarization-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/summarization-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `summarization-ai_embeddings`
...
**Fairness metrics** (from data/fairness/summarization-ai.json):
