# Pipeline Checklist · code-review-comment-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `code_review_comment_anomaly_raw` |
| Cleaned | Postgres | `code_review_comment_anomaly_clean` |
| Features | Feast / Postgres | `code_review_comment_anomaly_features` |
| Predictions | Postgres | `code_review_comment_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/code-review-comment-anomaly/` |
...
1. Input → `code_review_comment_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `code_review_comment_anomaly_predictions`
4. Log → ELK / Loki
