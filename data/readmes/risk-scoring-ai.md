# Risk Scoring AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `risk-scoring-ai` · **Kind**: `tabular_ml`
**Accuracy**: 0.969 · **Algorithm**: GradientBoostingClassifier

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for risk scoring ai tasks in
...
**FR-1**: System MUST accept input matching `risk-scoring-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/risk-scoring-ai/predict
        │
        ▼
...
- `predictions_risk-scoring-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/risk-scoring-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/risk-scoring-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/risk-scoring-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/risk-scoring-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `risk-scoring-ai_embeddings`
...
**Fairness metrics** (from data/fairness/risk-scoring-ai.json):
