# HOLY Beverage — Marketing — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Audience segment refreshes + content GenAI + attribution**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/marketing` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/marketing?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/marketing?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/marketing?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/marketing/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/marketing` returns 200 + chronological feed
