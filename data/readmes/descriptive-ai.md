# Descriptive AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `descriptive-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for descriptive ai tasks in
...
**FR-1**: System MUST accept input matching `descriptive-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/descriptive-ai/predict
        │
        ▼
...
- `predictions_descriptive-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/descriptive-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/descriptive-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/descriptive-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/descriptive-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `descriptive-ai_embeddings`
...
**Fairness metrics** (from data/fairness/descriptive-ai.json):
