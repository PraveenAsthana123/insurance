# Anomaly-Text-4 · adjuster-narrative-anomaly
...
Dept 7 Claims · BERT reconstruction
...
|---|---|---|---|
| `INSUR-ADJUSTER_NARRATIVE_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-ADJUSTER_NARRATIVE_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-ADJUSTER_NARRATIVE_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-ADJUSTER_NARRATIVE_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-ADJUSTER_NARRATIVE_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-ADJUSTER_NARRATIVE_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
