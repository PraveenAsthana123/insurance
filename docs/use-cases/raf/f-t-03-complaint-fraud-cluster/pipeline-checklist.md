# Pipeline Checklist · complaint-fraud-cluster
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `complaint_fraud_cluster_raw` |
| Cleaned | Postgres | `complaint_fraud_cluster_clean` |
| Features | Feast / Postgres | `complaint_fraud_cluster_features` |
| Predictions | Postgres | `complaint_fraud_cluster_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/complaint-fraud-cluster/` |
...
1. Input → `complaint_fraud_cluster_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `complaint_fraud_cluster_predictions`
4. Log → ELK / Loki
