# Fraud Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `fraud-detection-ai` · **Kind**: `tabular_ml`
**Accuracy**: 0.967 · **Algorithm**: GradientBoostingClassifier

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for fraud detection ai tasks in
...
**FR-1**: System MUST accept input matching `fraud-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/fraud-detection-ai/predict
        │
        ▼
...
- `predictions_fraud-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/fraud-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/fraud-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/fraud-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/fraud-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `fraud-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/fraud-detection-ai.json):
