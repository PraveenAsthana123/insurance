# EEG AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `eeg-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for eeg ai tasks in
...
**FR-1**: System MUST accept input matching `eeg-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/eeg-ai/predict
        │
        ▼
...
- `predictions_eeg-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/eeg-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/eeg-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/eeg-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/eeg-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/eeg-ai.json):
