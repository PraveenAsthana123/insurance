# Pipeline Checklist · synthetic-identity-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `synthetic_identity_fraud_raw` |
| Cleaned | Postgres | `synthetic_identity_fraud_clean` |
| Features | Feast / Postgres | `synthetic_identity_fraud_features` |
| Predictions | Postgres | `synthetic_identity_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/synthetic-identity-fraud/` |
...
1. Input → `synthetic_identity_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `synthetic_identity_fraud_predictions`
4. Log → ELK / Loki
