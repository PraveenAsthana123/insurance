...
WHAT broke?      Check `/data/metrics/dataops-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/dataops-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/dataops-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/dataops-ai/model.joblib.backup models/dataops-ai/model.joblib`
