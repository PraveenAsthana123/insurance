...
WHAT broke?      Check `/data/metrics/ragops.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ragops.json`
WHO touched?     `git log --since=<date> data/ai_types/ragops.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ragops/model.joblib.backup models/ragops/model.joblib`
