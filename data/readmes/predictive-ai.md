# Predictive AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `predictive-ai` · **Kind**: `tabular_ml`
**Accuracy**: 0.975 · **Algorithm**: GradientBoostingClassifier

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for predictive ai tasks in
...
**FR-1**: System MUST accept input matching `predictive-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/predictive-ai/predict
        │
        ▼
...
- `predictions_predictive-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/predictive-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/predictive-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/predictive-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/predictive-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `predictive-ai_embeddings`
...
**Fairness metrics** (from data/fairness/predictive-ai.json):
