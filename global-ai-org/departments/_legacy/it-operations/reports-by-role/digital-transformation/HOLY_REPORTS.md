# HOLY Beverage — It Operations — Digital Transformation Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **digital-transformation** in **it-operations** uses.
...
- **Portal**: every report retrievable at `/holy/it-operations/reports?role=digital-transformation&id=<report_id>`
...
- `request_id`, `actor_role=digital-transformation`, `dept=it-operations`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/it-operations/digital-transformation
  → list of standard reports

POST /api/v1/holy/reports/it-operations/digital-transformation/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/it-operations/digital-transformation/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/it-operations/reports?role=digital-transformation`
