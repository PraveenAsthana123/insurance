# Recommender-Image-2 · property-photo-benchmark-rec
...
Dept 4 UW · ResNet per-region index
...
|---|---|---|---|
| `INSUR-PROPERTY_PHOTO_BENCHMARK_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PROPERTY_PHOTO_BENCHMARK_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PROPERTY_PHOTO_BENCHMARK_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PROPERTY_PHOTO_BENCHMARK_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PROPERTY_PHOTO_BENCHMARK_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PROPERTY_PHOTO_BENCHMARK_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
