# HOLY Beverage — Finance — Simulation Tab
...
under `REFERENCE_PROCESSES[("finance", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/finance/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/finance/{process}/runs | List past runs |
| GET /api/v1/holy/sim/finance/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/finance/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for finance in `REFERENCE_PROCESSES` dict
