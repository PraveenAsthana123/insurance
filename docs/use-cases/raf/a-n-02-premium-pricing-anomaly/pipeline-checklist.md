# Pipeline Checklist · premium-pricing-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `premium_pricing_anomaly_raw` |
| Cleaned | Postgres | `premium_pricing_anomaly_clean` |
| Features | Feast / Postgres | `premium_pricing_anomaly_features` |
| Predictions | Postgres | `premium_pricing_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/premium-pricing-anomaly/` |
...
1. Input → `premium_pricing_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `premium_pricing_anomaly_predictions`
4. Log → ELK / Loki
