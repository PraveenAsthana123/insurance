# Computer Vision AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `computer-vision-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for computer vision ai tasks in
...
**FR-1**: System MUST accept input matching `computer-vision-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/computer-vision-ai/predict
        │
        ▼
...
- `predictions_computer-vision-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/computer-vision-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/computer-vision-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/computer-vision-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/computer-vision-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `computer-vision-ai_embeddings`
...
**Fairness metrics** (from data/fairness/computer-vision-ai.json):
