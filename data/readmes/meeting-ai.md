# Meeting AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `meeting-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for meeting ai tasks in
...
**FR-1**: System MUST accept input matching `meeting-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/meeting-ai/predict
        │
        ▼
...
- `predictions_meeting-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/meeting-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/meeting-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/meeting-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/meeting-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `meeting-ai_embeddings`
...
**Fairness metrics** (from data/fairness/meeting-ai.json):
