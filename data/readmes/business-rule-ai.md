# Business Rule AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `business-rule-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for business rule ai tasks in
...
**FR-1**: System MUST accept input matching `business-rule-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/business-rule-ai/predict
        │
        ▼
...
- `predictions_business-rule-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/business-rule-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/business-rule-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/business-rule-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/business-rule-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `business-rule-ai_embeddings`
...
**Fairness metrics** (from data/fairness/business-rule-ai.json):
