# System Design — Underwriting

Per operator 2026-06-01: "technical architect, flow internal flow, system design".

Consolidated technical architecture for the Underwriting dept. Pairs with
...
| Domain agents | 9 | Python + Ollama / LLM-router |
...
- L2 processes: 9
- L3 sub-processes: 40
- Total addressable for automation: 40
...
| Mean time to recover P1 (MTTR) | 2 hrs |
...
- ADR — Ollama vs OpenAI for Underwriting (cost + latency + data residency)
- ADR — Chunking strategy for Underwriting RAG corpus (fixed / sentence / semantic-paragraph)
- ADR — Tenant isolation: row-level vs column-level
- ADR — Decision audit retention (7y regulated vs 1y default)
- ADR — Idempotency key TTL (Underwriting retry storm pattern)
