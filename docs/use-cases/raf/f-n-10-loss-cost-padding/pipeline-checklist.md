# Pipeline Checklist · loss-cost-padding
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `loss_cost_padding_raw` |
| Cleaned | Postgres | `loss_cost_padding_clean` |
| Features | Feast / Postgres | `loss_cost_padding_features` |
| Predictions | Postgres | `loss_cost_padding_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/loss-cost-padding/` |
...
1. Input → `loss_cost_padding_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `loss_cost_padding_predictions`
4. Log → ELK / Loki
