# HOLY Beverage — Product Rd — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Concept scoring runs + feature extraction NLP calls**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/product-rd` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/product-rd?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/product-rd?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/product-rd?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/product-rd/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/product-rd` returns 200 + chronological feed
