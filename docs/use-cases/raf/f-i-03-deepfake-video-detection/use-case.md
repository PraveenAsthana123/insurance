# Fraud-Image-3 · deepfake-video-detection
...
Dept 8 SIU · Faceforensics++ + temporal CNN
...
|---|---|---|---|
| `INSUR-DEEPFAKE_VIDEO_DETECTION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DEEPFAKE_VIDEO_DETECTION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DEEPFAKE_VIDEO_DETECTION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DEEPFAKE_VIDEO_DETECTION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-DEEPFAKE_VIDEO_DETECTION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-DEEPFAKE_VIDEO_DETECTION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
