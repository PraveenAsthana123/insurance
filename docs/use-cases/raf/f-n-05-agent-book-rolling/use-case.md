# Fraud-Numerical-5 · agent-book-rolling
...
|---|---|---|---|
| `INSUR-AGENT_BOOK_ROLLING-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-AGENT_BOOK_ROLLING-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-AGENT_BOOK_ROLLING-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-AGENT_BOOK_ROLLING-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-AGENT_BOOK_ROLLING-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-AGENT_BOOK_ROLLING-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
