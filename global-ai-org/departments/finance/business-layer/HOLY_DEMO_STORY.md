# HOLY Beverage — Finance — Demo User Story
...
The persona needs to: **close books in 3 days (vs 12), cut audit findings 80%, AR DSO from 48 to 30 days**

Main KPI being moved: **Days to close, DSO, audit-finding count, forecast accuracy**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/finance` (the HOLY navigation page for this dept).
...
> "Today Amanda spends 80% of their week on manual data
...
> counterfactual. We don't replace Amanda — we replace
...
|---|---|---|---|
| 1 | Open `/holy/finance` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/finance` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_finance.py` (planned).
