# AI Workforce · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-workforce` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for ai workforce tasks in
...
**FR-1**: System MUST accept input matching `ai-workforce` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-workforce/predict
        │
        ▼
...
- `predictions_ai-workforce`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-workforce/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-workforce/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-workforce/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-workforce/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-workforce_embeddings`
...
**Fairness metrics** (from data/fairness/ai-workforce.json):
