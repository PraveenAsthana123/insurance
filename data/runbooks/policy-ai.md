...
WHAT broke?      Check `/data/metrics/policy-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/policy-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/policy-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/policy-ai/model.joblib.backup models/policy-ai/model.joblib`
