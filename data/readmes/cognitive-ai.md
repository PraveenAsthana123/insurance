# Cognitive AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `cognitive-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for cognitive ai tasks in
...
**FR-1**: System MUST accept input matching `cognitive-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/cognitive-ai/predict
        │
        ▼
...
- `predictions_cognitive-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/cognitive-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/cognitive-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/cognitive-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/cognitive-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `cognitive-ai_embeddings`
...
**Fairness metrics** (from data/fairness/cognitive-ai.json):
