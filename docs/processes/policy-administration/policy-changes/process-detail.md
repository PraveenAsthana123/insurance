...
> Department 5 · Policy Administration · slug `policy-changes` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/policy-changes/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/5/<domain>/policy-changes?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/5/<domain>/policy-changes?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/policy-changes.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/policy-administration/policy-changes/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-POLICY_CHANGES-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-POLICY_CHANGES-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-POLICY_CHANGES-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-POLICY_CHANGES-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-POLICY_CHANGES-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-POLICY_CHANGES-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-POLICY_CHANGES-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/policy-administration/policy-changes/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-POLICY_CHANGES-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/policy-administration/policy-changes/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/policy-administration/policy-changes/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/policy-administration/policy-changes/stats/`
