# RLHF · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `rlhf` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for rlhf tasks in
...
**FR-1**: System MUST accept input matching `rlhf` schema
...
- `predictions_rlhf`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/rlhf/predict` · body: features → prediction
- `GET /api/v1/ai-types/rlhf/metrics` → live metrics.json
- `GET /api/v1/ai-types/rlhf/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/rlhf/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/rlhf.json):
