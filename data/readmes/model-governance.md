# Model Governance · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `model-governance` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for model governance tasks in
...
**FR-1**: System MUST accept input matching `model-governance` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/model-governance/predict
        │
        ▼
...
- `predictions_model-governance`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/model-governance/predict` · body: features → prediction
- `GET /api/v1/ai-types/model-governance/metrics` → live metrics.json
- `GET /api/v1/ai-types/model-governance/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/model-governance/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `model-governance_embeddings`
...
**Fairness metrics** (from data/fairness/model-governance.json):
