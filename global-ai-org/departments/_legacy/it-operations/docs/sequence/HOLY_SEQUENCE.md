# HOLY Beverage — It Operations — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/it-operations
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Log Anomaly Triage**

_Log batch → embedding → anomaly score → group → incident open_
...
When a user types a goal in `/holy/it-operations/agentic`:
