...
> Department 8 · Special Investigation Unit (SIU) / Fraud · slug `fraud-monitoring` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/fraud-monitoring/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/8/<domain>/fraud-monitoring?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/8/<domain>/fraud-monitoring?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/fraud-monitoring.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/special-investigation-unit-siu-fraud/fraud-monitoring/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-FRAUD_MONITORING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-FRAUD_MONITORING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-FRAUD_MONITORING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-FRAUD_MONITORING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-FRAUD_MONITORING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-FRAUD_MONITORING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-FRAUD_MONITORING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/special-investigation-unit-siu-fraud/fraud-monitoring/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-FRAUD_MONITORING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/special-investigation-unit-siu-fraud/fraud-monitoring/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/special-investigation-unit-siu-fraud/fraud-monitoring/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/special-investigation-unit-siu-fraud/fraud-monitoring/stats/`
