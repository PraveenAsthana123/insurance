# Pipeline Checklist · caselaw-precedent-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `caselaw_precedent_rec_raw` |
| Cleaned | Postgres | `caselaw_precedent_rec_clean` |
| Features | Feast / Postgres | `caselaw_precedent_rec_features` |
| Predictions | Postgres | `caselaw_precedent_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/caselaw-precedent-rec/` |
...
1. Input → `caselaw_precedent_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `caselaw_precedent_rec_predictions`
4. Log → ELK / Loki
