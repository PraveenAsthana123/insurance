# HOLY Beverage — Manufacturing — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/manufacturing
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Defect Detection on Line**

_Camera frame → CV model → defect class → divert + alert_
...
When a user types a goal in `/holy/manufacturing/agentic`:
