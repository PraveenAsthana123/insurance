# HOLY Beverage — Customer Experience — Agentic Stack
...
> actions against and the required scope for each. The Customer Experience manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Customer Experience)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Customer Experience agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Customer Experience agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Customer Experience and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/customer-experience.yaml](../../../../config/scopes/customer-experience.yaml)
...
| Action irreversible (deletion, signing) | Manager (Customer Experience) approval |
| Confidence < 0.6 | Manager (Customer Experience) review queue |
...
`/holy/customer-experience/agentic` (per §64.40.5).

## Rollback plan

Every Layer-10 write action MUST have a tested rollback before scope grant.
Per-app rollback specs live in
[ops/runbook/customer-experience-agentic-rollback.md](../../../../ops/runbook/).
...
will enforce every grant in `config/scopes/customer-experience.yaml` has a matching audit
