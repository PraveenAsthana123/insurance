# HOLY Beverage — Marketing — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **audience-segments + brand-master + campaign-codes + channel-master**.
...
This dept (**marketing**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/marketing` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/marketing/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/marketing/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/marketing/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/marketing/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/marketing` returns 200 + entity catalog
