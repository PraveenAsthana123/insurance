...
> Anchor flow: **employee ─ assigned to position ─ predicted by attrition_model**.
...
```
employee ─ assigned to position ─ predicted by attrition_model
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in hr touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/hr` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/hr/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/hr/neighbors/<node_id>` | 1-hop neighbors of a node |
