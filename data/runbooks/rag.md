...
WHAT broke?      Check `/data/metrics/rag.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/rag.json`
WHO touched?     `git log --since=<date> data/ai_types/rag.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/rag/model.joblib.backup models/rag/model.joblib`
