...
> Department 21 · Sales, Distribution, Broker, Agency & Partner Management · slug `upsell` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/upsell/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/21/<domain>/upsell?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/21/<domain>/upsell?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/upsell.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/sales-distribution-broker-agency-partner-management/upsell/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-UPSELL-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-UPSELL-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-UPSELL-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-UPSELL-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-UPSELL-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-UPSELL-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-UPSELL-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/sales-distribution-broker-agency-partner-management/upsell/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-UPSELL-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/sales-distribution-broker-agency-partner-management/upsell/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/sales-distribution-broker-agency-partner-management/upsell/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/sales-distribution-broker-agency-partner-management/upsell/stats/`
