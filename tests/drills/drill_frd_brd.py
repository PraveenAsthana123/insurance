#!/usr/bin/env python3
"""Drill: HOLY FRD + BRD content invariants per dept (§47 + §52 + §64).

Steps (10 total; 3 negative):
    1. (+) all 19 docs/brd/HOLY_BRD.md exist + ≥ 1500B
    2. (+) all 19 docs/frd/HOLY_FRD.md exist + ≥ 2000B
    ...
    6. (+) every FRD references its parent BRD (HOLY_BRD.md link present)
    7. (+) every BRD references its child FRD (HOLY_FRD.md link present)
...
def main():
    print("\nDRILL: HOLY FRD + BRD per dept (§47 + §52 + §64)\n")
    t0 = time.time()
    ...
    for dept in DEPARTMENTS:
        brd = root / dept / "docs" / "brd" / "HOLY_BRD.md"
        if not brd.exists():
    ...
    bad_frd: list[str] = []
    for dept in DEPARTMENTS:
        frd = root / dept / "docs" / "frd" / "HOLY_FRD.md"
        if not frd.exists():
    ...
    for dept in DEPARTMENTS:
        body = (root / dept / "docs" / "brd" / "HOLY_BRD.md").read_text()
    ...
    bad_cats: list[str] = []
    for dept in DEPARTMENTS:
        body = (root / dept / "docs" / "frd" / "HOLY_FRD.md").read_text()
    ...
    fake_brd = root / fake_dept / "docs" / "brd" / "HOLY_BRD.md"
    fake_frd = root / fake_dept / "docs" / "frd" / "HOLY_FRD.md"
    ...
    for dept in DEPARTMENTS:
        for kind in ("brd", "frd"):
            f = root / dept / "docs" / kind / f"HOLY_{kind.upper()}.md"
    ...
    # We count markdown links pointing at HOLY_*.md files.
    ...
    for dept in DEPARTMENTS:
        ...
        n_links = len(set(holy_link_pat.findall(footer)))
        if n_links < 5:
