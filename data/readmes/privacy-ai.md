# Privacy AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `privacy-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for privacy ai tasks in
...
**FR-1**: System MUST accept input matching `privacy-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/privacy-ai/predict
        │
        ▼
...
- `predictions_privacy-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/privacy-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/privacy-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/privacy-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/privacy-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `privacy-ai_embeddings`
...
**Fairness metrics** (from data/fairness/privacy-ai.json):
