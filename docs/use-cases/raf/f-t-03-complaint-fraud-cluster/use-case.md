# Fraud-Text-3 · complaint-fraud-cluster
...
Dept 8 SIU · BERTopic + cluster anomaly
...
|---|---|---|---|
| `INSUR-COMPLAINT_FRAUD_CLUSTER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-COMPLAINT_FRAUD_CLUSTER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-COMPLAINT_FRAUD_CLUSTER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-COMPLAINT_FRAUD_CLUSTER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-COMPLAINT_FRAUD_CLUSTER-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-COMPLAINT_FRAUD_CLUSTER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
