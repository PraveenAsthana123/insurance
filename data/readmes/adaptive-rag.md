# Adaptive RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `adaptive-rag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for adaptive rag tasks in
...
**FR-1**: System MUST accept input matching `adaptive-rag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/adaptive-rag/predict
        │
        ▼
...
- `predictions_adaptive-rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/adaptive-rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/adaptive-rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/adaptive-rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/adaptive-rag/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `adaptive-rag_embeddings`
...
**Fairness metrics** (from data/fairness/adaptive-rag.json):
