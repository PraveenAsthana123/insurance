#!/usr/bin/env python3
"""Drill: HOLY transactional history router (§38 + §41.3 + §47.6 + §57.5 + §57.6 + §64).
...
Steps (10 total; 3 negative):
   ...
   10. (+) HOLY_TRANSACTIONS.md exists per dept (under business-layer/)
...
def main():
    print("\nDRILL: HOLY transactional history (§38 + §47.6 + §57.6 + §66)\n")
    t0 = time.time()
    ...
    has_catalog = (
        ...
        and hasattr(txn, "HOLY_DEPTS") and len(txn.HOLY_DEPTS) == 19
        and hasattr(txn, "PII_TOKENS") and len(txn.PII_TOKENS) >= 8
    )
    step(1, "router + CRON_AUDIT_DIRS + HOLY_DEPTS + PII_TOKENS populated",
         has_catalog,
         f"cron={len(txn.CRON_AUDIT_DIRS)} depts={len(txn.HOLY_DEPTS)} pii={len(txn.PII_TOKENS)}")
    ...
    r = client.get("/api/v1/holy/transactions/sales")
    ...
    r = client.get("/api/v1/holy/transactions/not-a-real-dept")
    ...
    r = client.get("/api/v1/holy/transactions/sales?limit=1000")
    ...
    r = client.get("/api/v1/holy/transactions/sales?source=cron")
    body = r.json() if r.status_code == 200 else {}
    leaked_non_cron = [
    ...
    r = client.get(f"/api/v1/holy/transactions/sales?since_epoch={future}")
    body = r.json() if r.status_code == 200 else {}
    ...
    r = client.get("/api/v1/holy/transactions/_global")
    body = r.json() if r.status_code == 200 else {}
    ...
    # ----- Step 10: HOLY_TRANSACTIONS.md per dept -----
    ...
    missing_md = [
        ...
        if not (gao / "departments" / dept / "business-layer" / "HOLY_TRANSACTIONS.md").exists()
    ]
    step(10, f"HOLY_TRANSACTIONS.md exists for all {len(EXPECTED_DEPTS)} depts",
