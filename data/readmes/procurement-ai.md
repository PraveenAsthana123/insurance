# Procurement AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `procurement-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for procurement ai tasks in
...
**FR-1**: System MUST accept input matching `procurement-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/procurement-ai/predict
        │
        ▼
...
- `predictions_procurement-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/procurement-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/procurement-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/procurement-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/procurement-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `procurement-ai_embeddings`
...
**Fairness metrics** (from data/fairness/procurement-ai.json):
