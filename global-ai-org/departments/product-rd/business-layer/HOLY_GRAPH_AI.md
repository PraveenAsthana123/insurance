# HOLY Beverage — Product Rd — Graph AI
...
> Anchor flow: **concept ─ scored by NLP ─ becomes product**.
...
```
concept ─ scored by NLP ─ becomes product
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in product-rd touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/product-rd` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/product-rd/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/product-rd/neighbors/<node_id>` | 1-hop neighbors of a node |
