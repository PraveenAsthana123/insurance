# Neuro AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `neuro-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for neuro ai tasks in
...
**FR-1**: System MUST accept input matching `neuro-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/neuro-ai/predict
        │
        ▼
...
- `predictions_neuro-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/neuro-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/neuro-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/neuro-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/neuro-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `neuro-ai_embeddings`
...
**Fairness metrics** (from data/fairness/neuro-ai.json):
