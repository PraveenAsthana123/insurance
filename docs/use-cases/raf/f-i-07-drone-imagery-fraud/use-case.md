# Fraud-Image-7 · drone-imagery-fraud
...
Dept 11 Reinsurance · Siamese + EXIF cross-check
...
|---|---|---|---|
| `INSUR-DRONE_IMAGERY_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DRONE_IMAGERY_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DRONE_IMAGERY_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DRONE_IMAGERY_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-DRONE_IMAGERY_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-DRONE_IMAGERY_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
