...
> Department 7 · Claims · slug `coverage-verification` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/coverage-verification/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/7/<domain>/coverage-verification?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/7/<domain>/coverage-verification?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/coverage-verification.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/claims/coverage-verification/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-COVERAGE_VERIFICATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-COVERAGE_VERIFICATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-COVERAGE_VERIFICATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-COVERAGE_VERIFICATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-COVERAGE_VERIFICATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-COVERAGE_VERIFICATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-COVERAGE_VERIFICATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/claims/coverage-verification/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-COVERAGE_VERIFICATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/claims/coverage-verification/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/claims/coverage-verification/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/claims/coverage-verification/stats/`
