# HOLY Beverage — IT Operations — Agentic Stack
...
> actions against and the required scope for each. The IT Operations manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (IT Operations)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every IT Operations agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives IT Operations agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between IT Operations and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/it-operations.yaml](../../../../config/scopes/it-operations.yaml)
...
| Action irreversible (deletion, signing) | Manager (IT Operations) approval |
| Confidence < 0.6 | Manager (IT Operations) review queue |
...
will enforce every grant in `config/scopes/it-operations.yaml` has a matching audit
