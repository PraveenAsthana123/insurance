# Pipeline Checklist · claim-shop-collusion
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `claim_shop_collusion_raw` |
| Cleaned | Postgres | `claim_shop_collusion_clean` |
| Features | Feast / Postgres | `claim_shop_collusion_features` |
| Predictions | Postgres | `claim_shop_collusion_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/claim-shop-collusion/` |
...
1. Input → `claim_shop_collusion_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `claim_shop_collusion_predictions`
4. Log → ELK / Loki
