...
> Department 1 · Product Management Department · slug `pricing` · AI items: 6
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/pricing/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/1/<domain>/pricing?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/1/<domain>/pricing?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/pricing.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/product-management-department/pricing/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRICING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRICING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRICING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRICING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRICING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRICING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRICING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/product-management-department/pricing/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRICING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/product-management-department/pricing/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/product-management-department/pricing/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/product-management-department/pricing/stats/`
