# HOLY Beverage — Legal — Monitoring AI
...
> Persona: **Legal Counsel**. Focus: **contract-clause extraction NLP + risk-flag precision**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/legal` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/legal/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/legal/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #legal-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #legal-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/legal` returns 200 + job list
