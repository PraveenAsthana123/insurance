...
WHAT broke?      Check `/data/metrics/clinical-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/clinical-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/clinical-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/clinical-ai/model.joblib.backup models/clinical-ai/model.joblib`
