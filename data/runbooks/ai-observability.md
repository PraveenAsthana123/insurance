...
WHAT broke?      Check `/data/metrics/ai-observability.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ai-observability.json`
WHO touched?     `git log --since=<date> data/ai_types/ai-observability.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ai-observability/model.joblib.backup models/ai-observability/model.joblib`
