...
any 3-5 to assemble a It Operations board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in It Operations
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is Admin (System)'s view of It Operations. In 90 seconds we'll move MTTD + incident volume by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: Dept Manager in It Operations
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is Dept Manager's view of It Operations. In 90 seconds we'll move MTTD + incident volume by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: Team Member in It Operations
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is Team Member's view of It Operations. In 90 seconds we'll move MTTD + incident volume by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in It Operations
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is QA / Tester's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: SecOps Engineer in It Operations
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is SecOps Engineer's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in It Operations
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is DevOps's view of It Operations. In 90 seconds we'll move MTTD + incident volume by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: AI Reviewer in It Operations
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is AI Reviewer's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: DT Lead in It Operations
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is DT Lead's view of It Operations. In 90 seconds we'll move MTTD + incident volume by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: System Architect in It Operations
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is System Architect's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: Test Architect in It Operations
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is Test Architect's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: DB Architect in It Operations
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is DB Architect's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: API Architect in It Operations
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is API Architect's view of It Operations. In 90 seconds we'll move MTTD + incident volume by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: Data Owner in It Operations
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is Data Owner's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: AI Strategy Lead in It Operations
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is AI Strategy Lead's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
- **Persona**: InfoSec / CISO Office in It Operations
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the It Operations dashboards
- **KPI moved**: MTTD + incident volume
...
- **Talking points**: "This is InfoSec / CISO Office's view of It Operations. In 90 seconds we'll move MTTD + incident volume by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/it-operations` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/it-operations` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/it-operations/<role>` | Single role demo detail |
