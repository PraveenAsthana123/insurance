# Self-RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `self-rag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for self-rag tasks in
...
**FR-1**: System MUST accept input matching `self-rag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/self-rag/predict
        │
        ▼
...
- `predictions_self-rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/self-rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/self-rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/self-rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/self-rag/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `self-rag_embeddings`
...
**Fairness metrics** (from data/fairness/self-rag.json):
