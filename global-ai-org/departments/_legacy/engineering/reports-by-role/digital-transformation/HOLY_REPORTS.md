# HOLY Beverage — Engineering — Digital Transformation Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **digital-transformation** in **engineering** uses.
...
- **Portal**: every report retrievable at `/holy/engineering/reports?role=digital-transformation&id=<report_id>`
...
- `request_id`, `actor_role=digital-transformation`, `dept=engineering`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/engineering/digital-transformation
  → list of standard reports

POST /api/v1/holy/reports/engineering/digital-transformation/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/engineering/digital-transformation/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/engineering/reports?role=digital-transformation`
