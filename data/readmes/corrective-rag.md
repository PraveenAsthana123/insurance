# Corrective RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `corrective-rag` · **Kind**: `rag_variant`
**Accuracy**: 1.0 · **Algorithm**: TF-IDF + cosine retrieval (upgrade to BGE-M3 in prod)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for corrective rag tasks in
...
**FR-1**: System MUST accept input matching `corrective-rag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/corrective-rag/predict
        │
        ▼
...
- `predictions_corrective-rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/corrective-rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/corrective-rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/corrective-rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/corrective-rag/model.joblib]
            ↘ [preprocessor.py]
...
**Used**: yes (rag_variant)

**Schema** (Qdrant collection · §123 stack):
- collection: `corrective-rag_embeddings`
...
**Fairness metrics** (from data/fairness/corrective-rag.json):
