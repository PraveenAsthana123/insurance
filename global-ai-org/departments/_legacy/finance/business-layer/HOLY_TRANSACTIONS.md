# HOLY Beverage — Finance — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Fraud decisions + forecast runs + close-process events**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/finance` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/finance?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/finance?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/finance?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/finance/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/finance` returns 200 + chronological feed
