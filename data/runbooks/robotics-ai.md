...
WHAT broke?      Check `/data/metrics/robotics-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/robotics-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/robotics-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/robotics-ai/model.joblib.backup models/robotics-ai/model.joblib`
