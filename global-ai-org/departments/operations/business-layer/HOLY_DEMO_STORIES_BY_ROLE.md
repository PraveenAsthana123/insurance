...
any 3-5 to assemble a Operations board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Operations
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is Admin (System)'s view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: Dept Manager in Operations
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is Dept Manager's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: Team Member in Operations
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is Team Member's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Operations
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is QA / Tester's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: SecOps Engineer in Operations
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is SecOps Engineer's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Operations
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is DevOps's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: AI Reviewer in Operations
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is AI Reviewer's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: DT Lead in Operations
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is DT Lead's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: System Architect in Operations
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is System Architect's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: Test Architect in Operations
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is Test Architect's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: DB Architect in Operations
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is DB Architect's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: API Architect in Operations
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is API Architect's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: Data Owner in Operations
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is Data Owner's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: AI Strategy Lead in Operations
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is AI Strategy Lead's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
- **Persona**: InfoSec / CISO Office in Operations
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Operations dashboards
- **KPI moved**: Process cycle + anomaly rate
...
- **Talking points**: "This is InfoSec / CISO Office's view of Operations. In 90 seconds we'll move Process cycle + anomaly rate by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/operations` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/operations` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/operations/<role>` | Single role demo detail |
