# Vector Search AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `vector-search-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for vector search ai tasks in
...
**FR-1**: System MUST accept input matching `vector-search-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/vector-search-ai/predict
        │
        ▼
...
- `predictions_vector-search-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/vector-search-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/vector-search-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/vector-search-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/vector-search-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `vector-search-ai_embeddings`
...
**Fairness metrics** (from data/fairness/vector-search-ai.json):
