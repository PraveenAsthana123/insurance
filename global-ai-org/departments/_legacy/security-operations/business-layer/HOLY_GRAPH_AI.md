# HOLY Beverage — Security Operations — Graph AI
...
> Anchor flow: **alert ─ classified by threat_model ─ enriches with threat_intel**.
...
```
alert ─ classified by threat_model ─ enriches with threat_intel
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in security-operations touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/security-operations` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/security-operations/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/security-operations/neighbors/<node_id>` | 1-hop neighbors of a node |
