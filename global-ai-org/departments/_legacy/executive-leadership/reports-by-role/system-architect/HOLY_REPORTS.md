# HOLY Beverage — Executive Leadership — System Architect Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **system-architect** in **executive-leadership** uses.

## Standard Reports

| # | Report Name | Cadence | Format | Recipients |
|---|---|---|---|---|
| 1 | Monthly architecture review | monthly | PDF | manager + eng |
| 2 | Quarterly capacity plan | quarterly | PDF | manager + finops |
| 3 | ADR landing summary | on-demand | PDF | architecture council |
...
- **Portal**: every report retrievable at `/holy/executive-leadership/reports?role=system-architect&id=<report_id>`
...
- `request_id`, `actor_role=system-architect`, `dept=executive-leadership`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/executive-leadership/system-architect
  → list of standard reports

POST /api/v1/holy/reports/executive-leadership/system-architect/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/executive-leadership/system-architect/<report_id>/runs/<run_id>
  → status + signed download URL
```

## Related Artifacts

- `../../dashboards-by-role/system-architect/HOLY_DASHBOARD.md` — paired dashboard
- `../../roles/system-architect/HOLY_README.md` — role definition
- Frontend route: `/holy/executive-leadership/reports?role=system-architect`
