# Sentiment AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `sentiment-ai` · **Kind**: `nlp_baseline`
...
**Business goal**: Reduce manual effort + improve accuracy for sentiment ai tasks in
...
**FR-1**: System MUST accept input matching `sentiment-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/sentiment-ai/predict
        │
        ▼
...
- `predictions_sentiment-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/sentiment-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/sentiment-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/sentiment-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/sentiment-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `sentiment-ai_embeddings`
...
**Fairness metrics** (from data/fairness/sentiment-ai.json):
