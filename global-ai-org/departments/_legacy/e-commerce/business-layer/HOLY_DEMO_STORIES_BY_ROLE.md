...
any 3-5 to assemble a E Commerce board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in E Commerce
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is Admin (System)'s view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: Dept Manager in E Commerce
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is Dept Manager's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: Team Member in E Commerce
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is Team Member's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in E Commerce
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is QA / Tester's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: SecOps Engineer in E Commerce
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is SecOps Engineer's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in E Commerce
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is DevOps's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: AI Reviewer in E Commerce
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is AI Reviewer's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: DT Lead in E Commerce
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is DT Lead's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: System Architect in E Commerce
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is System Architect's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: Test Architect in E Commerce
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is Test Architect's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: DB Architect in E Commerce
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is DB Architect's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: API Architect in E Commerce
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is API Architect's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: Data Owner in E Commerce
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is Data Owner's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: AI Strategy Lead in E Commerce
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is AI Strategy Lead's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
- **Persona**: InfoSec / CISO Office in E Commerce
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the E Commerce dashboards
- **KPI moved**: Cart conversion + AOV
...
- **Talking points**: "This is InfoSec / CISO Office's view of E Commerce. In 90 seconds we'll move Cart conversion + AOV by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/e-commerce` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/e-commerce` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/e-commerce/<role>` | Single role demo detail |
