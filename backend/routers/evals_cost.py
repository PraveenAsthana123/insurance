...
4 endpoints under /api/v1/holy/evals/cost/* federated via core.holy_audit
(surface=evals_cost). Sibling of §68.8 functional + §68.10 safety; RBAC
catch-all `/api/v1/holy/evals/*` already gates this surface.
...
router = APIRouter(
    prefix="/api/v1/holy/evals/cost",
    tags=["holy", "evals", "cost"],
)
...
def cost_global(http_request: Request) -> dict[str, Any]:
    ...
    log_holy_access(http_request, "evals_cost", "cost_global")
...
def cost_by_model(
    ...
    log_holy_access(http_request, "evals_cost", "cost_by_model",
                    extra={"since": since})
...
def cost_by_request(http_request: Request, request_id: str) -> dict[str, Any]:
    ...
    log_holy_access(http_request, "evals_cost", "cost_by_request",
...
def cost_per_tenant(
    ...
    log_holy_access(http_request, "evals_cost", "cost_per_tenant",
