# Satellite AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `satellite-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for satellite ai tasks in
...
**FR-1**: System MUST accept input matching `satellite-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/satellite-ai/predict
        │
        ▼
...
- `predictions_satellite-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/satellite-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/satellite-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/satellite-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/satellite-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `satellite-ai_embeddings`
...
**Fairness metrics** (from data/fairness/satellite-ai.json):
