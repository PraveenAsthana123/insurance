# XR AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `xr-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for xr ai tasks in
...
**FR-1**: System MUST accept input matching `xr-ai` schema
...
- `predictions_xr-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/xr-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/xr-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/xr-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/xr-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/xr-ai.json):
