# Pipeline Checklist · kb-article-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `kb_article_rec_raw` |
| Cleaned | Postgres | `kb_article_rec_clean` |
| Features | Feast / Postgres | `kb_article_rec_features` |
| Predictions | Postgres | `kb_article_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/kb-article-rec/` |
...
1. Input → `kb_article_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `kb_article_rec_predictions`
4. Log → ELK / Loki
