# Anomaly-Image-5 · network-topology-anomaly
...
Dept 19 IT · OCR + structure embed
...
|---|---|---|---|
| `INSUR-NETWORK_TOPOLOGY_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-NETWORK_TOPOLOGY_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-NETWORK_TOPOLOGY_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-NETWORK_TOPOLOGY_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-NETWORK_TOPOLOGY_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-NETWORK_TOPOLOGY_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
