# HOLY Beverage — Customer Support Network Flow
...
```
Customer Support API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Customer Support API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          customer-support_user_events       analytics-svc
Backend API       ──►          customer-support_audit              audit-store
ML Worker         ──►          customer-support_predictions        prediction-store
Drift detector    ──►          customer-support_alerts             on-call PagerDuty
...
|---|---|---|---|
| customer-support-api | postgresql | 5432 | TCP |
| customer-support-api | redis | 6379 | TCP |
| customer-support-api | ollama | 11434 | TCP |
| customer-support-api | otel-collector | 4317 | gRPC |
| customer-support-api | (other dept APIs) | 8000 | HTTP via service mesh |
