...
WHAT broke?      Check `/data/metrics/meeting-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/meeting-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/meeting-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/meeting-ai/model.joblib.backup models/meeting-ai/model.joblib`
