...
> Department 20 · Cybersecurity, Identity & Fraud Defense · slug `security-governance` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/security-governance/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/20/<domain>/security-governance?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/20/<domain>/security-governance?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/security-governance.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/cybersecurity-identity-fraud-defense/security-governance/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-SECURITY_GOVERNANCE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-SECURITY_GOVERNANCE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-SECURITY_GOVERNANCE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-SECURITY_GOVERNANCE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-SECURITY_GOVERNANCE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-SECURITY_GOVERNANCE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-SECURITY_GOVERNANCE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/cybersecurity-identity-fraud-defense/security-governance/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-SECURITY_GOVERNANCE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/cybersecurity-identity-fraud-defense/security-governance/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/cybersecurity-identity-fraud-defense/security-governance/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/cybersecurity-identity-fraud-defense/security-governance/stats/`
