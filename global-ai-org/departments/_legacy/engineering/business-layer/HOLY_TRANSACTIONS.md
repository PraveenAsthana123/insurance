# HOLY Beverage — Engineering — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **PR review scores + flaky-test predictions + deploy decisions**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/engineering` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/engineering?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/engineering?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/engineering?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/engineering/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/engineering` returns 200 + chronological feed
