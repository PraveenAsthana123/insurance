# HOLY Beverage — Procurement — Admin Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **admin** in **procurement** uses.
...
- **Portal**: every report retrievable at `/holy/procurement/reports?role=admin&id=<report_id>`
...
- `request_id`, `actor_role=admin`, `dept=procurement`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/procurement/admin
  → list of standard reports

POST /api/v1/holy/reports/procurement/admin/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/procurement/admin/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/procurement/reports?role=admin`
