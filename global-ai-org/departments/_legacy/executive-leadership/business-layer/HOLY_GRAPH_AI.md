# HOLY Beverage — Executive Leadership — Graph AI
...
> Anchor flow: **kpi ─ rolled up from dept ─ visible on board_pack**.
...
```
kpi ─ rolled up from dept ─ visible on board_pack
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in executive-leadership touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/executive-leadership` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/executive-leadership/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/executive-leadership/neighbors/<node_id>` | 1-hop neighbors of a node |
