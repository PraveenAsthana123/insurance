# Clinical AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `clinical-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for clinical ai tasks in
...
**FR-1**: System MUST accept input matching `clinical-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/clinical-ai/predict
        │
        ▼
...
- `predictions_clinical-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/clinical-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/clinical-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/clinical-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/clinical-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `clinical-ai_embeddings`
...
**Fairness metrics** (from data/fairness/clinical-ai.json):
