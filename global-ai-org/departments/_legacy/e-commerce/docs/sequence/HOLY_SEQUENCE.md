# HOLY Beverage — E Commerce — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/e-commerce
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Cart Abandonment Recovery**

_Cart abandoned event → propensity score → channel → message_
...
When a user types a goal in `/holy/e-commerce/agentic`:
