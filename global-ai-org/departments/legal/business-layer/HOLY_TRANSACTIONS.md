# HOLY Beverage — Legal — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Contract reviews + clause risk scoring + compliance evidence**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/legal` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/legal?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/legal?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/legal?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/legal/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/legal` returns 200 + chronological feed
