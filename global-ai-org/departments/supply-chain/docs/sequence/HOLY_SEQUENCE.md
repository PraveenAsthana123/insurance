# HOLY Beverage — Supply Chain — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/supply-chain
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Demand Forecast Refresh**

_POS data → feature build → forecast model → reorder recommendations_
...
When a user types a goal in `/holy/supply-chain/agentic`:
