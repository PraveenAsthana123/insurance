# Fraud-Numerical-3 · hard-fraud-staged-claim
...
Dept 8 SIU · XGB + class-weight + SMOTE
...
|---|---|---|---|
| `INSUR-HARD_FRAUD_STAGED_CLAIM-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HARD_FRAUD_STAGED_CLAIM-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HARD_FRAUD_STAGED_CLAIM-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HARD_FRAUD_STAGED_CLAIM-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-HARD_FRAUD_STAGED_CLAIM-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-HARD_FRAUD_STAGED_CLAIM-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
