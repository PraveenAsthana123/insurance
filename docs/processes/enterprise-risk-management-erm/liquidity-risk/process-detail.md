...
> Department 15 · Enterprise Risk Management (ERM) · slug `liquidity-risk` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/liquidity-risk/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/15/<domain>/liquidity-risk?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/15/<domain>/liquidity-risk?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/liquidity-risk.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/enterprise-risk-management-erm/liquidity-risk/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-LIQUIDITY_RISK-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-LIQUIDITY_RISK-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-LIQUIDITY_RISK-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-LIQUIDITY_RISK-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-LIQUIDITY_RISK-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-LIQUIDITY_RISK-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-LIQUIDITY_RISK-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/enterprise-risk-management-erm/liquidity-risk/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-LIQUIDITY_RISK-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/enterprise-risk-management-erm/liquidity-risk/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/enterprise-risk-management-erm/liquidity-risk/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/enterprise-risk-management-erm/liquidity-risk/stats/`
