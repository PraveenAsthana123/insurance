# HOLY Beverage — Legal — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/legal
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Contract Clause Risk**

_Doc upload → NLP extract → clause classifier → risk-flag + escalate_
...
When a user types a goal in `/holy/legal/agentic`:
