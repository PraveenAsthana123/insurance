# HOLY Beverage — Digital Marketing — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/digital-marketing
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Campaign Attribution**

_Customer interaction → MTA model → channel scoring → recommendation_
...
When a user types a goal in `/holy/digital-marketing/agentic`:
