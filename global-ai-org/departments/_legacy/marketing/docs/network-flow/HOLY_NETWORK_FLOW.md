# HOLY Beverage — Marketing Network Flow
...
```
Marketing API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Marketing API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          marketing_user_events       analytics-svc
Backend API       ──►          marketing_audit              audit-store
ML Worker         ──►          marketing_predictions        prediction-store
Drift detector    ──►          marketing_alerts             on-call PagerDuty
...
|---|---|---|---|
| marketing-api | postgresql | 5432 | TCP |
| marketing-api | redis | 6379 | TCP |
| marketing-api | ollama | 11434 | TCP |
| marketing-api | otel-collector | 4317 | gRPC |
| marketing-api | (other dept APIs) | 8000 | HTTP via service mesh |
