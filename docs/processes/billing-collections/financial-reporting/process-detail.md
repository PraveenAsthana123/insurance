...
> Department 6 · Billing & Collections · slug `financial-reporting` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/financial-reporting/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/6/<domain>/financial-reporting?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/6/<domain>/financial-reporting?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/financial-reporting.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/billing-collections/financial-reporting/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-FINANCIAL_REPORTING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-FINANCIAL_REPORTING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-FINANCIAL_REPORTING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-FINANCIAL_REPORTING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-FINANCIAL_REPORTING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-FINANCIAL_REPORTING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-FINANCIAL_REPORTING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/billing-collections/financial-reporting/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-FINANCIAL_REPORTING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/billing-collections/financial-reporting/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/billing-collections/financial-reporting/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/billing-collections/financial-reporting/stats/`
