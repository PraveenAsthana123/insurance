# Anomaly-Text-5 · regulatory-language-drift
...
Dept 12 Compliance · embed drift
...
|---|---|---|---|
| `INSUR-REGULATORY_LANGUAGE_DRIFT-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-REGULATORY_LANGUAGE_DRIFT-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-REGULATORY_LANGUAGE_DRIFT-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-REGULATORY_LANGUAGE_DRIFT-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-REGULATORY_LANGUAGE_DRIFT-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-REGULATORY_LANGUAGE_DRIFT-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
