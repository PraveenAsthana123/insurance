# FinOps AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `finops-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for finops ai tasks in
...
**FR-1**: System MUST accept input matching `finops-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/finops-ai/predict
        │
        ▼
...
- `predictions_finops-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/finops-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/finops-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/finops-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/finops-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `finops-ai_embeddings`
...
**Fairness metrics** (from data/fairness/finops-ai.json):
