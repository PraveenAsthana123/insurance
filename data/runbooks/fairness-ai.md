...
WHAT broke?      Check `/data/metrics/fairness-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/fairness-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/fairness-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/fairness-ai/model.joblib.backup models/fairness-ai/model.joblib`
