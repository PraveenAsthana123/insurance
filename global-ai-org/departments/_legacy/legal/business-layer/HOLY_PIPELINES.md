# HOLY Beverage — Legal — Automated Pipelines
...
|---|---|---|---|
| 1 | Contract Clause Risk | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Contract Clause Risk + Compliance Evidence + Precedent Search**._
...
## 3.1 — Contract Clause Risk

**Lifecycle type:** Tabular ML (default — extend per dept-specific lifecycle)  
**Audit prefix:** `pipeline.legal.contract_clause_risk`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream legal systems | `outcome=data_received` |
...
| 5 Report | Legal manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/legal` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/legal/<process_id>` | Full 5-phase spec for one process |
