...
WHAT broke?      Check `/data/metrics/asi.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/asi.json`
WHO touched?     `git log --since=<date> data/ai_types/asi.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/asi/model.joblib.backup models/asi/model.joblib`
