# HOLY Beverage — Security Operations — Data Owner Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **data-owner** in **security-operations** uses.
...
- **Portal**: every report retrievable at `/holy/security-operations/reports?role=data-owner&id=<report_id>`
...
- `request_id`, `actor_role=data-owner`, `dept=security-operations`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/security-operations/data-owner
  → list of standard reports

POST /api/v1/holy/reports/security-operations/data-owner/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/security-operations/data-owner/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/security-operations/reports?role=data-owner`
