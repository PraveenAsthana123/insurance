# GraphRAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `graphrag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for graphrag tasks in
...
**FR-1**: System MUST accept input matching `graphrag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/graphrag/predict
        │
        ▼
...
- `predictions_graphrag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/graphrag/predict` · body: features → prediction
- `GET /api/v1/ai-types/graphrag/metrics` → live metrics.json
- `GET /api/v1/ai-types/graphrag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/graphrag/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `graphrag_embeddings`
...
**Fairness metrics** (from data/fairness/graphrag.json):
