# HOLY Beverage — Sales — Demo User Story
...
The persona needs to: **raise quota attainment from 62% to 80% and shorten cycle from 95 to 50 days**

Main KPI being moved: **Quota attainment, sales cycle length, win rate**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/sales` (the HOLY navigation page for this dept).
...
> "Today Tom spends 80% of their week on manual data
...
> counterfactual. We don't replace Tom — we replace
...
|---|---|---|---|
| 1 | Open `/holy/sales` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/sales` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_sales.py` (planned).
