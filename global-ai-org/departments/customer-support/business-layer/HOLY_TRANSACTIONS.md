# HOLY Beverage — Customer Support — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Intent classifications + RAG resolutions + escalation events**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/customer-support` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/customer-support?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/customer-support?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/customer-support?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/customer-support/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/customer-support` returns 200 + chronological feed
