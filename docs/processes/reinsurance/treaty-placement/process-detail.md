...
> Department 11 · Reinsurance · slug `treaty-placement` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/treaty-placement/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/11/<domain>/treaty-placement?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/11/<domain>/treaty-placement?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/treaty-placement.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/reinsurance/treaty-placement/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-TREATY_PLACEMENT-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-TREATY_PLACEMENT-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-TREATY_PLACEMENT-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-TREATY_PLACEMENT-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-TREATY_PLACEMENT-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-TREATY_PLACEMENT-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-TREATY_PLACEMENT-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/reinsurance/treaty-placement/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-TREATY_PLACEMENT-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/reinsurance/treaty-placement/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/reinsurance/treaty-placement/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/reinsurance/treaty-placement/stats/`
