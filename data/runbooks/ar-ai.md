...
WHAT broke?      Check `/data/metrics/ar-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ar-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/ar-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ar-ai/model.joblib.backup models/ar-ai/model.joblib`
