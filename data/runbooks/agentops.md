...
WHAT broke?      Check `/data/metrics/agentops.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/agentops.json`
WHO touched?     `git log --since=<date> data/ai_types/agentops.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/agentops/model.joblib.backup models/agentops/model.joblib`
