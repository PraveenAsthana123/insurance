# Recommender-Image-6 · doc-template-image-rec
...
Dept 5 Policy Admin · LayoutLMv3 + vector
...
|---|---|---|---|
| `INSUR-DOC_TEMPLATE_IMAGE_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DOC_TEMPLATE_IMAGE_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DOC_TEMPLATE_IMAGE_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DOC_TEMPLATE_IMAGE_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-DOC_TEMPLATE_IMAGE_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-DOC_TEMPLATE_IMAGE_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
