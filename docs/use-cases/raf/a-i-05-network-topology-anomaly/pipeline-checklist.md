# Pipeline Checklist · network-topology-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `network_topology_anomaly_raw` |
| Cleaned | Postgres | `network_topology_anomaly_clean` |
| Features | Feast / Postgres | `network_topology_anomaly_features` |
| Predictions | Postgres | `network_topology_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/network-topology-anomaly/` |
...
1. Input → `network_topology_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `network_topology_anomaly_predictions`
4. Log → ELK / Loki
