# Anomaly-Numerical-9 · uw-decision-drift
...
Dept 4 UW · KS-test + per-cohort drift
...
|---|---|---|---|
| `INSUR-UW_DECISION_DRIFT-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-UW_DECISION_DRIFT-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-UW_DECISION_DRIFT-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-UW_DECISION_DRIFT-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-UW_DECISION_DRIFT-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-UW_DECISION_DRIFT-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
