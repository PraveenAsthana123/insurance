# HOLY Beverage — Customer Support — Monitoring AI
...
> Persona: **Support Manager**. Focus: **intent-classifier accuracy + RAG citation faithfulness**.
...
Tab: `/holy/customer-support/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/customer-support` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/customer-support/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/customer-support/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #customer-support-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #customer-support-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/customer-support` returns 200 + job list
