# HOLY Beverage — Supply Chain — Agentic Stack
...
> actions against and the required scope for each. The Supply Chain manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Supply Chain)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Supply Chain agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Supply Chain agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Supply Chain and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/supply-chain.yaml](../../../../config/scopes/supply-chain.yaml)
...
| Action irreversible (deletion, signing) | Manager (Supply Chain) approval |
| Confidence < 0.6 | Manager (Supply Chain) review queue |
...
will enforce every grant in `config/scopes/supply-chain.yaml` has a matching audit
