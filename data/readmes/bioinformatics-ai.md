# Bioinformatics AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `bioinformatics-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for bioinformatics ai tasks in
...
**FR-1**: System MUST accept input matching `bioinformatics-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/bioinformatics-ai/predict
        │
        ▼
...
- `predictions_bioinformatics-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/bioinformatics-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/bioinformatics-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/bioinformatics-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/bioinformatics-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `bioinformatics-ai_embeddings`
...
**Fairness metrics** (from data/fairness/bioinformatics-ai.json):
