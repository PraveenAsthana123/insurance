...
> Department 4 · Underwriting · slug `risk-scoring` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/risk-scoring/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/4/<domain>/risk-scoring?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/4/<domain>/risk-scoring?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/risk-scoring.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/underwriting/risk-scoring/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-RISK_SCORING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-RISK_SCORING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-RISK_SCORING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-RISK_SCORING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-RISK_SCORING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-RISK_SCORING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-RISK_SCORING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/underwriting/risk-scoring/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-RISK_SCORING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/underwriting/risk-scoring/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/underwriting/risk-scoring/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/underwriting/risk-scoring/stats/`
