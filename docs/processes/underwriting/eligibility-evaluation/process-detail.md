...
> Department 4 · Underwriting · slug `eligibility-evaluation` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/eligibility-evaluation/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/4/<domain>/eligibility-evaluation?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/4/<domain>/eligibility-evaluation?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/eligibility-evaluation.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/underwriting/eligibility-evaluation/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-ELIGIBILITY_EVALUATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-ELIGIBILITY_EVALUATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-ELIGIBILITY_EVALUATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-ELIGIBILITY_EVALUATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-ELIGIBILITY_EVALUATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-ELIGIBILITY_EVALUATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-ELIGIBILITY_EVALUATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/underwriting/eligibility-evaluation/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-ELIGIBILITY_EVALUATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/underwriting/eligibility-evaluation/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/underwriting/eligibility-evaluation/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/underwriting/eligibility-evaluation/stats/`
