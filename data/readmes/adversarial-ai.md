# Adversarial AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `adversarial-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for adversarial ai tasks in
...
**FR-1**: System MUST accept input matching `adversarial-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/adversarial-ai/predict
        │
        ▼
...
- `predictions_adversarial-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/adversarial-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/adversarial-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/adversarial-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/adversarial-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `adversarial-ai_embeddings`
...
**Fairness metrics** (from data/fairness/adversarial-ai.json):
