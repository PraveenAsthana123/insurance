# Pipeline Checklist · investment-allocation-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `investment_allocation_rec_raw` |
| Cleaned | Postgres | `investment_allocation_rec_clean` |
| Features | Feast / Postgres | `investment_allocation_rec_features` |
| Predictions | Postgres | `investment_allocation_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/investment-allocation-rec/` |
...
1. Input → `investment_allocation_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `investment_allocation_rec_predictions`
4. Log → ELK / Loki
