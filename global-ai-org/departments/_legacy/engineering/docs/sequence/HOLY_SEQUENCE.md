# HOLY Beverage — Engineering — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/engineering
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **AI Code Review on PR**

_PR opens → diff → review LLM → annotate + score → block-or-pass_
...
When a user types a goal in `/holy/engineering/agentic`:
