# Marketing AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `marketing-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for marketing ai tasks in
...
**FR-1**: System MUST accept input matching `marketing-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/marketing-ai/predict
        │
        ▼
...
- `predictions_marketing-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/marketing-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/marketing-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/marketing-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/marketing-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `marketing-ai_embeddings`
...
**Fairness metrics** (from data/fairness/marketing-ai.json):
