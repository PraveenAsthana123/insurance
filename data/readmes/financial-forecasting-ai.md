# Financial Forecasting AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `financial-forecasting-ai` · **Kind**: `spec_only`
**Accuracy**: 0.9567 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for financial forecasting ai tasks in
...
**FR-1**: System MUST accept input matching `financial-forecasting-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/financial-forecasting-ai/predict
        │
        ▼
...
- `predictions_financial-forecasting-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/financial-forecasting-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/financial-forecasting-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/financial-forecasting-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/financial-forecasting-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `financial-forecasting-ai_embeddings`
...
**Fairness metrics** (from data/fairness/financial-forecasting-ai.json):
