# HOLY Beverage — Operations — Agentic Stack
...
> actions against and the required scope for each. The Operations manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Operations)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Operations agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Operations agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Operations and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/operations.yaml](../../../../config/scopes/operations.yaml)
...
| Action irreversible (deletion, signing) | Manager (Operations) approval |
| Confidence < 0.6 | Manager (Operations) review queue |
...
will enforce every grant in `config/scopes/operations.yaml` has a matching audit
