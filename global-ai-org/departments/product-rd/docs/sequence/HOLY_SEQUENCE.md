# HOLY Beverage — Product Rd — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/product-rd
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Concept Scoring**

_Concept brief → NLP feature extract → success-likelihood model → rank_
...
When a user types a goal in `/holy/product-rd/agentic`:
