# Pipeline Checklist · gl-journal-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `gl_journal_anomaly_raw` |
| Cleaned | Postgres | `gl_journal_anomaly_clean` |
| Features | Feast / Postgres | `gl_journal_anomaly_features` |
| Predictions | Postgres | `gl_journal_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/gl-journal-anomaly/` |
...
1. Input → `gl_journal_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `gl_journal_anomaly_predictions`
4. Log → ELK / Loki
