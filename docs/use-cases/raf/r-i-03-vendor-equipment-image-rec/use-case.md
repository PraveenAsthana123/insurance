# Recommender-Image-3 · vendor-equipment-image-rec
...
Dept 17 Procurement · CLIP + metadata
...
```mermaid
graph LR
    Input["Image Input"] --> Preprocess[G1-G5 preprocessing]
    Preprocess --> Feature[G4 feature eng]
    Feature --> Model[Model · R-specific]
...
|---|---|---|---|
| `INSUR-VENDOR_EQUIPMENT_IMAGE_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VENDOR_EQUIPMENT_IMAGE_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VENDOR_EQUIPMENT_IMAGE_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VENDOR_EQUIPMENT_IMAGE_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-VENDOR_EQUIPMENT_IMAGE_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-VENDOR_EQUIPMENT_IMAGE_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
