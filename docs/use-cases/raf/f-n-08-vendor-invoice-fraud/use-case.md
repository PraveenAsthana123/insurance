# Fraud-Numerical-8 · vendor-invoice-fraud
...
Dept 17 Procurement · XGB + dup-scan + Benford
...
|---|---|---|---|
| `INSUR-VENDOR_INVOICE_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VENDOR_INVOICE_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VENDOR_INVOICE_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VENDOR_INVOICE_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-VENDOR_INVOICE_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-VENDOR_INVOICE_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
