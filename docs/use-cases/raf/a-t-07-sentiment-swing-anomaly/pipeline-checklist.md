# Pipeline Checklist · sentiment-swing-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `sentiment_swing_anomaly_raw` |
| Cleaned | Postgres | `sentiment_swing_anomaly_clean` |
| Features | Feast / Postgres | `sentiment_swing_anomaly_features` |
| Predictions | Postgres | `sentiment_swing_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/sentiment-swing-anomaly/` |
...
1. Input → `sentiment_swing_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `sentiment_swing_anomaly_predictions`
4. Log → ELK / Loki
