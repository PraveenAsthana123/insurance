# Optimization AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `optimization-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for optimization ai tasks in
...
**FR-1**: System MUST accept input matching `optimization-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/optimization-ai/predict
        │
        ▼
...
- `predictions_optimization-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/optimization-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/optimization-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/optimization-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/optimization-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `optimization-ai_embeddings`
...
**Fairness metrics** (from data/fairness/optimization-ai.json):
