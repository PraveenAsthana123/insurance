# OCR AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ocr-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for ocr ai tasks in
...
**FR-1**: System MUST accept input matching `ocr-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ocr-ai/predict
        │
        ▼
...
- `predictions_ocr-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ocr-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/ocr-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/ocr-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ocr-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/ocr-ai.json):
