# HOLY Beverage — Product Rd Network Flow
...
```
Product Rd API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Product Rd API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          product-rd_user_events       analytics-svc
Backend API       ──►          product-rd_audit              audit-store
ML Worker         ──►          product-rd_predictions        prediction-store
Drift detector    ──►          product-rd_alerts             on-call PagerDuty
...
|---|---|---|---|
| product-rd-api | postgresql | 5432 | TCP |
| product-rd-api | redis | 6379 | TCP |
| product-rd-api | ollama | 11434 | TCP |
| product-rd-api | otel-collector | 4317 | gRPC |
| product-rd-api | (other dept APIs) | 8000 | HTTP via service mesh |
