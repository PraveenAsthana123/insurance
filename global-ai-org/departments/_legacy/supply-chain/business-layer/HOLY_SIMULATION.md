# HOLY Beverage — Supply Chain — Simulation Tab
...
under `REFERENCE_PROCESSES[("supply-chain", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/supply-chain/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/supply-chain/{process}/runs | List past runs |
| GET /api/v1/holy/sim/supply-chain/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/supply-chain/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for supply-chain in `REFERENCE_PROCESSES` dict
