# HOLY Beverage — Procurement — Monitoring AI
...
> Persona: **Procurement Manager**. Focus: **spend-classification + vendor-risk scoring drift**.
...
Tab: `/holy/procurement/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/procurement` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/procurement/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/procurement/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #procurement-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #procurement-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/procurement` returns 200 + job list
