...
> Department 7 · Claims · slug `claim-registration` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/claim-registration/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/7/<domain>/claim-registration?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/7/<domain>/claim-registration?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/claim-registration.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/claims/claim-registration/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-CLAIM_REGISTRATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-CLAIM_REGISTRATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-CLAIM_REGISTRATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-CLAIM_REGISTRATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-CLAIM_REGISTRATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-CLAIM_REGISTRATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-CLAIM_REGISTRATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/claims/claim-registration/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-CLAIM_REGISTRATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/claims/claim-registration/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/claims/claim-registration/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/claims/claim-registration/stats/`
