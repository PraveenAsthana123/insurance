# HOLY Beverage — Marketing — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/marketing
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Audience Segmentation Refresh**

_Behavior data → cluster → segment update → channel router_
...
When a user types a goal in `/holy/marketing/agentic`:
