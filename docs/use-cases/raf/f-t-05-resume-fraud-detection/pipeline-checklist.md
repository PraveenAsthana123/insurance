# Pipeline Checklist · resume-fraud-detection
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `resume_fraud_detection_raw` |
| Cleaned | Postgres | `resume_fraud_detection_clean` |
| Features | Feast / Postgres | `resume_fraud_detection_features` |
| Predictions | Postgres | `resume_fraud_detection_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/resume-fraud-detection/` |
...
1. Input → `resume_fraud_detection_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `resume_fraud_detection_predictions`
4. Log → ELK / Loki
