# HOLY Beverage — Executive Leadership — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **KPI rollups + variance flags + board pack generations**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/executive-leadership` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/executive-leadership?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/executive-leadership?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/executive-leadership?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/executive-leadership/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/executive-leadership` returns 200 + chronological feed
