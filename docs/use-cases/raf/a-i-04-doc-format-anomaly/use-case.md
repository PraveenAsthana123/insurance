# Anomaly-Image-4 · doc-format-anomaly
...
Dept 5 Policy Admin · LayoutLM + format embed
...
|---|---|---|---|
| `INSUR-DOC_FORMAT_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DOC_FORMAT_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DOC_FORMAT_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DOC_FORMAT_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-DOC_FORMAT_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-DOC_FORMAT_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
