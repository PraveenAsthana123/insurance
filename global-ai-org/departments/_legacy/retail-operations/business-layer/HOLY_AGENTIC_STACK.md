# HOLY Beverage — Retail Operations — Agentic Stack
...
> actions against and the required scope for each. The Retail Operations manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Retail Operations)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Retail Operations agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Retail Operations agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Retail Operations and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/retail-operations.yaml](../../../../config/scopes/retail-operations.yaml)
...
| Action irreversible (deletion, signing) | Manager (Retail Operations) approval |
| Confidence < 0.6 | Manager (Retail Operations) review queue |
...
will enforce every grant in `config/scopes/retail-operations.yaml` has a matching audit
