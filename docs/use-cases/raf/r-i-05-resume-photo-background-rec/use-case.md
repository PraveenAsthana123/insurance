# Recommender-Image-5 · resume-photo-background-rec
...
Dept 16 HR · face-attribute-blind embed
...
|---|---|---|---|
| `INSUR-RESUME_PHOTO_BACKGROUND_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RESUME_PHOTO_BACKGROUND_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RESUME_PHOTO_BACKGROUND_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RESUME_PHOTO_BACKGROUND_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-RESUME_PHOTO_BACKGROUND_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-RESUME_PHOTO_BACKGROUND_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
