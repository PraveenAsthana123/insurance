# Defect Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `defect-detection-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for defect detection ai tasks in
...
**FR-1**: System MUST accept input matching `defect-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/defect-detection-ai/predict
        │
        ▼
...
- `predictions_defect-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/defect-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/defect-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/defect-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/defect-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `defect-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/defect-detection-ai.json):
