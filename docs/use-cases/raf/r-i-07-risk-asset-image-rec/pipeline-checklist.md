# Pipeline Checklist · risk-asset-image-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `risk_asset_image_rec_raw` |
| Cleaned | Postgres | `risk_asset_image_rec_clean` |
| Features | Feast / Postgres | `risk_asset_image_rec_features` |
| Predictions | Postgres | `risk_asset_image_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/risk-asset-image-rec/` |
...
1. Input → `risk_asset_image_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `risk_asset_image_rec_predictions`
4. Log → ELK / Loki
