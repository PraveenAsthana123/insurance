# Energy AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `energy-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for energy ai tasks in
...
**FR-1**: System MUST accept input matching `energy-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/energy-ai/predict
        │
        ▼
...
- `predictions_energy-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/energy-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/energy-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/energy-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/energy-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `energy-ai_embeddings`
...
**Fairness metrics** (from data/fairness/energy-ai.json):
