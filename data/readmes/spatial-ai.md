# Spatial AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `spatial-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for spatial ai tasks in
...
**FR-1**: System MUST accept input matching `spatial-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/spatial-ai/predict
        │
        ▼
...
- `predictions_spatial-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/spatial-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/spatial-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/spatial-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/spatial-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `spatial-ai_embeddings`
...
**Fairness metrics** (from data/fairness/spatial-ai.json):
