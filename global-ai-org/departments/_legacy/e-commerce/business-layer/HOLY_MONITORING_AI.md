# HOLY Beverage — E Commerce — Monitoring AI
...
> Persona: **E-Comm Manager**. Focus: **recommender drift + cart-abandonment ML accuracy**.
...
Tab: `/holy/e-commerce/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/e-commerce` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/e-commerce/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/e-commerce/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #e-commerce-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #e-commerce-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/e-commerce` returns 200 + job list
