# Recommender-Text-6 · training-content-rec
...
Dept 16 HR · item collab + content embed
...
|---|---|---|---|
| `INSUR-TRAINING_CONTENT_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-TRAINING_CONTENT_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-TRAINING_CONTENT_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-TRAINING_CONTENT_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-TRAINING_CONTENT_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-TRAINING_CONTENT_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
