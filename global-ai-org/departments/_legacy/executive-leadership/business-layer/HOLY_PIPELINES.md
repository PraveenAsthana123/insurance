# HOLY Beverage — Executive Leadership — Automated Pipelines
...
|---|---|---|---|
| 1 | KPI Rollup | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **KPI Rollup + Variance Detection + Board Pack Generation**._
...
**Audit prefix:** `pipeline.executive_leadership.kpi_rollup`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream executive-leadership systems | `outcome=data_received` |
...
| 5 Report | Executive Leadership manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/executive-leadership` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/executive-leadership/<process_id>` | Full 5-phase spec for one process |
