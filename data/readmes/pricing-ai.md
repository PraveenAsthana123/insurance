# Pricing AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `pricing-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for pricing ai tasks in
...
**FR-1**: System MUST accept input matching `pricing-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/pricing-ai/predict
        │
        ▼
...
- `predictions_pricing-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/pricing-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/pricing-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/pricing-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/pricing-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `pricing-ai_embeddings`
...
**Fairness metrics** (from data/fairness/pricing-ai.json):
