# Embodied AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `embodied-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for embodied ai tasks in
...
**FR-1**: System MUST accept input matching `embodied-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/embodied-ai/predict
        │
        ▼
...
- `predictions_embodied-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/embodied-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/embodied-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/embodied-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/embodied-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `embodied-ai_embeddings`
...
**Fairness metrics** (from data/fairness/embodied-ai.json):
