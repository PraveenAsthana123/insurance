...
any 3-5 to assemble a Finance board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Finance
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is Admin (System)'s view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: Dept Manager in Finance
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is Dept Manager's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: Team Member in Finance
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is Team Member's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Finance
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is QA / Tester's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: SecOps Engineer in Finance
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is SecOps Engineer's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Finance
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is DevOps's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: AI Reviewer in Finance
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is AI Reviewer's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: DT Lead in Finance
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is DT Lead's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: System Architect in Finance
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is System Architect's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: Test Architect in Finance
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is Test Architect's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: DB Architect in Finance
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is DB Architect's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: API Architect in Finance
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is API Architect's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: Data Owner in Finance
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is Data Owner's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: AI Strategy Lead in Finance
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is AI Strategy Lead's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
- **Persona**: InfoSec / CISO Office in Finance
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Finance dashboards
- **KPI moved**: Forecast accuracy + fraud loss
...
- **Talking points**: "This is InfoSec / CISO Office's view of Finance. In 90 seconds we'll move Forecast accuracy + fraud loss by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/finance` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/finance` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/finance/<role>` | Single role demo detail |
