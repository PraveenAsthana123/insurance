# HOLY Beverage — Sales — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Lead scores + churn predictions + next-best-action decisions**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/sales` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/sales?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/sales?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/sales?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/sales/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/sales` returns 200 + chronological feed
