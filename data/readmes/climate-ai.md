# Climate AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `climate-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for climate ai tasks in
...
**FR-1**: System MUST accept input matching `climate-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/climate-ai/predict
        │
        ▼
...
- `predictions_climate-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/climate-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/climate-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/climate-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/climate-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `climate-ai_embeddings`
...
**Fairness metrics** (from data/fairness/climate-ai.json):
