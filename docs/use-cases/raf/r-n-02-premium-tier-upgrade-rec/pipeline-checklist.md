# Pipeline Checklist · premium-tier-upgrade-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `premium_tier_upgrade_rec_raw` |
| Cleaned | Postgres | `premium_tier_upgrade_rec_clean` |
| Features | Feast / Postgres | `premium_tier_upgrade_rec_features` |
| Predictions | Postgres | `premium_tier_upgrade_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/premium-tier-upgrade-rec/` |
...
1. Input → `premium_tier_upgrade_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `premium_tier_upgrade_rec_predictions`
4. Log → ELK / Loki
