...
> Department 21 · Sales, Distribution, Broker, Agency & Partner Management · slug `producer-licensing` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/producer-licensing/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/21/<domain>/producer-licensing?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/21/<domain>/producer-licensing?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/producer-licensing.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/sales-distribution-broker-agency-partner-management/producer-licensing/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRODUCER_LICENSING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRODUCER_LICENSING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRODUCER_LICENSING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRODUCER_LICENSING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRODUCER_LICENSING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRODUCER_LICENSING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRODUCER_LICENSING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/sales-distribution-broker-agency-partner-management/producer-licensing/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRODUCER_LICENSING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/sales-distribution-broker-agency-partner-management/producer-licensing/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/sales-distribution-broker-agency-partner-management/producer-licensing/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/sales-distribution-broker-agency-partner-management/producer-licensing/stats/`
