# Simulation UI — Underwriting

Per global §64.34 + operator 2026-06-01.

This dept's `/insur/underwriting/simulation` tab specification. Manual-mode + Automatic-mode
side-by-side with 5 layers visible per run.

## Tab path
`/insur/underwriting/simulation?process=<L2-process-name>`
...
| Endpoint | Purpose |
|---|---|
| `POST /api/v1/insur/sim/underwriting/{process}/run` | Trigger sim; body: `{mode, inputs, seed}` |
| `GET /api/v1/insur/sim/underwriting/{process}/runs/{sim_id}/events` | SSE stream for live waterfall |
| `GET /api/v1/insur/sim/underwriting/{process}/runs/{sim_id}/manifest` | Per-layer summary + comparison report |
| `GET /api/v1/insur/sim/underwriting/{process}/runs/{sim_id}/replay` | Frame-by-frame replay of past run |
...
| Audit-trail capture | `data/eval/sim/underwriting/<process>/<sim_id>/events.jsonl` |
