# HOLY Beverage — Executive Leadership — Monitoring AI
...
> Persona: **CEO Office**. Focus: **exec-KPI rollup freshness + decision-audit completeness**.
...
Tab: `/holy/executive-leadership/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/executive-leadership` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/executive-leadership/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/executive-leadership/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #executive-leadership-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #executive-leadership-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/executive-leadership` returns 200 + job list
