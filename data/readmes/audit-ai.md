# Audit AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `audit-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for audit ai tasks in
...
**FR-1**: System MUST accept input matching `audit-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/audit-ai/predict
        │
        ▼
...
- `predictions_audit-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/audit-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/audit-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/audit-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/audit-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `audit-ai_embeddings`
...
**Fairness metrics** (from data/fairness/audit-ai.json):
