...
WHAT broke?      Check `/data/metrics/reactive-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/reactive-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/reactive-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/reactive-ai/model.joblib.backup models/reactive-ai/model.joblib`
