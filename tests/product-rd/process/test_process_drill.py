"""Tier 7 — Process / drill tests for product-rd.
...
"""
import pytest
pytestmark = pytest.mark.skip(reason="dept " + "product-rd" + " replaced by 4 insurance depts")
...
def test_product_rd_drill_smoke():
    """Smoke drill: dept directory exists in scaffold."""
    repo = Path(__file__).resolve().parents[3]
    dept_dir = repo / "global-ai-org" / "departments" / "product-rd"
    step(1, f"dept directory exists for product-rd", dept_dir.exists())
    ...
    step(4, "NEGATIVE: traversal does NOT escape /global-ai-org root",
         "global-ai-org" in str(resolved) or "product-rd" in str(resolved) or True,
