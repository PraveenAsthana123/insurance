...
graph LR
    ...
    features --> model[NLP, OCR, RAG, Document AI, Compliance AI]
...
- Decision lineage: every API call writes a row to `legal_audit` (see LLD §2)

## L6 — Observability

- Logs: structured JSON with `correlation_id`, `tenant_id`, `user_id`, `model_version`, `prompt_version`
- Metrics: `legal_request_count`, `legal_latency_p95`, `legal_model_confidence`, `legal_hallucination_rate`
