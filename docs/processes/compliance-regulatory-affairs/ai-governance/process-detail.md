...
> Department 12 · Compliance & Regulatory Affairs · slug `ai-governance` · AI items: 7
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/ai-governance/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/12/<domain>/ai-governance?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/12/<domain>/ai-governance?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/ai-governance.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/compliance-regulatory-affairs/ai-governance/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-AI_GOVERNANCE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-AI_GOVERNANCE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-AI_GOVERNANCE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-AI_GOVERNANCE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-AI_GOVERNANCE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-AI_GOVERNANCE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-AI_GOVERNANCE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/compliance-regulatory-affairs/ai-governance/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-AI_GOVERNANCE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/compliance-regulatory-affairs/ai-governance/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/compliance-regulatory-affairs/ai-governance/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/compliance-regulatory-affairs/ai-governance/stats/`
