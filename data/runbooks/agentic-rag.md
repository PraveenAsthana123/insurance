...
WHAT broke?      Check `/data/metrics/agentic-rag.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/agentic-rag.json`
WHO touched?     `git log --since=<date> data/ai_types/agentic-rag.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/agentic-rag/model.joblib.backup models/agentic-rag/model.joblib`
