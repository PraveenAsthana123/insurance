# Pipeline Checklist · drone-imagery-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `drone_imagery_fraud_raw` |
| Cleaned | Postgres | `drone_imagery_fraud_clean` |
| Features | Feast / Postgres | `drone_imagery_fraud_features` |
| Predictions | Postgres | `drone_imagery_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/drone-imagery-fraud/` |
...
1. Input → `drone_imagery_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `drone_imagery_fraud_predictions`
4. Log → ELK / Loki
