# AI Alignment · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-alignment` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for ai alignment tasks in
...
**FR-1**: System MUST accept input matching `ai-alignment` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-alignment/predict
        │
        ▼
...
- `predictions_ai-alignment`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-alignment/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-alignment/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-alignment/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-alignment/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-alignment_embeddings`
...
**Fairness metrics** (from data/fairness/ai-alignment.json):
