# What-if AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `what-if-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for what-if ai tasks in
...
**FR-1**: System MUST accept input matching `what-if-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/what-if-ai/predict
        │
        ▼
...
- `predictions_what-if-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/what-if-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/what-if-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/what-if-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/what-if-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `what-if-ai_embeddings`
...
**Fairness metrics** (from data/fairness/what-if-ai.json):
