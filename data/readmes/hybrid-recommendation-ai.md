# Hybrid Recommendation AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `hybrid-recommendation-ai` · **Kind**: `spec_only`
**Accuracy**: 0.9417 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for hybrid recommendation ai tasks in
...
**FR-1**: System MUST accept input matching `hybrid-recommendation-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/hybrid-recommendation-ai/predict
        │
        ▼
...
- `predictions_hybrid-recommendation-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/hybrid-recommendation-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/hybrid-recommendation-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/hybrid-recommendation-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/hybrid-recommendation-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `hybrid-recommendation-ai_embeddings`
...
**Fairness metrics** (from data/fairness/hybrid-recommendation-ai.json):
