# HOLY Beverage — Procurement — Simulation Tab
...
under `REFERENCE_PROCESSES[("procurement", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/procurement/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/procurement/{process}/runs | List past runs |
| GET /api/v1/holy/sim/procurement/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/procurement/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for procurement in `REFERENCE_PROCESSES` dict
