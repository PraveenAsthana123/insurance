# HOLY Beverage — Executive Leadership — Admin Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **admin** in **executive-leadership**.

## Persona

The **admin** at Executive Leadership. They use this dashboard
...
System health + user mgmt + feature-flag state + RBAC events
...
- RBAC-change time-series
- Failed-login heatmap (hour × day)
- Feature-flag adoption funnel

Library dispatch per chart (recharts / plotly / echarts / d3 / vega-lite)
returned by backend `GET /api/v1/holy/dashboards/executive-leadership/admin/charts/<chart_id>`.
...
Inline actions the **admin** can take from this dashboard:
...
```
GET /api/v1/holy/dashboards/executive-leadership/admin
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/executive-leadership/admin/charts/<chart_id>
  → {library, type, data, baseline, threshold, drill_url, refreshed_at}
```

## Related Artifacts

- `../../reports-by-role/admin/HOLY_REPORTS.md` — paired standard reports
...
- Frontend route: `/holy/executive-leadership/dashboard?role=admin`
