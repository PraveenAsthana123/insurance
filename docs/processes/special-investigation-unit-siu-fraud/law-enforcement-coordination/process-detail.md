...
> Department 8 · Special Investigation Unit (SIU) / Fraud · slug `law-enforcement-coordination` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/law-enforcement-coordination/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/8/<domain>/law-enforcement-coordination?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/8/<domain>/law-enforcement-coordination?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/special-investigation-unit-siu-fraud/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/law-enforcement-coordination.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/special-investigation-unit-siu-fraud/law-enforcement-coordination/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-LAW_ENFORCEMENT_COORDINATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-LAW_ENFORCEMENT_COORDINATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-LAW_ENFORCEMENT_COORDINATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-LAW_ENFORCEMENT_COORDINATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-LAW_ENFORCEMENT_COORDINATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-LAW_ENFORCEMENT_COORDINATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-LAW_ENFORCEMENT_COORDINATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/special-investigation-unit-siu-fraud/law-enforcement-coordination/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-LAW_ENFORCEMENT_COORDINATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/special-investigation-unit-siu-fraud/law-enforcement-coordination/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/special-investigation-unit-siu-fraud/law-enforcement-coordination/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/special-investigation-unit-siu-fraud/law-enforcement-coordination/stats/`
