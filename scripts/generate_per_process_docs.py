#!/usr/bin/env python3
"""generate_per_process_docs.py — drop a per-process doc stub for every process in the blueprint.

Per operator instructions (2026-06-08 · accumulated):
Every process MUST have all 19 mandatory sections in `process-detail.md`:
  1. Problem explanation
  2. AS-IS flow (manual today)
  3. TO-BE flow (automated target)
  4. Impact analysis (productivity + cost + revenue · detailed)
  5. AI capabilities (from blueprint)
  6. User demo flow (per process · operator-runnable)
  7. Simulation tab (what-if scenarios)
  8. Manual process workbench (data/model/accuracy/HP picker)
  9. Automatic process pipeline (Celery/LangGraph DAG)
 10. Phase tracking (threshold · score · update · QC · status)
 11. Final testing + summary report generation
 12. Cron jobs + DB update list
 13. Database layers (graph + vector + historical)
 14. RAG evaluation + DeepEval
 15. Vector DB ingestion cron
 16. Per-operation universal audit (input/process/output/log/trace/prompt)
 17. Crash recovery (every state change saved before continue)
 18. Comprehensive testing matrix (UI/API/data/model/accuracy/output/chunking/agent/orchestration/mCP/Ragas/DeepEval/PII)
 19. Statistical + DBA + management analysis (ResAI · ExpAI · etc.)

Outputs: docs/processes/<dept_slug>/<process_slug>/process-detail.md (322 files).
...
BLUEPRINT = REPO / "data" / "insurance" / "blueprint.json"
PROCESS_DOCS_ROOT = REPO / "docs" / "processes"
...
# Template uses ${TOKEN} markers (no f-string · no Python format string conflict).
# All literal {} braces in code/tables stay as-is.
TEMPLATE = r"""# ${PROC_NAME}

> Department ${DEPT_ID} · ${DEPT_NAME} · slug `${PROC_SLUG}` · AI items: ${AI_COUNT}
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/${PROC_SLUG}/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/${DEPT_ID}/<domain>/${PROC_SLUG}?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/${DEPT_ID}/<domain>/${PROC_SLUG}?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/${DEPT_SLUG}/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/${PROC_SLUG}.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/${DEPT_SLUG}/${PROC_SLUG}/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-${PROC_UPPER_SLUG}-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-${PROC_UPPER_SLUG}-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-${PROC_UPPER_SLUG}-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-${PROC_UPPER_SLUG}-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-${PROC_UPPER_SLUG}-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-${PROC_UPPER_SLUG}-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-${PROC_UPPER_SLUG}-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/${DEPT_SLUG}/${PROC_SLUG}/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-${PROC_UPPER_SLUG}-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/${DEPT_SLUG}/${PROC_SLUG}/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/${DEPT_SLUG}/${PROC_SLUG}/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/${DEPT_SLUG}/${PROC_SLUG}/stats/`
...
def render(dept: dict, proc: dict) -> str:
    dept_name = dept.get("name", "Unknown Dept")
    dept_id = str(dept.get("id", "?"))
    dept_slug = slugify(dept_name)
    proc_name = proc.get("name", "Unknown Process")
    proc_slug = slugify(proc_name)
    proc_upper_slug = proc_slug.upper().replace("-", "_")
    ai_items = proc.get("ai") or []
    ai_count = len(ai_items)
    ai_list = "\n".join(
        f"- {a.get('name') if isinstance(a, dict) else a}" for a in ai_items
    ) or "- (none defined yet)"
    ...
    for key, val in [
        ...
        ("DEPT_SLUG", dept_slug),
        ("PROC_SLUG", proc_slug),
        ("PROC_UPPER_SLUG", proc_upper_slug),
...
def main() -> int:
    ...
    p.add_argument("--limit", type=int, default=0, help="Limit to N (debug)")
    args = p.parse_args()
    ...
    depts = bp.get("department_catalog", [])
    if not depts:
        print("ERROR: no department_catalog in blueprint", file=sys.stderr)
        return 3

    PROCESS_DOCS_ROOT.mkdir(parents=True, exist_ok=True)
    written = skipped = 0
    count = 0
    for dept in depts:
        dept_slug = slugify(dept.get("name", f"dept-{dept.get('id', '?')}"))
        for proc in dept.get("processes", []) or []:
            count += 1
            if args.limit and count > args.limit:
                break
            proc_slug = slugify(proc.get("name", "unknown"))
            target = PROCESS_DOCS_ROOT / dept_slug / proc_slug / "process-detail.md"
            if target.exists() and not args.force:
                skipped += 1
                continue
            content = render(dept, proc)
            if args.dry_run:
    ...
    print(f"\nGenerated: {written} written · {skipped} skipped (exists)")
    print(f"Root: {PROCESS_DOCS_ROOT.relative_to(REPO)}")
    return 0
