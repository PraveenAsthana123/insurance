// Tier 8 — Smoke / E2E test for operations.
//
// Recommended: playwright (per global §64.42).
//
// Run with:  npx playwright test tests/operations/smoke/
//
// Per global §64.30 tier 8: top 20 critical endpoints + UI routes per deploy.

const { test, expect } = require('@playwright/test');

test.describe('operations smoke tests', () => {
  test('INSUR nav page for operations loads', async ({ page }) => {
    await page.goto('http://localhost:3000/insur/operations');
    await expect(page).toHaveURL(/\/insur\/operations/);
