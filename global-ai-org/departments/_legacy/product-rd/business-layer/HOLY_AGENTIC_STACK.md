# HOLY Beverage — Product R&D — Agentic Stack
...
> actions against and the required scope for each. The Product R&D manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Product R&D)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Product R&D agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Product R&D agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Product R&D and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/product-rd.yaml](../../../../config/scopes/product-rd.yaml)
...
| Action irreversible (deletion, signing) | Manager (Product R&D) approval |
| Confidence < 0.6 | Manager (Product R&D) review queue |
...
will enforce every grant in `config/scopes/product-rd.yaml` has a matching audit
