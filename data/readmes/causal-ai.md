# Causal AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `causal-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for causal ai tasks in
...
**FR-1**: System MUST accept input matching `causal-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/causal-ai/predict
        │
        ▼
...
- `predictions_causal-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/causal-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/causal-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/causal-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/causal-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `causal-ai_embeddings`
...
**Fairness metrics** (from data/fairness/causal-ai.json):
