# Explainable AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `explainable-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for explainable ai tasks in
...
**FR-1**: System MUST accept input matching `explainable-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/explainable-ai/predict
        │
        ▼
...
- `predictions_explainable-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/explainable-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/explainable-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/explainable-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/explainable-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `explainable-ai_embeddings`
...
**Fairness metrics** (from data/fairness/explainable-ai.json):
