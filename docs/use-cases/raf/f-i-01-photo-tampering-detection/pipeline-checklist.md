# Pipeline Checklist · photo-tampering-detection
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `photo_tampering_detection_raw` |
| Cleaned | Postgres | `photo_tampering_detection_clean` |
| Features | Feast / Postgres | `photo_tampering_detection_features` |
| Predictions | Postgres | `photo_tampering_detection_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/photo-tampering-detection/` |
...
1. Input → `photo_tampering_detection_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `photo_tampering_detection_predictions`
4. Log → ELK / Loki
