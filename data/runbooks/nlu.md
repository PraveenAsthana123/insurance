...
WHAT broke?      Check `/data/metrics/nlu.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/nlu.json`
WHO touched?     `git log --since=<date> data/ai_types/nlu.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/nlu/model.joblib.backup models/nlu/model.joblib`
