# Pipeline Checklist · drone-imagery-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `drone_imagery_anomaly_raw` |
| Cleaned | Postgres | `drone_imagery_anomaly_clean` |
| Features | Feast / Postgres | `drone_imagery_anomaly_features` |
| Predictions | Postgres | `drone_imagery_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/drone-imagery-anomaly/` |
...
1. Input → `drone_imagery_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `drone_imagery_anomaly_predictions`
4. Log → ELK / Loki
