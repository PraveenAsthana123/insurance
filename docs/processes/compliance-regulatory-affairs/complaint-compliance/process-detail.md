...
> Department 12 · Compliance & Regulatory Affairs · slug `complaint-compliance` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/complaint-compliance/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/12/<domain>/complaint-compliance?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/12/<domain>/complaint-compliance?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/complaint-compliance.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/compliance-regulatory-affairs/complaint-compliance/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-COMPLAINT_COMPLIANCE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-COMPLAINT_COMPLIANCE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-COMPLAINT_COMPLIANCE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-COMPLAINT_COMPLIANCE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-COMPLAINT_COMPLIANCE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-COMPLAINT_COMPLIANCE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-COMPLAINT_COMPLIANCE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/compliance-regulatory-affairs/complaint-compliance/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-COMPLAINT_COMPLIANCE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/compliance-regulatory-affairs/complaint-compliance/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/compliance-regulatory-affairs/complaint-compliance/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/compliance-regulatory-affairs/complaint-compliance/stats/`
