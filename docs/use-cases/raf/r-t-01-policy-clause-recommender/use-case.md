# Recommender-Text-1 · policy-clause-recommender
...
|---|---|---|---|
| `INSUR-POLICY_CLAUSE_RECOMMENDER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-POLICY_CLAUSE_RECOMMENDER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-POLICY_CLAUSE_RECOMMENDER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-POLICY_CLAUSE_RECOMMENDER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-POLICY_CLAUSE_RECOMMENDER-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-POLICY_CLAUSE_RECOMMENDER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
