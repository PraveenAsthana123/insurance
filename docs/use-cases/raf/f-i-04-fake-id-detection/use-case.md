# Fraud-Image-4 · fake-id-detection
...
Dept 21 Sales · OCR + liveness + KYC RAG
...
|---|---|---|---|
| `INSUR-FAKE_ID_DETECTION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-FAKE_ID_DETECTION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-FAKE_ID_DETECTION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-FAKE_ID_DETECTION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-FAKE_ID_DETECTION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-FAKE_ID_DETECTION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
