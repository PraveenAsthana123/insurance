# Schedule Optimization AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `schedule-optimization-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for schedule optimization ai tasks in
...
**FR-1**: System MUST accept input matching `schedule-optimization-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/schedule-optimization-ai/predict
        │
        ▼
...
- `predictions_schedule-optimization-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/schedule-optimization-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/schedule-optimization-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/schedule-optimization-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/schedule-optimization-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `schedule-optimization-ai_embeddings`
...
**Fairness metrics** (from data/fairness/schedule-optimization-ai.json):
