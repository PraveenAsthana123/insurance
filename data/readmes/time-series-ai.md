# Time-Series AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `time-series-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for time-series ai tasks in
...
**FR-1**: System MUST accept input matching `time-series-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/time-series-ai/predict
        │
        ▼
...
- `predictions_time-series-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/time-series-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/time-series-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/time-series-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/time-series-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `time-series-ai_embeddings`
...
**Fairness metrics** (from data/fairness/time-series-ai.json):
