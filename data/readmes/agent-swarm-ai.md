# Agent Swarm AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `agent-swarm-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for agent swarm ai tasks in
...
**FR-1**: System MUST accept input matching `agent-swarm-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/agent-swarm-ai/predict
        │
        ▼
...
- `predictions_agent-swarm-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/agent-swarm-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/agent-swarm-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/agent-swarm-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/agent-swarm-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `agent-swarm-ai_embeddings`
...
**Fairness metrics** (from data/fairness/agent-swarm-ai.json):
