# Anomaly-Numerical-7 · vendor-spend-anomaly
...
Dept 17 Procurement · Holt-Winters + IsoForest
...
|---|---|---|---|
| `INSUR-VENDOR_SPEND_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-VENDOR_SPEND_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-VENDOR_SPEND_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-VENDOR_SPEND_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-VENDOR_SPEND_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-VENDOR_SPEND_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
