# Fraud-Image-2 · fake-document-detection
...
Dept 5 Policy Admin · LayoutLM + tamper-CNN
...
|---|---|---|---|
| `INSUR-FAKE_DOCUMENT_DETECTION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-FAKE_DOCUMENT_DETECTION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-FAKE_DOCUMENT_DETECTION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-FAKE_DOCUMENT_DETECTION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-FAKE_DOCUMENT_DETECTION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-FAKE_DOCUMENT_DETECTION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
