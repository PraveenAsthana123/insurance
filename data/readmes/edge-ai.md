# Edge AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `edge-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for edge ai tasks in
...
**FR-1**: System MUST accept input matching `edge-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/edge-ai/predict
        │
        ▼
...
- `predictions_edge-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/edge-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/edge-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/edge-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/edge-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/edge-ai.json):
