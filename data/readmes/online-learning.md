# Online Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `online-learning` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for online learning tasks in
...
**FR-1**: System MUST accept input matching `online-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/online-learning/predict
        │
        ▼
...
- `predictions_online-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/online-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/online-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/online-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/online-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `online-learning_embeddings`
...
**Fairness metrics** (from data/fairness/online-learning.json):
