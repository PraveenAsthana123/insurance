# HOLY Beverage — It Operations — Monitoring AI
...
> Persona: **IT-Ops Manager**. Focus: **incident-classifier + log-anomaly detector drift**.
...
Tab: `/holy/it-operations/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/it-operations` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/it-operations/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/it-operations/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #it-operations-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #it-operations-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/it-operations` returns 200 + job list
