# AgentOps · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `agentops` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for agentops tasks in
...
**FR-1**: System MUST accept input matching `agentops` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/agentops/predict
        │
        ▼
...
- `predictions_agentops`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/agentops/predict` · body: features → prediction
- `GET /api/v1/ai-types/agentops/metrics` → live metrics.json
- `GET /api/v1/ai-types/agentops/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/agentops/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `agentops_embeddings`
...
**Fairness metrics** (from data/fairness/agentops.json):
