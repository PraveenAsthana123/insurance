# Hybrid Search AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `hybrid-search-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for hybrid search ai tasks in
...
**FR-1**: System MUST accept input matching `hybrid-search-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/hybrid-search-ai/predict
        │
        ▼
...
- `predictions_hybrid-search-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/hybrid-search-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/hybrid-search-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/hybrid-search-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/hybrid-search-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `hybrid-search-ai_embeddings`
...
**Fairness metrics** (from data/fairness/hybrid-search-ai.json):
