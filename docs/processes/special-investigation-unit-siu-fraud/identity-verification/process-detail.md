...
> Department 8 · Special Investigation Unit (SIU) / Fraud · slug `identity-verification` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/identity-verification/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/8/<domain>/identity-verification?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/8/<domain>/identity-verification?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/identity-verification.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/special-investigation-unit-siu-fraud/identity-verification/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-IDENTITY_VERIFICATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-IDENTITY_VERIFICATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-IDENTITY_VERIFICATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-IDENTITY_VERIFICATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-IDENTITY_VERIFICATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-IDENTITY_VERIFICATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-IDENTITY_VERIFICATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/special-investigation-unit-siu-fraud/identity-verification/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-IDENTITY_VERIFICATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/special-investigation-unit-siu-fraud/identity-verification/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/special-investigation-unit-siu-fraud/identity-verification/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/special-investigation-unit-siu-fraud/identity-verification/stats/`
