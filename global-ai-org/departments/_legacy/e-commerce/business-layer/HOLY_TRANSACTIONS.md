# HOLY Beverage — E Commerce — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Recommendation surfaces + cart-abandonment decisions**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/e-commerce` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/e-commerce?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/e-commerce?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/e-commerce?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/e-commerce/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/e-commerce` returns 200 + chronological feed
