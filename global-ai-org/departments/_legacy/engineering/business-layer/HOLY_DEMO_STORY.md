# HOLY Beverage — Engineering — Demo User Story
...
The persona needs to: **cut deploy lead time from 14d to 1d, cut incident MTTR from 4h to 30min, raise change-fail rate < 5%**

Main KPI being moved: **DORA metrics — DF, LT, MTTR, CFR**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/engineering` (the HOLY navigation page for this dept).
...
> "Today Alex spends 80% of their week on manual data
...
> counterfactual. We don't replace Alex — we replace
...
|---|---|---|---|
| 1 | Open `/holy/engineering` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/engineering` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_engineering.py` (planned).
