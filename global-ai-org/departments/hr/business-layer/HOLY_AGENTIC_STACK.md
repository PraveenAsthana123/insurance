# HOLY Beverage — HR — Agentic Stack
...
> actions against and the required scope for each. The HR manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (HR)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every HR agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| **Harness Agent** | Cross-agent sync between HR and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/hr.yaml](../../../../config/scopes/hr.yaml)
...
| Action irreversible (deletion, signing) | Manager (HR) approval |
| Confidence < 0.6 | Manager (HR) review queue |
...
will enforce every grant in `config/scopes/hr.yaml` has a matching audit
