# HOLY Beverage — Procurement — Automated Pipelines
...
|---|---|---|---|
| 1 | Spend Classification | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Spend Classification + Vendor Risk + Negotiation Copilot**._
...
## 3.1 — Spend Classification

**Lifecycle type:** Tabular ML (default — extend per dept-specific lifecycle)  
**Audit prefix:** `pipeline.procurement.spend_classification`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream procurement systems | `outcome=data_received` |
...
| 5 Report | Procurement manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/procurement` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/procurement/<process_id>` | Full 5-phase spec for one process |
