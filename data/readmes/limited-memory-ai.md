# Limited Memory AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `limited-memory-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for limited memory ai tasks in
...
**FR-1**: System MUST accept input matching `limited-memory-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/limited-memory-ai/predict
        │
        ▼
...
- `predictions_limited-memory-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/limited-memory-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/limited-memory-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/limited-memory-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/limited-memory-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `limited-memory-ai_embeddings`
...
**Fairness metrics** (from data/fairness/limited-memory-ai.json):
