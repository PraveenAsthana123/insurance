# Pipeline Checklist · similar-claim-photo-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `similar_claim_photo_rec_raw` |
| Cleaned | Postgres | `similar_claim_photo_rec_clean` |
| Features | Feast / Postgres | `similar_claim_photo_rec_features` |
| Predictions | Postgres | `similar_claim_photo_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/similar-claim-photo-rec/` |
...
1. Input → `similar_claim_photo_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `similar_claim_photo_rec_predictions`
4. Log → ELK / Loki
