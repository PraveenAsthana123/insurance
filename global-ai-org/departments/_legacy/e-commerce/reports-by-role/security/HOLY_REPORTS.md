# HOLY Beverage — E Commerce — Security Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **security** in **e-commerce** uses.
...
- **Portal**: every report retrievable at `/holy/e-commerce/reports?role=security&id=<report_id>`
...
- `request_id`, `actor_role=security`, `dept=e-commerce`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/e-commerce/security
  → list of standard reports

POST /api/v1/holy/reports/e-commerce/security/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/e-commerce/security/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/e-commerce/reports?role=security`
