# Symbolic AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `symbolic-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for symbolic ai tasks in
...
**FR-1**: System MUST accept input matching `symbolic-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/symbolic-ai/predict
        │
        ▼
...
- `predictions_symbolic-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/symbolic-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/symbolic-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/symbolic-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/symbolic-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `symbolic-ai_embeddings`
...
**Fairness metrics** (from data/fairness/symbolic-ai.json):
