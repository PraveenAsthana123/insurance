"""Tier 3 — API tests for security-operations.
...
def test_security_operations_list_returns_200():
    ...
    body = r.json()
    assert "security-operations" in body.get("departments", []), f"security-operations not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_security_operations_invalid_input_returns_400():
