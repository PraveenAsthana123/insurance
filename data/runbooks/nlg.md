...
WHAT broke?      Check `/data/metrics/nlg.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/nlg.json`
WHO touched?     `git log --since=<date> data/ai_types/nlg.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/nlg/model.joblib.backup models/nlg/model.joblib`
