# Pipeline Checklist · premium-rate-evasion
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `premium_rate_evasion_raw` |
| Cleaned | Postgres | `premium_rate_evasion_clean` |
| Features | Feast / Postgres | `premium_rate_evasion_features` |
| Predictions | Postgres | `premium_rate_evasion_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/premium-rate-evasion/` |
...
1. Input → `premium_rate_evasion_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `premium_rate_evasion_predictions`
4. Log → ELK / Loki
