# Drone AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `drone-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for drone ai tasks in
...
**FR-1**: System MUST accept input matching `drone-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/drone-ai/predict
        │
        ▼
...
- `predictions_drone-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/drone-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/drone-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/drone-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/drone-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `drone-ai_embeddings`
...
**Fairness metrics** (from data/fairness/drone-ai.json):
