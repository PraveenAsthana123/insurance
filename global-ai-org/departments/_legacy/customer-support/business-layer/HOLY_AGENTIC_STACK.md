# HOLY Beverage — Customer Support — Agentic Stack
...
> actions against and the required scope for each. The Customer Support manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Customer Support)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Customer Support agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Customer Support agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Customer Support and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/customer-support.yaml](../../../../config/scopes/customer-support.yaml)
...
| Action irreversible (deletion, signing) | Manager (Customer Support) approval |
| Confidence < 0.6 | Manager (Customer Support) review queue |
...
will enforce every grant in `config/scopes/customer-support.yaml` has a matching audit
