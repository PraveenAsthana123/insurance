# Drug Discovery AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `drug-discovery-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for drug discovery ai tasks in
...
**FR-1**: System MUST accept input matching `drug-discovery-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/drug-discovery-ai/predict
        │
        ▼
...
- `predictions_drug-discovery-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/drug-discovery-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/drug-discovery-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/drug-discovery-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/drug-discovery-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `drug-discovery-ai_embeddings`
...
**Fairness metrics** (from data/fairness/drug-discovery-ai.json):
