# Pipeline Checklist · next-best-product-cross-sell
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `next_best_product_cross_sell_raw` |
| Cleaned | Postgres | `next_best_product_cross_sell_clean` |
| Features | Feast / Postgres | `next_best_product_cross_sell_features` |
| Predictions | Postgres | `next_best_product_cross_sell_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/next-best-product-cross-sell/` |
...
1. Input → `next_best_product_cross_sell_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `next_best_product_cross_sell_predictions`
4. Log → ELK / Loki
