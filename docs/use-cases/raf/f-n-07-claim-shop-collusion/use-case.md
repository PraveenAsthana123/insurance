# Fraud-Numerical-7 · claim-shop-collusion
...
Dept 8 SIU · GNN + community detection
...
|---|---|---|---|
| `INSUR-CLAIM_SHOP_COLLUSION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CLAIM_SHOP_COLLUSION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CLAIM_SHOP_COLLUSION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CLAIM_SHOP_COLLUSION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-CLAIM_SHOP_COLLUSION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-CLAIM_SHOP_COLLUSION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
