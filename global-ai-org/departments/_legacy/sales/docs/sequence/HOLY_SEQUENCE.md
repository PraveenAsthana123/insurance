# HOLY Beverage — Sales — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/sales
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Lead Scoring + Next-Best**

_Lead enter → enrichment → XGBoost score → next-best-action → CRM_
...
When a user types a goal in `/holy/sales/agentic`:
