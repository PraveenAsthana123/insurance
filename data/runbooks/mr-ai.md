...
WHAT broke?      Check `/data/metrics/mr-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/mr-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/mr-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/mr-ai/model.joblib.backup models/mr-ai/model.joblib`
