"""Tier 3 — API tests for manufacturing.
...
def test_manufacturing_list_returns_200():
    ...
    body = r.json()
    assert "manufacturing" in body.get("departments", []), f"manufacturing not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_manufacturing_invalid_input_returns_400():
