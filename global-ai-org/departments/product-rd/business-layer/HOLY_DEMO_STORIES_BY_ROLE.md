...
any 3-5 to assemble a Product Rd board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Product Rd
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is Admin (System)'s view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: Dept Manager in Product Rd
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is Dept Manager's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: Team Member in Product Rd
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is Team Member's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Product Rd
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is QA / Tester's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: SecOps Engineer in Product Rd
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is SecOps Engineer's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Product Rd
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is DevOps's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: AI Reviewer in Product Rd
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is AI Reviewer's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: DT Lead in Product Rd
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is DT Lead's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: System Architect in Product Rd
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is System Architect's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: Test Architect in Product Rd
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is Test Architect's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: DB Architect in Product Rd
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is DB Architect's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: API Architect in Product Rd
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is API Architect's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: Data Owner in Product Rd
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is Data Owner's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: AI Strategy Lead in Product Rd
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is AI Strategy Lead's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
- **Persona**: InfoSec / CISO Office in Product Rd
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Product Rd dashboards
- **KPI moved**: Concept-to-launch + hit rate
...
- **Talking points**: "This is InfoSec / CISO Office's view of Product Rd. In 90 seconds we'll move Concept-to-launch + hit rate by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/product-rd` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/product-rd` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/product-rd/<role>` | Single role demo detail |
