# NLP AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `nlp-ai` · **Kind**: `nlp_baseline`
...
**Business goal**: Reduce manual effort + improve accuracy for nlp ai tasks in
...
**FR-1**: System MUST accept input matching `nlp-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/nlp-ai/predict
        │
        ▼
...
- `predictions_nlp-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/nlp-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/nlp-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/nlp-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/nlp-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/nlp-ai.json):
