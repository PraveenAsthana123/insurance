# Supply Chain AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `supply-chain-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for supply chain ai tasks in
...
**FR-1**: System MUST accept input matching `supply-chain-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/supply-chain-ai/predict
        │
        ▼
...
- `predictions_supply-chain-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/supply-chain-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/supply-chain-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/supply-chain-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/supply-chain-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `supply-chain-ai_embeddings`
...
**Fairness metrics** (from data/fairness/supply-chain-ai.json):
