# Benchmarking AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `benchmarking-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for benchmarking ai tasks in
...
**FR-1**: System MUST accept input matching `benchmarking-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/benchmarking-ai/predict
        │
        ▼
...
- `predictions_benchmarking-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/benchmarking-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/benchmarking-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/benchmarking-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/benchmarking-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `benchmarking-ai_embeddings`
...
**Fairness metrics** (from data/fairness/benchmarking-ai.json):
