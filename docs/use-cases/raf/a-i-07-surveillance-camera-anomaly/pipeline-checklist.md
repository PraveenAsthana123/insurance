# Pipeline Checklist · surveillance-camera-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `surveillance_camera_anomaly_raw` |
| Cleaned | Postgres | `surveillance_camera_anomaly_clean` |
| Features | Feast / Postgres | `surveillance_camera_anomaly_features` |
| Predictions | Postgres | `surveillance_camera_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/surveillance-camera-anomaly/` |
...
1. Input → `surveillance_camera_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `surveillance_camera_anomaly_predictions`
4. Log → ELK / Loki
