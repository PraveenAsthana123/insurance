# HOLY Beverage — Marketing — Automated Pipelines
...
|---|---|---|---|
| 1 | Audience Segmentation | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Audience Segmentation + GenAI Content + Channel Attribution**._
...
**Audit prefix:** `pipeline.marketing.audience_segmentation`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream marketing systems | `outcome=data_received` |
...
| 5 Report | Marketing manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/marketing` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/marketing/<process_id>` | Full 5-phase spec for one process |
