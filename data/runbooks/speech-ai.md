...
WHAT broke?      Check `/data/metrics/speech-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/speech-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/speech-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/speech-ai/model.joblib.backup models/speech-ai/model.joblib`
