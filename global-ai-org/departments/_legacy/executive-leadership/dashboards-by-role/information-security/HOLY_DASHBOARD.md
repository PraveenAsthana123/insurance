# HOLY Beverage — Executive Leadership — Information Security Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **information-security** in **executive-leadership**.

## Persona

The **information-security** at Executive Leadership. They use this dashboard
...
Pen-test results + compliance gates + CVE backlog + third-party risk

## KPIs Surfaced (Tiles)

- Pen-test findings open
- Compliance gates passed/total
- CVE backlog (CVSS≥7)
- Third-party risk score
- Sensitive-data scan findings
- Audit-row volume (24h)
...
- Compliance gate matrix (control × passing)
- CVE backlog by severity (stacked)
- Third-party risk per vendor (heatmap)

Library dispatch per chart (recharts / plotly / echarts / d3 / vega-lite)
returned by backend `GET /api/v1/holy/dashboards/executive-leadership/information-security/charts/<chart_id>`.
...
Inline actions the **information-security** can take from this dashboard:
...
```
GET /api/v1/holy/dashboards/executive-leadership/information-security
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/executive-leadership/information-security/charts/<chart_id>
  → {library, type, data, baseline, threshold, drill_url, refreshed_at}
```

## Related Artifacts

- `../../reports-by-role/information-security/HOLY_REPORTS.md` — paired standard reports
...
- Frontend route: `/holy/executive-leadership/dashboard?role=information-security`
