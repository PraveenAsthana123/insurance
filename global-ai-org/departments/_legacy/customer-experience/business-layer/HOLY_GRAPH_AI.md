# HOLY Beverage — Customer Experience — Graph AI
...
> Anchor flow: **customer ─ files ticket ─ scored by intent_classifier**.
...
```
customer ─ files ticket ─ scored by intent_classifier
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in customer-experience touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/customer-experience` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/customer-experience/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/customer-experience/neighbors/<node_id>` | 1-hop neighbors of a node |
