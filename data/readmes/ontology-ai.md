# Ontology AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ontology-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for ontology ai tasks in
...
**FR-1**: System MUST accept input matching `ontology-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ontology-ai/predict
        │
        ▼
...
- `predictions_ontology-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ontology-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/ontology-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/ontology-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ontology-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ontology-ai_embeddings`
...
**Fairness metrics** (from data/fairness/ontology-ai.json):
