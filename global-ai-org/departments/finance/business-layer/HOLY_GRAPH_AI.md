# HOLY Beverage — Finance — Graph AI
...
> Anchor flow: **transaction ─ scored by fraud_model ─ routed by decision_layer**.
...
```
transaction ─ scored by fraud_model ─ routed by decision_layer
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in finance touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/finance` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/finance/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/finance/neighbors/<node_id>` | 1-hop neighbors of a node |
