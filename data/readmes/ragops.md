# RAGOps · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ragops` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for ragops tasks in
...
**FR-1**: System MUST accept input matching `ragops` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ragops/predict
        │
        ▼
...
- `predictions_ragops`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ragops/predict` · body: features → prediction
- `GET /api/v1/ai-types/ragops/metrics` → live metrics.json
- `GET /api/v1/ai-types/ragops/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ragops/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/ragops.json):
