# HOLY Beverage — Procurement — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/procurement
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Vendor Risk Score Refresh**

_Vendor data → news+sanction RAG → risk model → ranked scorecard_
...
When a user types a goal in `/holy/procurement/agentic`:
