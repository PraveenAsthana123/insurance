# Data Observability AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `data-observability-ai` · **Kind**: `ops_existing`
**Accuracy**: 0.9533 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for data observability ai tasks in
...
**FR-1**: System MUST accept input matching `data-observability-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/data-observability-ai/predict
        │
        ▼
...
- `predictions_data-observability-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/data-observability-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/data-observability-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/data-observability-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/data-observability-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `data-observability-ai_embeddings`
...
**Fairness metrics** (from data/fairness/data-observability-ai.json):
