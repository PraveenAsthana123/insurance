# Pipeline Checklist · compliance-report-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `compliance_report_fraud_raw` |
| Cleaned | Postgres | `compliance_report_fraud_clean` |
| Features | Feast / Postgres | `compliance_report_fraud_features` |
| Predictions | Postgres | `compliance_report_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/compliance-report-fraud/` |
...
1. Input → `compliance_report_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `compliance_report_fraud_predictions`
4. Log → ELK / Loki
