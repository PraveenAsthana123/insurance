# HOLY Beverage — Operations — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/operations
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Process Anomaly Triage**

_Telemetry → drift detect → root-cause LLM → runbook → action_
...
When a user types a goal in `/holy/operations/agentic`:
