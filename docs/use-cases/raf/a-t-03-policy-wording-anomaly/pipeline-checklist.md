# Pipeline Checklist · policy-wording-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `policy_wording_anomaly_raw` |
| Cleaned | Postgres | `policy_wording_anomaly_clean` |
| Features | Feast / Postgres | `policy_wording_anomaly_features` |
| Predictions | Postgres | `policy_wording_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/policy-wording-anomaly/` |
...
1. Input → `policy_wording_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `policy_wording_anomaly_predictions`
4. Log → ELK / Loki
