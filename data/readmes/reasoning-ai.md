# Reasoning AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `reasoning-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for reasoning ai tasks in
...
**FR-1**: System MUST accept input matching `reasoning-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/reasoning-ai/predict
        │
        ▼
...
- `predictions_reasoning-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/reasoning-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/reasoning-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/reasoning-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/reasoning-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `reasoning-ai_embeddings`
...
**Fairness metrics** (from data/fairness/reasoning-ai.json):
