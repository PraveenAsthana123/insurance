# HOLY Beverage — It Operations — Simulation Tab
...
under `REFERENCE_PROCESSES[("it-operations", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/it-operations/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/it-operations/{process}/runs | List past runs |
| GET /api/v1/holy/sim/it-operations/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/it-operations/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for it-operations in `REFERENCE_PROCESSES` dict
