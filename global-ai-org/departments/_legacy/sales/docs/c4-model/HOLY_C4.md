...
graph LR
    ...
    features --> model[Lead Scoring ML, RAG, GenAI, Optimization AI, Forecasting]
...
- Decision lineage: every API call writes a row to `sales_audit` (see LLD §2)

## L6 — Observability

- Logs: structured JSON with `correlation_id`, `tenant_id`, `user_id`, `model_version`, `prompt_version`
- Metrics: `sales_request_count`, `sales_latency_p95`, `sales_model_confidence`, `sales_hallucination_rate`
