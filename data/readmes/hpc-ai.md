# HPC AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `hpc-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for hpc ai tasks in
...
**FR-1**: System MUST accept input matching `hpc-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/hpc-ai/predict
        │
        ▼
...
- `predictions_hpc-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/hpc-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/hpc-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/hpc-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/hpc-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/hpc-ai.json):
