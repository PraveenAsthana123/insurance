...
> Department 1 · Product Management Department · slug `product-performance-monitoring` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/product-performance-monitoring/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/1/<domain>/product-performance-monitoring?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/1/<domain>/product-performance-monitoring?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/product-management-department/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/product-performance-monitoring.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/product-management-department/product-performance-monitoring/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRODUCT_PERFORMANCE_MONITORING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/product-management-department/product-performance-monitoring/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRODUCT_PERFORMANCE_MONITORING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/product-management-department/product-performance-monitoring/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/product-management-department/product-performance-monitoring/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/product-management-department/product-performance-monitoring/stats/`
