# Responsible AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `responsible-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for responsible ai tasks in
...
**FR-1**: System MUST accept input matching `responsible-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/responsible-ai/predict
        │
        ▼
...
- `predictions_responsible-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/responsible-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/responsible-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/responsible-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/responsible-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `responsible-ai_embeddings`
...
**Fairness metrics** (from data/fairness/responsible-ai.json):
