# HOLY Beverage — Operations — Demo User Story
...
The persona needs to: **raise cross-dept throughput 25%, cut operational waste 40%, lift NPS 12 pts**

Main KPI being moved: **Process cycle time, defect rate, operational cost-per-unit**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/operations` (the HOLY navigation page for this dept).
...
> "Today Robert spends 80% of their week on manual data
...
> counterfactual. We don't replace Robert — we replace
...
|---|---|---|---|
| 1 | Open `/holy/operations` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/operations` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_operations.py` (planned).
