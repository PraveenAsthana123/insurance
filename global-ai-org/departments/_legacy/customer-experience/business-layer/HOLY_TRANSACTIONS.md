# HOLY Beverage — Customer Experience — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Ticket intent classifications + auto-reply generations**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/customer-experience` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/customer-experience?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/customer-experience?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/customer-experience?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/customer-experience/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/customer-experience` returns 200 + chronological feed
