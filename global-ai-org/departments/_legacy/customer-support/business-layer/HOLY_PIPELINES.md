# HOLY Beverage — Customer Support — Automated Pipelines
...
|---|---|---|---|
| 1 | Intent Classification | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Intent Classification + RAG Resolution + Escalation Routing**._
...
**Audit prefix:** `pipeline.customer_support.intent_classification`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream customer-support systems | `outcome=data_received` |
...
| 5 Report | Customer Support manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/customer-support` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/customer-support/<process_id>` | Full 5-phase spec for one process |
