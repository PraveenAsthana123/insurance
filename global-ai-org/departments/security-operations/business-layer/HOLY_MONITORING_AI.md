# HOLY Beverage — Security Operations — Monitoring AI
...
> Persona: **SOC Lead**. Focus: **threat-classifier precision + UEBA drift**.
...
Tab: `/holy/security-operations/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/security-operations` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/security-operations/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/security-operations/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #security-operations-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #security-operations-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/security-operations` returns 200 + job list
