# HOLY Beverage — Procurement — Graph AI
...
> Anchor flow: **vendor ─ scored by risk_model ─ holds contract**.
...
```
vendor ─ scored by risk_model ─ holds contract
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in procurement touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/procurement` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/procurement/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/procurement/neighbors/<node_id>` | 1-hop neighbors of a node |
