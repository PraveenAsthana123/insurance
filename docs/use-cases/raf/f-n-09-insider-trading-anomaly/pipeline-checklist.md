# Pipeline Checklist · insider-trading-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `insider_trading_anomaly_raw` |
| Cleaned | Postgres | `insider_trading_anomaly_clean` |
| Features | Feast / Postgres | `insider_trading_anomaly_features` |
| Predictions | Postgres | `insider_trading_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/insider-trading-anomaly/` |
...
1. Input → `insider_trading_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `insider_trading_anomaly_predictions`
4. Log → ELK / Loki
