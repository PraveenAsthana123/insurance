# Recommender-Image-7 · risk-asset-image-rec
...
Dept 15 ERM · ResNet + GIS metadata
...
|---|---|---|---|
| `INSUR-RISK_ASSET_IMAGE_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RISK_ASSET_IMAGE_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RISK_ASSET_IMAGE_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RISK_ASSET_IMAGE_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-RISK_ASSET_IMAGE_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-RISK_ASSET_IMAGE_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
