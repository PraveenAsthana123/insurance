"""Tier 3 — API tests for marketing.
...
def test_marketing_list_returns_200():
    ...
    body = r.json()
    assert "marketing" in body.get("departments", []), f"marketing not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_marketing_invalid_input_returns_400():
