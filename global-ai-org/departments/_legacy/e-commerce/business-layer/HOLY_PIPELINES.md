# HOLY Beverage — E Commerce — Automated Pipelines
...
|---|---|---|---|
| 1 | Recommendation | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Recommendation + Cart Abandonment + Dynamic Pricing**._
...
**Audit prefix:** `pipeline.e_commerce.recommendation`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream e-commerce systems | `outcome=data_received` |
...
| 5 Report | E Commerce manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/e-commerce` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/e-commerce/<process_id>` | Full 5-phase spec for one process |
