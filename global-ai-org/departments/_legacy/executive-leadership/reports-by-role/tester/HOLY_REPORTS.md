# HOLY Beverage — Executive Leadership — Tester Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **tester** in **executive-leadership** uses.

## Standard Reports

| # | Report Name | Cadence | Format | Recipients |
|---|---|---|---|---|
| 1 | Pre-release test report | per release | PDF | manager + dev + QA |
| 2 | Weekly defect rollup | weekly | CSV | dev manager |
| 3 | Monthly test-strategy review | monthly | PDF | test-architect + manager |
...
- **Portal**: every report retrievable at `/holy/executive-leadership/reports?role=tester&id=<report_id>`
...
- `request_id`, `actor_role=tester`, `dept=executive-leadership`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/executive-leadership/tester
  → list of standard reports

POST /api/v1/holy/reports/executive-leadership/tester/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/executive-leadership/tester/<report_id>/runs/<run_id>
  → status + signed download URL
```

## Related Artifacts

- `../../dashboards-by-role/tester/HOLY_DASHBOARD.md` — paired dashboard
- `../../roles/tester/HOLY_README.md` — role definition
- Frontend route: `/holy/executive-leadership/reports?role=tester`
