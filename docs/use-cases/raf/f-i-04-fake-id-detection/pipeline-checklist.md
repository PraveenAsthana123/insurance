# Pipeline Checklist · fake-id-detection
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `fake_id_detection_raw` |
| Cleaned | Postgres | `fake_id_detection_clean` |
| Features | Feast / Postgres | `fake_id_detection_features` |
| Predictions | Postgres | `fake_id_detection_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/fake-id-detection/` |
...
1. Input → `fake_id_detection_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `fake_id_detection_predictions`
4. Log → ELK / Loki
