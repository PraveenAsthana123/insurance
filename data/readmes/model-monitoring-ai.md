# Model Monitoring AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `model-monitoring-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for model monitoring ai tasks in
...
**FR-1**: System MUST accept input matching `model-monitoring-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/model-monitoring-ai/predict
        │
        ▼
...
- `predictions_model-monitoring-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/model-monitoring-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/model-monitoring-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/model-monitoring-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/model-monitoring-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `model-monitoring-ai_embeddings`
...
**Fairness metrics** (from data/fairness/model-monitoring-ai.json):
