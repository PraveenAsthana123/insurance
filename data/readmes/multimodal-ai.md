# Multimodal AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `multimodal-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for multimodal ai tasks in
...
**FR-1**: System MUST accept input matching `multimodal-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/multimodal-ai/predict
        │
        ▼
...
- `predictions_multimodal-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/multimodal-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/multimodal-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/multimodal-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/multimodal-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `multimodal-ai_embeddings`
...
**Fairness metrics** (from data/fairness/multimodal-ai.json):
