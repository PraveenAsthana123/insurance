# HOLY Beverage — Marketing — Graph AI
...
> Anchor flow: **audience_segment ─ targeted by campaign ─ delivered via channel**.
...
```
audience_segment ─ targeted by campaign ─ delivered via channel
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in marketing touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/marketing` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/marketing/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/marketing/neighbors/<node_id>` | 1-hop neighbors of a node |
