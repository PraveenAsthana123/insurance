# HOLY Beverage — Marketing — Api Architect Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **api-architect** in **marketing** uses.
...
- **Portal**: every report retrievable at `/holy/marketing/reports?role=api-architect&id=<report_id>`
...
- `request_id`, `actor_role=api-architect`, `dept=marketing`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/marketing/api-architect
  → list of standard reports

POST /api/v1/holy/reports/marketing/api-architect/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/marketing/api-architect/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/marketing/reports?role=api-architect`
