# Pipeline Checklist · property-photo-benchmark-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `property_photo_benchmark_rec_raw` |
| Cleaned | Postgres | `property_photo_benchmark_rec_clean` |
| Features | Feast / Postgres | `property_photo_benchmark_rec_features` |
| Predictions | Postgres | `property_photo_benchmark_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/property-photo-benchmark-rec/` |
...
1. Input → `property_photo_benchmark_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `property_photo_benchmark_rec_predictions`
4. Log → ELK / Loki
