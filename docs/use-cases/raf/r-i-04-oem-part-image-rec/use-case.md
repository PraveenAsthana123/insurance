# Recommender-Image-4 · oem-part-image-rec
...
Dept 7 Claims · CLIP + OEM catalog
...
|---|---|---|---|
| `INSUR-OEM_PART_IMAGE_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-OEM_PART_IMAGE_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-OEM_PART_IMAGE_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-OEM_PART_IMAGE_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-OEM_PART_IMAGE_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-OEM_PART_IMAGE_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
