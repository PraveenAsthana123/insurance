...
> Department 6 · Billing & Collections · slug `revenue-recognition` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/revenue-recognition/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/6/<domain>/revenue-recognition?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/6/<domain>/revenue-recognition?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/revenue-recognition.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/billing-collections/revenue-recognition/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-REVENUE_RECOGNITION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-REVENUE_RECOGNITION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-REVENUE_RECOGNITION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-REVENUE_RECOGNITION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-REVENUE_RECOGNITION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-REVENUE_RECOGNITION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-REVENUE_RECOGNITION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/billing-collections/revenue-recognition/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-REVENUE_RECOGNITION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/billing-collections/revenue-recognition/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/billing-collections/revenue-recognition/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/billing-collections/revenue-recognition/stats/`
