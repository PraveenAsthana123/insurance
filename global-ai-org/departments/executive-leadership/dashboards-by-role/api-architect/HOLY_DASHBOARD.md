# HOLY Beverage — Executive Leadership — Api Architect Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **api-architect** in **executive-leadership**.

## Persona

The **api-architect** at Executive Leadership. They use this dashboard
...
API latency + error rate + version adoption + deprecation tracker
...
- Latency per endpoint heatmap (endpoint × hour)
- Version-adoption sankey (v1 → v2)
- Error-rate per endpoint (bar)

Library dispatch per chart (recharts / plotly / echarts / d3 / vega-lite)
returned by backend `GET /api/v1/holy/dashboards/executive-leadership/api-architect/charts/<chart_id>`.
...
Inline actions the **api-architect** can take from this dashboard:
...
```
GET /api/v1/holy/dashboards/executive-leadership/api-architect
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/executive-leadership/api-architect/charts/<chart_id>
  → {library, type, data, baseline, threshold, drill_url, refreshed_at}
```

## Related Artifacts

- `../../reports-by-role/api-architect/HOLY_REPORTS.md` — paired standard reports
...
- Frontend route: `/holy/executive-leadership/dashboard?role=api-architect`
