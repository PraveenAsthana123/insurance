# AIOps · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `aiops` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for aiops tasks in
...
**FR-1**: System MUST accept input matching `aiops` schema
...
- `predictions_aiops`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/aiops/predict` · body: features → prediction
- `GET /api/v1/ai-types/aiops/metrics` → live metrics.json
- `GET /api/v1/ai-types/aiops/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/aiops/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/aiops.json):
