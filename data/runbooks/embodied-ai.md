...
WHAT broke?      Check `/data/metrics/embodied-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/embodied-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/embodied-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/embodied-ai/model.joblib.backup models/embodied-ai/model.joblib`
