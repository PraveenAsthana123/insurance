# HOLY Beverage — Product Rd — Manager Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **manager** in **product-rd**.

## Persona

The **manager** at Product Rd. They use this dashboard
...
returned by backend `GET /api/v1/holy/dashboards/product-rd/manager/charts/<chart_id>`.
...
```
GET /api/v1/holy/dashboards/product-rd/manager
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/product-rd/manager/charts/<chart_id>
...
- Frontend route: `/holy/product-rd/dashboard?role=manager`
