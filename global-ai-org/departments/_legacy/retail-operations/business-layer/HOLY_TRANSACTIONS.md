# HOLY Beverage — Retail Operations — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Footfall forecasts + planogram compliance checks**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/retail-operations` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/retail-operations?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/retail-operations?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/retail-operations?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/retail-operations/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/retail-operations` returns 200 + chronological feed
