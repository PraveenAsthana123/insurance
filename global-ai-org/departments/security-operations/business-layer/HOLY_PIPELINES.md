# HOLY Beverage — Security Operations — Automated Pipelines
...
|---|---|---|---|
| 1 | Threat Classifier | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Threat Classifier + UEBA + Phishing Detection**._
...
## 3.1 — Threat Classifier

**Lifecycle type:** Tabular ML (default — extend per dept-specific lifecycle)  
**Audit prefix:** `pipeline.security_operations.threat_classifier`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream security-operations systems | `outcome=data_received` |
...
| 5 Report | Security Operations manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/security-operations` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/security-operations/<process_id>` | Full 5-phase spec for one process |
