# HOLY Beverage — Manufacturing Network Flow
...
```
Manufacturing API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Manufacturing API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          manufacturing_user_events       analytics-svc
Backend API       ──►          manufacturing_audit              audit-store
ML Worker         ──►          manufacturing_predictions        prediction-store
Drift detector    ──►          manufacturing_alerts             on-call PagerDuty
...
|---|---|---|---|
| manufacturing-api | postgresql | 5432 | TCP |
| manufacturing-api | redis | 6379 | TCP |
| manufacturing-api | ollama | 11434 | TCP |
| manufacturing-api | otel-collector | 4317 | gRPC |
| manufacturing-api | (other dept APIs) | 8000 | HTTP via service mesh |
