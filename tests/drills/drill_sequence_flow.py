#!/usr/bin/env python3
"""Drill: INSUR sequence-flow docs per dept (§47 C4-L4 + §38 audit + §49 compose).

Steps (10 total; 3 negative):
    1. (+) all 19 docs/sequence/INSUR_SEQUENCE.md exist + ≥ 4000B
    ...
    7. (-) NEGATIVE — no doc references INSUR_*.md files that don't exist
...
def main():
    print("\nDRILL: INSUR sequence-flow per dept (§47 C4-L4 + §38 + §49)\n")
    t0 = time.time()
    ...
    for dept in DEPARTMENTS:
        f = root / dept / "docs" / "sequence" / "INSUR_SEQUENCE.md"
        if not f.exists():
            bad_exist.append(f"{dept}: missing")
        elif f.stat().st_size < 4000:
            bad_exist.append(f"{dept}: tiny ({f.stat().st_size}B)")
    step(1, f"all {len(DEPARTMENTS)} INSUR_SEQUENCE.md exist + ≥ 4000B",
    ...
    bad_count = []
    for dept in DEPARTMENTS:
        body = (root / dept / "docs" / "sequence" / "INSUR_SEQUENCE.md").read_text()
    ...
    # Compose-footer cites paths like ../hld/INSUR_HLD.md — verify each resolves.
    broken_links: list[str] = []
    link_pat = re.compile(r"\(\.\.?/[^)]*INSUR_[A-Z_]+\.md\)")
    for dept in DEPARTMENTS:
        seq_file = root / dept / "docs" / "sequence" / "INSUR_SEQUENCE.md"
    ...
    step(7, "NEGATIVE: no broken compose-footer links (every INSUR_*.md resolves)",
    ...
    for dept in DEPARTMENTS:
        ...
        unique_links = set(insur_link_pat.findall(footer))
    ...
    fake = root / "totally-bogus-dept-99" / "docs" / "sequence" / "INSUR_SEQUENCE.md"
