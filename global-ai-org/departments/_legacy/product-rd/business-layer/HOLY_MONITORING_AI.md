# HOLY Beverage — Product Rd — Monitoring AI
...
> Persona: **R&D Lead**. Focus: **feature-extraction NLP + concept-similarity drift**.
...
Tab: `/holy/product-rd/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/product-rd` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/product-rd/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/product-rd/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #product-rd-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #product-rd-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/product-rd` returns 200 + job list
