# Fraud-Image-5 · receipt-fraud-detection
...
Dept 17 Procurement · TrOCR + hash + tamper-CNN
...
|---|---|---|---|
| `INSUR-RECEIPT_FRAUD_DETECTION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RECEIPT_FRAUD_DETECTION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RECEIPT_FRAUD_DETECTION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RECEIPT_FRAUD_DETECTION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-RECEIPT_FRAUD_DETECTION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-RECEIPT_FRAUD_DETECTION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
