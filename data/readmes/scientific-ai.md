# Scientific AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `scientific-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for scientific ai tasks in
...
**FR-1**: System MUST accept input matching `scientific-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/scientific-ai/predict
        │
        ▼
...
- `predictions_scientific-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/scientific-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/scientific-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/scientific-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/scientific-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `scientific-ai_embeddings`
...
**Fairness metrics** (from data/fairness/scientific-ai.json):
