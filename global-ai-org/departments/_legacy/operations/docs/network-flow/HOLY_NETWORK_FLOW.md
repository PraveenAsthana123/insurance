# HOLY Beverage — Operations Network Flow
...
```
Operations API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Operations API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          operations_user_events       analytics-svc
Backend API       ──►          operations_audit              audit-store
ML Worker         ──►          operations_predictions        prediction-store
Drift detector    ──►          operations_alerts             on-call PagerDuty
...
|---|---|---|---|
| operations-api | postgresql | 5432 | TCP |
| operations-api | redis | 6379 | TCP |
| operations-api | ollama | 11434 | TCP |
| operations-api | otel-collector | 4317 | gRPC |
| operations-api | (other dept APIs) | 8000 | HTTP via service mesh |
