# AI Red Teaming · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-red-teaming` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for ai red teaming tasks in
...
**FR-1**: System MUST accept input matching `ai-red-teaming` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-red-teaming/predict
        │
        ▼
...
- `predictions_ai-red-teaming`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-red-teaming/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-red-teaming/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-red-teaming/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-red-teaming/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-red-teaming_embeddings`
...
**Fairness metrics** (from data/fairness/ai-red-teaming.json):
