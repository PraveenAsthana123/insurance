# Pipeline Checklist · vendor-equipment-image-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `vendor_equipment_image_rec_raw` |
| Cleaned | Postgres | `vendor_equipment_image_rec_clean` |
| Features | Feast / Postgres | `vendor_equipment_image_rec_features` |
| Predictions | Postgres | `vendor_equipment_image_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/vendor-equipment-image-rec/` |
...
1. Input → `vendor_equipment_image_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `vendor_equipment_image_rec_predictions`
4. Log → ELK / Loki
