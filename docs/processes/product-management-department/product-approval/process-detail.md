...
> Department 1 · Product Management Department · slug `product-approval` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/product-approval/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/1/<domain>/product-approval?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/1/<domain>/product-approval?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/product-approval.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/product-management-department/product-approval/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRODUCT_APPROVAL-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRODUCT_APPROVAL-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRODUCT_APPROVAL-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRODUCT_APPROVAL-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRODUCT_APPROVAL-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRODUCT_APPROVAL-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRODUCT_APPROVAL-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/product-management-department/product-approval/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRODUCT_APPROVAL-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/product-management-department/product-approval/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/product-management-department/product-approval/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/product-management-department/product-approval/stats/`
