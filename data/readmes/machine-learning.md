# Machine Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `machine-learning` · **Kind**: `spec_only`
**Accuracy**: 0.9667 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for machine learning tasks in
...
**FR-1**: System MUST accept input matching `machine-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/machine-learning/predict
        │
        ▼
...
- `predictions_machine-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/machine-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/machine-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/machine-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/machine-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `machine-learning_embeddings`
...
**Fairness metrics** (from data/fairness/machine-learning.json):
