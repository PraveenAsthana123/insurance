# Image Segmentation AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `image-segmentation-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for image segmentation ai tasks in
...
**FR-1**: System MUST accept input matching `image-segmentation-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/image-segmentation-ai/predict
        │
        ▼
...
- `predictions_image-segmentation-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/image-segmentation-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/image-segmentation-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/image-segmentation-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/image-segmentation-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `image-segmentation-ai_embeddings`
...
**Fairness metrics** (from data/fairness/image-segmentation-ai.json):
