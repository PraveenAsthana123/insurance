...
> Department 14 · Finance & Accounting · slug `forecasting` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/forecasting/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/14/<domain>/forecasting?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/14/<domain>/forecasting?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/forecasting.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/finance-accounting/forecasting/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-FORECASTING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-FORECASTING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-FORECASTING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-FORECASTING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-FORECASTING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-FORECASTING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-FORECASTING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/finance-accounting/forecasting/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-FORECASTING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/finance-accounting/forecasting/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/finance-accounting/forecasting/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/finance-accounting/forecasting/stats/`
