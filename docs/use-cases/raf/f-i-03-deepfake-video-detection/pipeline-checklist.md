# Pipeline Checklist · deepfake-video-detection
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `deepfake_video_detection_raw` |
| Cleaned | Postgres | `deepfake_video_detection_clean` |
| Features | Feast / Postgres | `deepfake_video_detection_features` |
| Predictions | Postgres | `deepfake_video_detection_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/deepfake-video-detection/` |
...
1. Input → `deepfake_video_detection_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `deepfake_video_detection_predictions`
4. Log → ELK / Loki
