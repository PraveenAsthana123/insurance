# Unsupervised Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `unsupervised-learning` · **Kind**: `tabular_ml`
...
**Business goal**: Reduce manual effort + improve accuracy for unsupervised learning tasks in
...
**FR-1**: System MUST accept input matching `unsupervised-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/unsupervised-learning/predict
        │
        ▼
...
- `predictions_unsupervised-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/unsupervised-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/unsupervised-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/unsupervised-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/unsupervised-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `unsupervised-learning_embeddings`
...
**Fairness metrics** (from data/fairness/unsupervised-learning.json):
