# Speech AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `speech-ai` · **Kind**: `audio_baseline`
**Accuracy**: 0.9375 · **Algorithm**: GradientBoost on synthetic mel-spectrogram features

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for speech ai tasks in
...
**FR-1**: System MUST accept input matching `speech-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/speech-ai/predict
        │
        ▼
...
- `predictions_speech-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/speech-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/speech-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/speech-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/speech-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `speech-ai_embeddings`
...
**Fairness metrics** (from data/fairness/speech-ai.json):
