# HOLY Beverage — Security Operations — AI Use-Case Catalog
...
> use cases for **security-operations**. Implementation status tracked per current
> codebase state.
...
|---|---|---|
| ✅ Implemented | **15** of 34 | Working module exists in `backend/ml/reference/*.py` (drill-locked) |
| 🟡 Partial | **14** of 34 | Existing module applies; dept-specific stub deferred |
| ❌ Not yet | **5** of 34 | Backlogged — needs new module or external integration |
...
## 2. Security Operations-Specific Use Cases

| Use Case | Category | Data Types | AI Area | Business Value | Status |
|---|---|---|---|---|---|
| SOC Copilot AI | Security | logs/alerts | Security AI | Faster triage | 🟡 council_agent.py + attack_simulators |
| Malware Detection AI | Security | binary files | Deep Learning | Higher detection | 🟡 cv_lifecycle + nlp_lifecycle applies |
| Phishing Detection AI | Security | email/URL | NLP + Classification | Less successful phishing | 🟡 nlp_lifecycle.py + attack_simulators corpus |
| UEBA (User Behavior AI) | Security | auth logs | Behavioral AI | Insider threat | 🟡 anomaly_lifecycle.py applies |
| Prompt Injection Defense | Security | prompts/responses | AI Security | Safer LLMs | 🟡 attack_simulators.py + agentic_stack PolicyEngine |
| AI Threat Detection (Network) | Security | logs/events/PCAP | Anomaly Detection | Faster detection | ✅ anomaly_lifecycle.py |
| AI Compliance Monitoring | Security | logs/policies | Rule + NLP | Less audit findings | 🟡 §38 audit-row stack |
