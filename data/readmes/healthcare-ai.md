# Healthcare AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `healthcare-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for healthcare ai tasks in
...
**FR-1**: System MUST accept input matching `healthcare-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/healthcare-ai/predict
        │
        ▼
...
- `predictions_healthcare-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/healthcare-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/healthcare-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/healthcare-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/healthcare-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `healthcare-ai_embeddings`
...
**Fairness metrics** (from data/fairness/healthcare-ai.json):
