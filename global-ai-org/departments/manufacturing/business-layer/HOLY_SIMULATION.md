# HOLY Beverage — Manufacturing — Simulation Tab
...
under `REFERENCE_PROCESSES[("manufacturing", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/manufacturing/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/manufacturing/{process}/runs | List past runs |
| GET /api/v1/holy/sim/manufacturing/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/manufacturing/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for manufacturing in `REFERENCE_PROCESSES` dict
