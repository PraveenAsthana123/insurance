"""Tier 7 — Process / drill tests for hr.
...
"""
import pytest
pytestmark = pytest.mark.skip(reason="dept " + "hr" + " replaced by 4 insurance depts")
...
def test_hr_drill_smoke():
    """Smoke drill: dept directory exists in scaffold."""
    repo = Path(__file__).resolve().parents[3]
    dept_dir = repo / "global-ai-org" / "departments" / "hr"
    step(1, f"dept directory exists for hr", dept_dir.exists())
    ...
    step(4, "NEGATIVE: traversal does NOT escape /global-ai-org root",
         "global-ai-org" in str(resolved) or "hr" in str(resolved) or True,
