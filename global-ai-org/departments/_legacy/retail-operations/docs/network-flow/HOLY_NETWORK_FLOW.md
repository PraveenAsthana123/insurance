# HOLY Beverage — Retail Operations Network Flow
...
```
Retail Operations API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Retail Operations API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          retail-operations_user_events       analytics-svc
Backend API       ──►          retail-operations_audit              audit-store
ML Worker         ──►          retail-operations_predictions        prediction-store
Drift detector    ──►          retail-operations_alerts             on-call PagerDuty
...
|---|---|---|---|
| retail-operations-api | postgresql | 5432 | TCP |
| retail-operations-api | redis | 6379 | TCP |
| retail-operations-api | ollama | 11434 | TCP |
| retail-operations-api | otel-collector | 4317 | gRPC |
| retail-operations-api | (other dept APIs) | 8000 | HTTP via service mesh |
