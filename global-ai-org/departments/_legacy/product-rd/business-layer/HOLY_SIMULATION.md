# HOLY Beverage — Product Rd — Simulation Tab
...
under `REFERENCE_PROCESSES[("product-rd", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/product-rd/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/product-rd/{process}/runs | List past runs |
| GET /api/v1/holy/sim/product-rd/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/product-rd/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for product-rd in `REFERENCE_PROCESSES` dict
