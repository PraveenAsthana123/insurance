# Email AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `email-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for email ai tasks in
...
**FR-1**: System MUST accept input matching `email-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/email-ai/predict
        │
        ▼
...
- `predictions_email-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/email-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/email-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/email-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/email-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `email-ai_embeddings`
...
**Fairness metrics** (from data/fairness/email-ai.json):
