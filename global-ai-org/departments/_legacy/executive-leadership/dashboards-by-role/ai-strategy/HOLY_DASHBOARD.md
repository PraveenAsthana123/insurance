# HOLY Beverage — Executive Leadership — Ai Strategy Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **ai-strategy** in **executive-leadership**.

## Persona

The **ai-strategy** at Executive Leadership. They use this dashboard
...
Automation backlog rank + AS-IS impact tracker + ROI realized

## KPIs Surfaced (Tiles)

- Automation backlog (top 10)
- AS-IS impact $ (sum)
- TO-BE deployed
- ROI realized %
- Open DT-strategy items
...
- Backlog ranked by score (horizontal bar with breakdown)
- Impact realized vs planned (waterfall)
- Per-dept automation maturity (radar)

Library dispatch per chart (recharts / plotly / echarts / d3 / vega-lite)
returned by backend `GET /api/v1/holy/dashboards/executive-leadership/ai-strategy/charts/<chart_id>`.
...
Inline actions the **ai-strategy** can take from this dashboard:
...
```
GET /api/v1/holy/dashboards/executive-leadership/ai-strategy
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/executive-leadership/ai-strategy/charts/<chart_id>
  → {library, type, data, baseline, threshold, drill_url, refreshed_at}
```

## Related Artifacts

- `../../reports-by-role/ai-strategy/HOLY_REPORTS.md` — paired standard reports
...
- Frontend route: `/holy/executive-leadership/dashboard?role=ai-strategy`
