# Knowledge Graph AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `knowledge-graph-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for knowledge graph ai tasks in
...
**FR-1**: System MUST accept input matching `knowledge-graph-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/knowledge-graph-ai/predict
        │
        ▼
...
- `predictions_knowledge-graph-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/knowledge-graph-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/knowledge-graph-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/knowledge-graph-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/knowledge-graph-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `knowledge-graph-ai_embeddings`
...
**Fairness metrics** (from data/fairness/knowledge-graph-ai.json):
