# NLU · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `nlu` · **Kind**: `nlp_baseline`
...
**Business goal**: Reduce manual effort + improve accuracy for nlu tasks in
...
**FR-1**: System MUST accept input matching `nlu` schema
...
- `predictions_nlu`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/nlu/predict` · body: features → prediction
- `GET /api/v1/ai-types/nlu/metrics` → live metrics.json
- `GET /api/v1/ai-types/nlu/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/nlu/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/nlu.json):
