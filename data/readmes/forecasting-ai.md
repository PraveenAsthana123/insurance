# Forecasting AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `forecasting-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for forecasting ai tasks in
...
**FR-1**: System MUST accept input matching `forecasting-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/forecasting-ai/predict
        │
        ▼
...
- `predictions_forecasting-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/forecasting-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/forecasting-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/forecasting-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/forecasting-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `forecasting-ai_embeddings`
...
**Fairness metrics** (from data/fairness/forecasting-ai.json):
