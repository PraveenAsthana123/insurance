...
WHAT broke?      Check `/data/metrics/ocr-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ocr-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/ocr-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ocr-ai/model.joblib.backup models/ocr-ai/model.joblib`
