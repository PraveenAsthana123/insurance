# Planning AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `planning-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for planning ai tasks in
...
**FR-1**: System MUST accept input matching `planning-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/planning-ai/predict
        │
        ▼
...
- `predictions_planning-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/planning-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/planning-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/planning-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/planning-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `planning-ai_embeddings`
...
**Fairness metrics** (from data/fairness/planning-ai.json):
