...
def section_1_terminal() -> list[dict]:
    return [
        ...
        {"item": "tool/MCP · name/payload/response/latency",
         "status": "⚠️" if _check_agent("sys_slack_mcp") else "❌",
         "where": "tool_registry + slack_mcp (Iter 49)"},
...
def section_2_ui() -> list[dict]:
    ...
    return [
        ...
        {"item": "Human approval · approve/reject/edit",
         "status": "⚠️", "where": "Intervention tab · POST not wired (Tier A1)"},
        ...
        {"item": "Audit trail · who did what when",
         "status": "⚠️", "where": "audit_chain exists · dedicated UI missing"},
        {"item": "Cost dashboard · token/API/workflow",
         "status": "⚠️", "where": "stage 17 per-run · aggregate UI missing"},
...
def section_3_agents() -> list[dict]:
    return [
        ...
        {"item": "Router Agent",         "status": "⚠️",
         "where": "stage_tool_picker (Iter 56) · no dedicated router"},
        ...
        {"item": "Memory Agent",         "status": "⚠️",
         "where": "stage_memory scaffold (Iter 56)"},
        ...
        {"item": "Cost Agent",           "status": "⚠️",
         "where": "sys_watchdog_tokens (Iter 53) · no dedicated cost agent"},
        {"item": "Monitoring Agent",     "status": "✅",
         "where": "24 watchdog agents (Iter 53)"},
        {"item": "Compliance Agent",     "status": "⚠️",
         "where": "sys_poliai exists · 'Compliance' not explicit name"},
    ]
...
def section_4_registries() -> list[dict]:
    return [
        ...
        {"item": "MCP Registry",        "status": "⚠️",
         "where": "in tool_registry filtered tool_type='MCP'"},
        {"item": "Prompt Registry",     "status": "⚠️",
         "where": "§21 prompt_tracker · no unified table yet"},
        {"item": "Workflow Registry",   "status": "⚠️",
         "where": "Iter 56 22 stages · no separate workflow table"},
        {"item": "Policy Registry",     "status": "✅" if _check_table("ai_policy") else "❌",
         "where": "ai_policy table (Iter 39)"},
        {"item": "Evaluation Registry", "status": "⚠️",
         "where": "Iter 47 test_catalog + Iter 57 evaluation"},
        {"item": "Model Registry",      "status": "⚠️",
         "where": "agent_registry.model_name · no separate table"},
        {"item": "Dataset Registry",    "status": "⚠️",
         "where": "knowledge_base · not formal registry"},
        ...
        {"item": "Cost Registry",       "status": "⚠️",
         "where": "agent_invocation.cost_usd · no aggregate table"},
        {"item": "Access Registry",     "status": "⚠️",
         "where": "rbac_middleware (Iter 31) · no full table"},
    ]
...
def section_5_controls() -> list[dict]:
    return [
        ...
        {"item": "Checkpoint/Resume",    "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Retry Policy",         "status": "⚠️",
         "where": "tool_registry.retry_count · not enforced runtime"},
        {"item": "Timeout Policy",       "status": "✅",
         "where": "agent_registry.timeout_seconds CHECK"},
        {"item": "Dead Letter Queue",    "status": "⚠️",
         "where": "agent_queue 'Failed' status · no DLQ table"},
        {"item": "Fallback Model",       "status": "✅",
         "where": "OpenAI→Anthropic→Ollama→stub (Iter 48)"},
        {"item": "Fallback Tool",        "status": "⚠️",
         "where": "pybreaker mentioned · not wired"},
        {"item": "Kill Switch",          "status": "⚠️",
         "where": "feature_flags · not per-agent"},
...
def section_6_security() -> list[dict]:
    return [
        {"item": "RBAC",                  "status": "⚠️",
         "where": "rbac_middleware (Iter 31) · scoped not full"},
        {"item": "ABAC",                  "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Tenant Isolation",     "status": "⚠️",
         "where": "tenant_id column · no Postgres RLS"},
        {"item": "Document Security",    "status": "❌",
         "where": "NOT WIRED"},
        {"item": "PII Redaction",         "status": "✅",
         "where": "Presidio (Iter 27) + stage 14 (Iter 56)"},
        {"item": "Prompt Injection Defense", "status": "⚠️",
         "where": "stage 14 + sys_watchdog_pii · regex only · Garak pending"},
...
def section_7_testing() -> list[dict]:
    return [
        ...
        {"item": "Agent Test",      "status": "✅",
         "where": "14 test_* agents (Iter 47)"},
        {"item": "Workflow Test",   "status": "⚠️",
         "where": "Iter 56 manual run · no CI yet"},
        ...
        {"item": "Human Review Test","status": "⚠️",
         "where": "approval_workflow exists · no test fixture"},
...
def section_8_observability() -> list[dict]:
    return [
        ...
        {"item": "Tool Latency",        "status": "⚠️",
         "where": "agent_trace_event.duration_ms · per-tool agg missing"},
        ...
        {"item": "Retry Count",         "status": "❌",
         "where": "NO retry_count column on agent_invocation"},
        {"item": "Hallucination Risk",  "status": "✅",
         "where": "Iter 57 RAGAS faithfulness · stage 17"},
        {"item": "Retrieval Score",     "status": "⚠️",
         "where": "TF-IDF score returned · not stored per call"},
