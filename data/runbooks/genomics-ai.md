...
WHAT broke?      Check `/data/metrics/genomics-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/genomics-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/genomics-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/genomics-ai/model.joblib.backup models/genomics-ai/model.joblib`
