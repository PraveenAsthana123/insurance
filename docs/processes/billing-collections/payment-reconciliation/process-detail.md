...
> Department 6 · Billing & Collections · slug `payment-reconciliation` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/payment-reconciliation/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/6/<domain>/payment-reconciliation?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/6/<domain>/payment-reconciliation?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/payment-reconciliation.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/billing-collections/payment-reconciliation/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PAYMENT_RECONCILIATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PAYMENT_RECONCILIATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PAYMENT_RECONCILIATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PAYMENT_RECONCILIATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PAYMENT_RECONCILIATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PAYMENT_RECONCILIATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PAYMENT_RECONCILIATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/billing-collections/payment-reconciliation/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PAYMENT_RECONCILIATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/billing-collections/payment-reconciliation/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/billing-collections/payment-reconciliation/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/billing-collections/payment-reconciliation/stats/`
