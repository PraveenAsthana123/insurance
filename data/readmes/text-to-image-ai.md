# Text-to-Image AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `text-to-image-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for text-to-image ai tasks in
...
**FR-1**: System MUST accept input matching `text-to-image-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/text-to-image-ai/predict
        │
        ▼
...
- `predictions_text-to-image-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/text-to-image-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/text-to-image-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/text-to-image-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/text-to-image-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `text-to-image-ai_embeddings`
...
**Fairness metrics** (from data/fairness/text-to-image-ai.json):
