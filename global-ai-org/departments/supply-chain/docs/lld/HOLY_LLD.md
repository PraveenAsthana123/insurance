# HOLY Beverage — Supply Chain Low-Level Design (LLD)
...
|---|---|---|---|
| `/api/v1/supply-chain/list` | GET | List entities | RBAC |
| `/api/v1/supply-chain/<id>` | GET | Get entity by id | RBAC |
| `/api/v1/supply-chain/<id>` | PUT | Update entity | RBAC + ABAC |
| `/api/v1/supply-chain/forecast` | POST | Trigger forecast/recommendation | RBAC |
| `/api/v1/supply-chain/approve/<id>` | POST | Human approval step | RBAC + audit |
...
CREATE TABLE supply_chain_audit (
...
CREATE INDEX idx_supply_chain_audit_ts ON supply_chain_audit(timestamp);
CREATE INDEX idx_supply_chain_audit_tenant ON supply_chain_audit(tenant_id, timestamp);
```
...
- Metrics (Prometheus): `supply-chain_request_count`, `supply-chain_request_latency_ms`, `supply-chain_model_confidence`
