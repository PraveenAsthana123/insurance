# Reactive AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `reactive-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for reactive ai tasks in
...
**FR-1**: System MUST accept input matching `reactive-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/reactive-ai/predict
        │
        ▼
...
- `predictions_reactive-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/reactive-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/reactive-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/reactive-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/reactive-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `reactive-ai_embeddings`
...
**Fairness metrics** (from data/fairness/reactive-ai.json):
