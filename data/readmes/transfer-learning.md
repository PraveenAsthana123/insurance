# Transfer Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `transfer-learning` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for transfer learning tasks in
...
**FR-1**: System MUST accept input matching `transfer-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/transfer-learning/predict
        │
        ▼
...
- `predictions_transfer-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/transfer-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/transfer-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/transfer-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/transfer-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `transfer-learning_embeddings`
...
**Fairness metrics** (from data/fairness/transfer-learning.json):
