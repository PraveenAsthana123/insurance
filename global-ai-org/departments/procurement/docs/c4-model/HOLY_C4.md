# HOLY Beverage — Procurement C4 Model
...
graph TB
    ...
    holy[HOLY Procurement Platform]
...
graph LR
    ...
    features --> model[NLP, OCR, Scoring ML, Optimization AI, Risk ML]
...
- Decision lineage: every API call writes a row to `procurement_audit` (see LLD §2)

## L6 — Observability

- Logs: structured JSON with `correlation_id`, `tenant_id`, `user_id`, `model_version`, `prompt_version`
- Metrics: `procurement_request_count`, `procurement_latency_p95`, `procurement_model_confidence`, `procurement_hallucination_rate`
