# HOLY Beverage — Sales — Monitoring AI
...
> Persona: **Sales Manager**. Focus: **lead-scoring drift + churn-model accuracy**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/sales` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/sales/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/sales/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #sales-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #sales-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/sales` returns 200 + job list
