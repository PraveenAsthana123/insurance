# HOLY Beverage — Engineering — Agentic Stack
...
> actions against and the required scope for each. The Engineering manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Engineering)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Engineering agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Engineering agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Engineering and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/engineering.yaml](../../../../config/scopes/engineering.yaml)
...
| Action irreversible (deletion, signing) | Manager (Engineering) approval |
| Confidence < 0.6 | Manager (Engineering) review queue |
...
will enforce every grant in `config/scopes/engineering.yaml` has a matching audit
