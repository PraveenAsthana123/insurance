# HOLY Beverage — Legal — Security Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **security** in **legal** uses.
...
- **Portal**: every report retrievable at `/holy/legal/reports?role=security&id=<report_id>`
...
- `request_id`, `actor_role=security`, `dept=legal`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/legal/security
  → list of standard reports

POST /api/v1/holy/reports/legal/security/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/legal/security/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/legal/reports?role=security`
