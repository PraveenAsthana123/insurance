...
> Department 7 · Claims · slug `settlement-recommendation` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/settlement-recommendation/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/7/<domain>/settlement-recommendation?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/7/<domain>/settlement-recommendation?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/settlement-recommendation.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/claims/settlement-recommendation/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-SETTLEMENT_RECOMMENDATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-SETTLEMENT_RECOMMENDATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-SETTLEMENT_RECOMMENDATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-SETTLEMENT_RECOMMENDATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-SETTLEMENT_RECOMMENDATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-SETTLEMENT_RECOMMENDATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-SETTLEMENT_RECOMMENDATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/claims/settlement-recommendation/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-SETTLEMENT_RECOMMENDATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/claims/settlement-recommendation/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/claims/settlement-recommendation/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/claims/settlement-recommendation/stats/`
