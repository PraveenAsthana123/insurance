...
WHAT broke?      Check `/data/metrics/energy-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/energy-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/energy-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/energy-ai/model.joblib.backup models/energy-ai/model.joblib`
