...
WHAT broke?      Check `/data/metrics/agentic-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/agentic-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/agentic-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/agentic-ai/model.joblib.backup models/agentic-ai/model.joblib`
