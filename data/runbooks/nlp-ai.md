...
WHAT broke?      Check `/data/metrics/nlp-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/nlp-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/nlp-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/nlp-ai/model.joblib.backup models/nlp-ai/model.joblib`
