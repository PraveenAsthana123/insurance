# HOLY Beverage — Retail Operations — Automated Pipelines
...
|---|---|---|---|
| 1 | Footfall Forecast | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Footfall Forecast + Planogram Compliance + Staff Scheduling**._
...
## 3.1 — Footfall Forecast

**Lifecycle type:** Tabular ML (default — extend per dept-specific lifecycle)  
**Audit prefix:** `pipeline.retail_operations.footfall_forecast`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream retail-operations systems | `outcome=data_received` |
...
| 5 Report | Retail Operations manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/retail-operations` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/retail-operations/<process_id>` | Full 5-phase spec for one process |
