# HOLY Beverage — Executive Leadership — Agentic Stack
...
> actions against and the required scope for each. The Executive Leadership manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Executive Leadership)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Executive Leadership agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Executive Leadership agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Executive Leadership and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/executive-leadership.yaml](../../../../config/scopes/executive-leadership.yaml)
...
| Action irreversible (deletion, signing) | Manager (Executive Leadership) approval |
| Confidence < 0.6 | Manager (Executive Leadership) review queue |
...
`/holy/executive-leadership/agentic` (per §64.40.5).

## Rollback plan

Every Layer-10 write action MUST have a tested rollback before scope grant.
Per-app rollback specs live in
[ops/runbook/executive-leadership-agentic-rollback.md](../../../../ops/runbook/).
...
will enforce every grant in `config/scopes/executive-leadership.yaml` has a matching audit
