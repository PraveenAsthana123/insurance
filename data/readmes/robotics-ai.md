# Robotics AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `robotics-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for robotics ai tasks in
...
**FR-1**: System MUST accept input matching `robotics-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/robotics-ai/predict
        │
        ▼
...
- `predictions_robotics-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/robotics-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/robotics-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/robotics-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/robotics-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `robotics-ai_embeddings`
...
**Fairness metrics** (from data/fairness/robotics-ai.json):
