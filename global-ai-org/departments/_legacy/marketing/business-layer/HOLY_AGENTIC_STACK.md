# HOLY Beverage — Marketing — Agentic Stack
...
> actions against and the required scope for each. The Marketing manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Marketing)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Marketing agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Marketing agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Marketing and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/marketing.yaml](../../../../config/scopes/marketing.yaml)
...
| Action irreversible (deletion, signing) | Manager (Marketing) approval |
| Confidence < 0.6 | Manager (Marketing) review queue |
...
will enforce every grant in `config/scopes/marketing.yaml` has a matching audit
