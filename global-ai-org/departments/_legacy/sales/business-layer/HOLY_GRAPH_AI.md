...
> Anchor flow: **lead ─ scored by lead_scoring ─ converts to customer**.
...
```
lead ─ scored by lead_scoring ─ converts to customer
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in sales touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/sales` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/sales/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/sales/neighbors/<node_id>` | 1-hop neighbors of a node |
