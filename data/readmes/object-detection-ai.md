# Object Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `object-detection-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for object detection ai tasks in
...
**FR-1**: System MUST accept input matching `object-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/object-detection-ai/predict
        │
        ▼
...
- `predictions_object-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/object-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/object-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/object-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/object-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `object-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/object-detection-ai.json):
