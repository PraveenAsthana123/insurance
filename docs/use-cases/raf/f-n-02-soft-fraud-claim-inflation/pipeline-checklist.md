# Pipeline Checklist · soft-fraud-claim-inflation
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `soft_fraud_claim_inflation_raw` |
| Cleaned | Postgres | `soft_fraud_claim_inflation_clean` |
| Features | Feast / Postgres | `soft_fraud_claim_inflation_features` |
| Predictions | Postgres | `soft_fraud_claim_inflation_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/soft-fraud-claim-inflation/` |
...
1. Input → `soft_fraud_claim_inflation_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `soft_fraud_claim_inflation_predictions`
4. Log → ELK / Loki
