# Anomaly-Text-2 · email-content-anomaly
...
Dept 9 CS · DistilBERT + embed outlier
...
|---|---|---|---|
| `INSUR-EMAIL_CONTENT_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-EMAIL_CONTENT_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-EMAIL_CONTENT_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-EMAIL_CONTENT_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-EMAIL_CONTENT_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-EMAIL_CONTENT_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
