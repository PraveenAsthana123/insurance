# HOLY Beverage — Retail Operations — Graph AI
...
> Anchor flow: **store ─ has planogram ─ holds sku ─ counts footfall**.
...
```
store ─ has planogram ─ holds sku ─ counts footfall
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in retail-operations touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/retail-operations` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/retail-operations/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/retail-operations/neighbors/<node_id>` | 1-hop neighbors of a node |
