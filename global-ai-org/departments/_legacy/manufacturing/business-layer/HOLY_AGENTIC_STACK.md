# HOLY Beverage — Manufacturing — Agentic Stack
...
> actions against and the required scope for each. The Manufacturing manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Manufacturing)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Manufacturing agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Manufacturing agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Manufacturing and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/manufacturing.yaml](../../../../config/scopes/manufacturing.yaml)
...
| Action irreversible (deletion, signing) | Manager (Manufacturing) approval |
| Confidence < 0.6 | Manager (Manufacturing) review queue |
...
will enforce every grant in `config/scopes/manufacturing.yaml` has a matching audit
