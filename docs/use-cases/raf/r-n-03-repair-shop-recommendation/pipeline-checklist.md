# Pipeline Checklist · repair-shop-recommendation
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `repair_shop_recommendation_raw` |
| Cleaned | Postgres | `repair_shop_recommendation_clean` |
| Features | Feast / Postgres | `repair_shop_recommendation_features` |
| Predictions | Postgres | `repair_shop_recommendation_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/repair-shop-recommendation/` |
...
1. Input → `repair_shop_recommendation_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `repair_shop_recommendation_predictions`
4. Log → ELK / Loki
