# Fraud-Image-9 · handwriting-forgery
...
Dept 13 Legal · Siamese signature
...
|---|---|---|---|
| `INSUR-HANDWRITING_FORGERY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HANDWRITING_FORGERY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HANDWRITING_FORGERY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HANDWRITING_FORGERY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-HANDWRITING_FORGERY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-HANDWRITING_FORGERY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
