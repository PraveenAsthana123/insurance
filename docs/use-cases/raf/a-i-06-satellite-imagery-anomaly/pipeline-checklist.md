# Pipeline Checklist · satellite-imagery-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `satellite_imagery_anomaly_raw` |
| Cleaned | Postgres | `satellite_imagery_anomaly_clean` |
| Features | Feast / Postgres | `satellite_imagery_anomaly_features` |
| Predictions | Postgres | `satellite_imagery_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/satellite-imagery-anomaly/` |
...
1. Input → `satellite_imagery_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `satellite_imagery_anomaly_predictions`
4. Log → ELK / Loki
