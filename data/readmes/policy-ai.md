# Policy AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `policy-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for policy ai tasks in
...
**FR-1**: System MUST accept input matching `policy-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/policy-ai/predict
        │
        ▼
...
- `predictions_policy-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/policy-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/policy-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/policy-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/policy-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `policy-ai_embeddings`
...
**Fairness metrics** (from data/fairness/policy-ai.json):
