...
WHAT broke?      Check `/data/metrics/planning-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/planning-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/planning-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/planning-ai/model.joblib.backup models/planning-ai/model.joblib`
