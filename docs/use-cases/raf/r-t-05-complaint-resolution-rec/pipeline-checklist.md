# Pipeline Checklist · complaint-resolution-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `complaint_resolution_rec_raw` |
| Cleaned | Postgres | `complaint_resolution_rec_clean` |
| Features | Feast / Postgres | `complaint_resolution_rec_features` |
| Predictions | Postgres | `complaint_resolution_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/complaint-resolution-rec/` |
...
1. Input → `complaint_resolution_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `complaint_resolution_rec_predictions`
4. Log → ELK / Loki
