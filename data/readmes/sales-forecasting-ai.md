# Sales Forecasting AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `sales-forecasting-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for sales forecasting ai tasks in
...
**FR-1**: System MUST accept input matching `sales-forecasting-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/sales-forecasting-ai/predict
        │
        ▼
...
- `predictions_sales-forecasting-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/sales-forecasting-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/sales-forecasting-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/sales-forecasting-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/sales-forecasting-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `sales-forecasting-ai_embeddings`
...
**Fairness metrics** (from data/fairness/sales-forecasting-ai.json):
