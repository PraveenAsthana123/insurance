# HOLY Beverage — Customer Experience — Demo User Story
...
**Mike, Director of Customer Experience**
...
The persona needs to: **reduce ticket-handling time from 18 min to 6 min while raising CSAT ≥ 4.6**

Main KPI being moved: **Average handle time (AHT) and first-contact resolution (FCR)**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/customer-experience` (the HOLY navigation page for this dept).
...
> "Today Mike spends 80% of their week on manual data
...
> counterfactual. We don't replace Mike — we replace
...
|---|---|---|---|
| 1 | Open `/holy/customer-experience` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/customer-experience` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_customer_experience.py` (planned).
