# Pipeline Checklist · claims-payment-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `claims_payment_anomaly_raw` |
| Cleaned | Postgres | `claims_payment_anomaly_clean` |
| Features | Feast / Postgres | `claims_payment_anomaly_features` |
| Predictions | Postgres | `claims_payment_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/claims-payment-anomaly/` |
...
1. Input → `claims_payment_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `claims_payment_anomaly_predictions`
4. Log → ELK / Loki
