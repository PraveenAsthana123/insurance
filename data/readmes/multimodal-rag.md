# Multimodal RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `multimodal-rag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for multimodal rag tasks in
...
**FR-1**: System MUST accept input matching `multimodal-rag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/multimodal-rag/predict
        │
        ▼
...
- `predictions_multimodal-rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/multimodal-rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/multimodal-rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/multimodal-rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/multimodal-rag/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `multimodal-rag_embeddings`
...
**Fairness metrics** (from data/fairness/multimodal-rag.json):
