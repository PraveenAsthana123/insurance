# Face Recognition AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `face-recognition-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for face recognition ai tasks in
...
**FR-1**: System MUST accept input matching `face-recognition-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/face-recognition-ai/predict
        │
        ▼
...
- `predictions_face-recognition-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/face-recognition-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/face-recognition-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/face-recognition-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/face-recognition-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `face-recognition-ai_embeddings`
...
**Fairness metrics** (from data/fairness/face-recognition-ai.json):
