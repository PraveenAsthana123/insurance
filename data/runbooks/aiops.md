...
WHAT broke?      Check `/data/metrics/aiops.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/aiops.json`
WHO touched?     `git log --since=<date> data/ai_types/aiops.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/aiops/model.joblib.backup models/aiops/model.joblib`
