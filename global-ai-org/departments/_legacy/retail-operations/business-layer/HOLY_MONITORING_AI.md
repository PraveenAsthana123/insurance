# HOLY Beverage — Retail Operations — Monitoring AI
...
> Persona: **Store-Ops Manager**. Focus: **footfall forecast + planogram-compliance CV**.
...
Tab: `/holy/retail-operations/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/retail-operations` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/retail-operations/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/retail-operations/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #retail-operations-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #retail-operations-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/retail-operations` returns 200 + job list
