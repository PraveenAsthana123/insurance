# Demand Forecasting AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `demand-forecasting-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for demand forecasting ai tasks in
...
**FR-1**: System MUST accept input matching `demand-forecasting-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/demand-forecasting-ai/predict
        │
        ▼
...
- `predictions_demand-forecasting-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/demand-forecasting-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/demand-forecasting-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/demand-forecasting-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/demand-forecasting-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `demand-forecasting-ai_embeddings`
...
**Fairness metrics** (from data/fairness/demand-forecasting-ai.json):
