# HOLY Beverage — Manufacturing — Demo User Story
...
The persona needs to: **raise OEE from 68% to 82% and cut unplanned downtime 50%**

Main KPI being moved: **Overall equipment effectiveness (OEE), mean time between failures**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/manufacturing` (the HOLY navigation page for this dept).
...
> "Today James spends 80% of their week on manual data
...
> counterfactual. We don't replace James — we replace
...
|---|---|---|---|
| 1 | Open `/holy/manufacturing` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/manufacturing` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_manufacturing.py` (planned).
