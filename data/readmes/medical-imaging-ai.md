# Medical Imaging AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `medical-imaging-ai` · **Kind**: `spec_only`
**Accuracy**: 0.9433 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for medical imaging ai tasks in
...
**FR-1**: System MUST accept input matching `medical-imaging-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/medical-imaging-ai/predict
        │
        ▼
...
- `predictions_medical-imaging-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/medical-imaging-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/medical-imaging-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/medical-imaging-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/medical-imaging-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `medical-imaging-ai_embeddings`
...
**Fairness metrics** (from data/fairness/medical-imaging-ai.json):
