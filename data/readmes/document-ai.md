# Document AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `document-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for document ai tasks in
...
**FR-1**: System MUST accept input matching `document-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/document-ai/predict
        │
        ▼
...
- `predictions_document-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/document-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/document-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/document-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/document-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `document-ai_embeddings`
...
**Fairness metrics** (from data/fairness/document-ai.json):
