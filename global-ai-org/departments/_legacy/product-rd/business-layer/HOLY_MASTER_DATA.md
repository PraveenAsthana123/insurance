# HOLY Beverage — Product Rd — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **product-master + concept-pipeline + IP-portfolio**.
...
This dept (**product-rd**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/product-rd` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/product-rd/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/product-rd/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/product-rd/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/product-rd/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/product-rd` returns 200 + entity catalog
