# Pipeline Checklist · fake-document-detection
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `fake_document_detection_raw` |
| Cleaned | Postgres | `fake_document_detection_clean` |
| Features | Feast / Postgres | `fake_document_detection_features` |
| Predictions | Postgres | `fake_document_detection_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/fake-document-detection/` |
...
1. Input → `fake_document_detection_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `fake_document_detection_predictions`
4. Log → ELK / Loki
