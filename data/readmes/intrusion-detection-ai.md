# Intrusion Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `intrusion-detection-ai` · **Kind**: `spec_only`
**Accuracy**: 0.945 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for intrusion detection ai tasks in
...
**FR-1**: System MUST accept input matching `intrusion-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/intrusion-detection-ai/predict
        │
        ▼
...
- `predictions_intrusion-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/intrusion-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/intrusion-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/intrusion-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/intrusion-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `intrusion-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/intrusion-detection-ai.json):
