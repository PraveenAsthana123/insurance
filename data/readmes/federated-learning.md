# Federated Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `federated-learning` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for federated learning tasks in
...
**FR-1**: System MUST accept input matching `federated-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/federated-learning/predict
        │
        ▼
...
- `predictions_federated-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/federated-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/federated-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/federated-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/federated-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `federated-learning_embeddings`
...
**Fairness metrics** (from data/fairness/federated-learning.json):
