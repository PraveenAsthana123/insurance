...
> Department 13 · Legal · slug `intellectual-property-ip` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/intellectual-property-ip/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/13/<domain>/intellectual-property-ip?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/13/<domain>/intellectual-property-ip?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/intellectual-property-ip.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/legal/intellectual-property-ip/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-INTELLECTUAL_PROPERTY_IP-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-INTELLECTUAL_PROPERTY_IP-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-INTELLECTUAL_PROPERTY_IP-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-INTELLECTUAL_PROPERTY_IP-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-INTELLECTUAL_PROPERTY_IP-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-INTELLECTUAL_PROPERTY_IP-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-INTELLECTUAL_PROPERTY_IP-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/legal/intellectual-property-ip/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-INTELLECTUAL_PROPERTY_IP-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/legal/intellectual-property-ip/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/legal/intellectual-property-ip/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/legal/intellectual-property-ip/stats/`
