...
WHAT broke?      Check `/data/metrics/symbolic-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/symbolic-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/symbolic-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/symbolic-ai/model.joblib.backup models/symbolic-ai/model.joblib`
