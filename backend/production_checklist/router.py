...
def section_1_terminal() -> list[dict]:
    return [
        ...
        {"item": "RAG · rewrite/top-k/score/citations",
         "status": "✅", "where": "/ril/knowledge/search + RAGAS scoring + eval_registry (Iter 43, 57, 68)"},
...
def section_2_ui() -> list[dict]:
    ...
    return [
        ...
        {"item": "Agent map · planner→worker→reviewer→supervisor",
         "status": "✅", "where": "ProductionPipeline 22-stage flow + Kill Switch panel (Iter 56, 69)"},
        {"item": "Status board",
...
def section_4_registries() -> list[dict]:
    return [
        ...
        {"item": "Incident Registry",   "status": "✅" if _check_table("agent_incident") else "❌",
         "where": "agent_incident + incident_management + incident_rca + incident_timeline + incident_postmortem (Iter 40)"},
...
def section_5_controls() -> list[dict]:
    return [
        ...
        {"item": "Rollback",             "status": "✅",
         "where": "model_registry.fallback + workflow ROLLED_BACK state + Iter 44 contracts"},
        {"item": "Rate Limit",           "status": "✅",
         "where": "concurrency_control · max_concurrent gate (Iter 69)"},
        ...
        {"item": "Concurrency Control",  "status": "✅",
         "where": "concurrency_control table + acquire/release (Iter 69)"},
        {"item": "Queue/Event Bus",      "status": "✅",
         "where": "agent_queue (Iter 38) · Celery (Iter 38)"},
        {"item": "Secrets Manager",      "status": "✅",
         "where": "secrets_vault table + rotation tracking (Iter 69)"},
    ]
...
def section_6_security() -> list[dict]:
    return [
        ...
        {"item": "Document Security",    "status": "✅",
         "where": "dataset_registry.pii_classification + ABAC PII policy + naming_policy (Iter 68-70)"},
...
def section_7_testing() -> list[dict]:
    return [
        ...
        {"item": "Golden Test Set", "status": "✅",
         "where": "golden_test_set table · 10 seeded · 5 agents (Iter 69)"},
        {"item": "Synthetic Data",  "status": "✅",
         "where": "synthetic_dataset table + /synthetic/generate (Iter 69)"},
    ]
