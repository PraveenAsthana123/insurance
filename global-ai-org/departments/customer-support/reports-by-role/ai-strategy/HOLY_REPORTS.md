# HOLY Beverage — Customer Support — Ai Strategy Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **ai-strategy** in **customer-support** uses.
...
- **Portal**: every report retrievable at `/holy/customer-support/reports?role=ai-strategy&id=<report_id>`
...
- `request_id`, `actor_role=ai-strategy`, `dept=customer-support`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/customer-support/ai-strategy
  → list of standard reports

POST /api/v1/holy/reports/customer-support/ai-strategy/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/customer-support/ai-strategy/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/customer-support/reports?role=ai-strategy`
