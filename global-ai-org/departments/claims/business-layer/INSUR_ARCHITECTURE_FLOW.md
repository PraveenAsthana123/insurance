...
flowchart TB
    ...
    subgraph "Domain Agents — Claims"
        A0[Claim Intake Agent]
        A1[Claim Assessment Agent]
        A2[Claim Validation Agent]
        A3[Claim Triage Agent]
        A4[Claim Settlement Agent]
        A5[Claims Investigation Agent]
        A6[Fraud Detection Agent]
        A7[Coverage Verification Agent]
    end
    ...
    Orchestration --> "Domain Agents — Claims"
    "Domain Agents — Claims" --> Models
    "Domain Agents — Claims" --> Memory
    "Domain Agents — Claims" --> Insurance Core
    "Domain Agents — Claims" --> Observability
```
...
| Domain | 11 agents | Claims business logic | Domain |
...
sequenceDiagram
    ...
    U->>GW: POST /api/v1/insur/claims/execute
