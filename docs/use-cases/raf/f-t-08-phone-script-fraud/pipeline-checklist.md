# Pipeline Checklist · phone-script-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `phone_script_fraud_raw` |
| Cleaned | Postgres | `phone_script_fraud_clean` |
| Features | Feast / Postgres | `phone_script_fraud_features` |
| Predictions | Postgres | `phone_script_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/phone-script-fraud/` |
...
1. Input → `phone_script_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `phone_script_fraud_predictions`
4. Log → ELK / Loki
