...
WHAT broke?      Check `/data/metrics/agi.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/agi.json`
WHO touched?     `git log --since=<date> data/ai_types/agi.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/agi/model.joblib.backup models/agi/model.joblib`
