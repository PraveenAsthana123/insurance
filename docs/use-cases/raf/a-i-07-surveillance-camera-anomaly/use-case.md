# Anomaly-Image-7 · surveillance-camera-anomaly
...
Dept 20 Cyber · YOLO + window baseline
...
|---|---|---|---|
| `INSUR-SURVEILLANCE_CAMERA_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SURVEILLANCE_CAMERA_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SURVEILLANCE_CAMERA_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SURVEILLANCE_CAMERA_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SURVEILLANCE_CAMERA_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SURVEILLANCE_CAMERA_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
