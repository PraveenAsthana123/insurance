# HOLY Beverage — Executive Leadership — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/executive-leadership
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Exec KPI Rollup**

_Per-dept KPI → aggregate → variance flag → board pack snapshot_
...
When a user types a goal in `/holy/executive-leadership/agentic`:
