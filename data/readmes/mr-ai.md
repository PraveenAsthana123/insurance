# MR AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `mr-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for mr ai tasks in
...
**FR-1**: System MUST accept input matching `mr-ai` schema
...
- `predictions_mr-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/mr-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/mr-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/mr-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/mr-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/mr-ai.json):
