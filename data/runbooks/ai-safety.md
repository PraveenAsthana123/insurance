...
WHAT broke?      Check `/data/metrics/ai-safety.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ai-safety.json`
WHO touched?     `git log --since=<date> data/ai_types/ai-safety.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ai-safety/model.joblib.backup models/ai-safety/model.joblib`
