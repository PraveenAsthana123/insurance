# HOLY Beverage — Hr — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **employee-master + position + cost-center-assignment + payroll-grades**.
...
This dept (**hr**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/hr` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/hr/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/hr/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/hr/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/hr/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/hr` returns 200 + entity catalog
