# HOLY Beverage — Finance Network Flow
...
```
Finance API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Finance API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          finance_user_events       analytics-svc
Backend API       ──►          finance_audit              audit-store
ML Worker         ──►          finance_predictions        prediction-store
Drift detector    ──►          finance_alerts             on-call PagerDuty
...
|---|---|---|---|
| finance-api | postgresql | 5432 | TCP |
| finance-api | redis | 6379 | TCP |
| finance-api | ollama | 11434 | TCP |
| finance-api | otel-collector | 4317 | gRPC |
| finance-api | (other dept APIs) | 8000 | HTTP via service mesh |
