# HOLY Beverage — Manufacturing — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Defect CV runs + OEE rollups + maintenance predictions**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/manufacturing` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/manufacturing?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/manufacturing?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/manufacturing?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/manufacturing/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/manufacturing` returns 200 + chronological feed
