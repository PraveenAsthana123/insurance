# Fraud-Text-7 · chat-fraud-account-takeover
...
Dept 20 Cyber · stylometric + few-shot LLM
...
|---|---|---|---|
| `INSUR-CHAT_FRAUD_ACCOUNT_TAKEOVER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CHAT_FRAUD_ACCOUNT_TAKEOVER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CHAT_FRAUD_ACCOUNT_TAKEOVER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CHAT_FRAUD_ACCOUNT_TAKEOVER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-CHAT_FRAUD_ACCOUNT_TAKEOVER-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-CHAT_FRAUD_ACCOUNT_TAKEOVER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
