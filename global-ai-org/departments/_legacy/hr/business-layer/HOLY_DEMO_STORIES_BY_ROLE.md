...
any 3-5 to assemble a Hr board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Hr
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is Admin (System)'s view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: Dept Manager in Hr
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is Dept Manager's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: Team Member in Hr
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is Team Member's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Hr
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is QA / Tester's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: SecOps Engineer in Hr
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is SecOps Engineer's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Hr
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is DevOps's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: AI Reviewer in Hr
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is AI Reviewer's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: DT Lead in Hr
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is DT Lead's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: System Architect in Hr
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is System Architect's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: Test Architect in Hr
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is Test Architect's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: DB Architect in Hr
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is DB Architect's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: API Architect in Hr
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is API Architect's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: Data Owner in Hr
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is Data Owner's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: AI Strategy Lead in Hr
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is AI Strategy Lead's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
- **Persona**: InfoSec / CISO Office in Hr
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Hr dashboards
- **KPI moved**: Time-to-hire + attrition
...
- **Talking points**: "This is InfoSec / CISO Office's view of Hr. In 90 seconds we'll move Time-to-hire + attrition by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/hr` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/hr` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/hr/<role>` | Single role demo detail |
