# HOLY Beverage — Executive Leadership — Demo User Story
...
**CEO + Executive Team (board-readout context)**
...
The persona needs to: **data-backed strategy decisions in real time vs quarterly post-mortems**

Main KPI being moved: **Decision cycle time, decision quality (post-hoc), board-meeting prep hours**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/executive-leadership` (the HOLY navigation page for this dept).
...
> "Today CEO + Executive Team (board-readout context) spends 80% of their week on manual data
...
> counterfactual. We don't replace CEO + Executive Team (board-readout context) — we replace
...
|---|---|---|---|
| 1 | Open `/holy/executive-leadership` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/executive-leadership` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_executive_leadership.py` (planned).
