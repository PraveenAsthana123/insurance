# Decision Intelligence · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `decision-intelligence` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for decision intelligence tasks in
...
**FR-1**: System MUST accept input matching `decision-intelligence` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/decision-intelligence/predict
        │
        ▼
...
- `predictions_decision-intelligence`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/decision-intelligence/predict` · body: features → prediction
- `GET /api/v1/ai-types/decision-intelligence/metrics` → live metrics.json
- `GET /api/v1/ai-types/decision-intelligence/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/decision-intelligence/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `decision-intelligence_embeddings`
...
**Fairness metrics** (from data/fairness/decision-intelligence.json):
