# Insurance AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `insurance-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for insurance ai tasks in
...
**FR-1**: System MUST accept input matching `insurance-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/insurance-ai/predict
        │
        ▼
...
- `predictions_insurance-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/insurance-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/insurance-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/insurance-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/insurance-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `insurance-ai_embeddings`
...
**Fairness metrics** (from data/fairness/insurance-ai.json):
