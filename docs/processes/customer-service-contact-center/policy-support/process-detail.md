...
> Department 9 · Customer Service / Contact Center · slug `policy-support` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/policy-support/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/9/<domain>/policy-support?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/9/<domain>/policy-support?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/policy-support.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/customer-service-contact-center/policy-support/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-POLICY_SUPPORT-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-POLICY_SUPPORT-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-POLICY_SUPPORT-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-POLICY_SUPPORT-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-POLICY_SUPPORT-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-POLICY_SUPPORT-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-POLICY_SUPPORT-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/customer-service-contact-center/policy-support/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-POLICY_SUPPORT-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/customer-service-contact-center/policy-support/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/customer-service-contact-center/policy-support/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/customer-service-contact-center/policy-support/stats/`
