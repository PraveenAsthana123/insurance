# Pipeline Checklist · reinsurance-treaty-match
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `reinsurance_treaty_match_raw` |
| Cleaned | Postgres | `reinsurance_treaty_match_clean` |
| Features | Feast / Postgres | `reinsurance_treaty_match_features` |
| Predictions | Postgres | `reinsurance_treaty_match_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/reinsurance-treaty-match/` |
...
1. Input → `reinsurance_treaty_match_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `reinsurance_treaty_match_predictions`
4. Log → ELK / Loki
