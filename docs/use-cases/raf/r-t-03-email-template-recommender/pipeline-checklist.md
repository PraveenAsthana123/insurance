# Pipeline Checklist · email-template-recommender
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `email_template_recommender_raw` |
| Cleaned | Postgres | `email_template_recommender_clean` |
| Features | Feast / Postgres | `email_template_recommender_features` |
| Predictions | Postgres | `email_template_recommender_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/email-template-recommender/` |
...
1. Input → `email_template_recommender_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `email_template_recommender_predictions`
4. Log → ELK / Loki
