# Self-Supervised Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `self-supervised-learning` · **Kind**: `tabular_ml`
**Accuracy**: 0.972 · **Algorithm**: GradientBoostingClassifier

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for self-supervised learning tasks in
...
**FR-1**: System MUST accept input matching `self-supervised-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/self-supervised-learning/predict
        │
        ▼
...
- `predictions_self-supervised-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/self-supervised-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/self-supervised-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/self-supervised-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/self-supervised-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `self-supervised-learning_embeddings`
...
**Fairness metrics** (from data/fairness/self-supervised-learning.json):
