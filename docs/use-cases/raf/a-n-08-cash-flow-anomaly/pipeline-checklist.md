# Pipeline Checklist · cash-flow-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `cash_flow_anomaly_raw` |
| Cleaned | Postgres | `cash_flow_anomaly_clean` |
| Features | Feast / Postgres | `cash_flow_anomaly_features` |
| Predictions | Postgres | `cash_flow_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/cash-flow-anomaly/` |
...
1. Input → `cash_flow_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `cash_flow_anomaly_predictions`
4. Log → ELK / Loki
