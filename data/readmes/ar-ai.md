# AR AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ar-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for ar ai tasks in
...
**FR-1**: System MUST accept input matching `ar-ai` schema
...
- `predictions_ar-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ar-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/ar-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/ar-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ar-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/ar-ai.json):
