# Vision-Language Models · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `vision-language-models` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for vision-language models tasks in
...
**FR-1**: System MUST accept input matching `vision-language-models` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/vision-language-models/predict
        │
        ▼
...
- `predictions_vision-language-models`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/vision-language-models/predict` · body: features → prediction
- `GET /api/v1/ai-types/vision-language-models/metrics` → live metrics.json
- `GET /api/v1/ai-types/vision-language-models/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/vision-language-models/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `vision-language-models_embeddings`
...
**Fairness metrics** (from data/fairness/vision-language-models.json):
