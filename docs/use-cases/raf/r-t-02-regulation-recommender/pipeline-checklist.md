# Pipeline Checklist · regulation-recommender
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `regulation_recommender_raw` |
| Cleaned | Postgres | `regulation_recommender_clean` |
| Features | Feast / Postgres | `regulation_recommender_features` |
| Predictions | Postgres | `regulation_recommender_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/regulation-recommender/` |
...
1. Input → `regulation_recommender_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `regulation_recommender_predictions`
4. Log → ELK / Loki
