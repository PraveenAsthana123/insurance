# Recommender-Numerical-5 · investment-allocation-rec
...
|---|---|---|---|
| `INSUR-INVESTMENT_ALLOCATION_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-INVESTMENT_ALLOCATION_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-INVESTMENT_ALLOCATION_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-INVESTMENT_ALLOCATION_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-INVESTMENT_ALLOCATION_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-INVESTMENT_ALLOCATION_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
