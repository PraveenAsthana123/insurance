# HOLY Beverage — Executive Leadership — Devops Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **devops** in **executive-leadership** uses.

## Standard Reports

| # | Report Name | Cadence | Format | Recipients |
|---|---|---|---|---|
| 1 | Weekly DORA report | weekly | PDF | eng manager |
| 2 | Monthly infra-cost optimization | monthly | Excel | finance + eng |
| 3 | Quarterly capacity review | quarterly | PDF | manager + finops |
...
- **Portal**: every report retrievable at `/holy/executive-leadership/reports?role=devops&id=<report_id>`
...
- `request_id`, `actor_role=devops`, `dept=executive-leadership`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/executive-leadership/devops
  → list of standard reports

POST /api/v1/holy/reports/executive-leadership/devops/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/executive-leadership/devops/<report_id>/runs/<run_id>
  → status + signed download URL
```

## Related Artifacts

- `../../dashboards-by-role/devops/HOLY_DASHBOARD.md` — paired dashboard
- `../../roles/devops/HOLY_README.md` — role definition
- Frontend route: `/holy/executive-leadership/reports?role=devops`
