# Reflection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `reflection-ai` · **Kind**: `agent_engine`
**Accuracy**: 0.9733 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for reflection ai tasks in
...
**FR-1**: System MUST accept input matching `reflection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/reflection-ai/predict
        │
        ▼
...
- `predictions_reflection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/reflection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/reflection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/reflection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/reflection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `reflection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/reflection-ai.json):
