...
any 3-5 to assemble a Procurement board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Procurement
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is Admin (System)'s view of Procurement. In 90 seconds we'll move Spend visibility + savings by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: Dept Manager in Procurement
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is Dept Manager's view of Procurement. In 90 seconds we'll move Spend visibility + savings by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: Team Member in Procurement
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is Team Member's view of Procurement. In 90 seconds we'll move Spend visibility + savings by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Procurement
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is QA / Tester's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: SecOps Engineer in Procurement
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is SecOps Engineer's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Procurement
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is DevOps's view of Procurement. In 90 seconds we'll move Spend visibility + savings by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: AI Reviewer in Procurement
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is AI Reviewer's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: DT Lead in Procurement
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is DT Lead's view of Procurement. In 90 seconds we'll move Spend visibility + savings by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: System Architect in Procurement
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is System Architect's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: Test Architect in Procurement
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is Test Architect's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: DB Architect in Procurement
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is DB Architect's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: API Architect in Procurement
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is API Architect's view of Procurement. In 90 seconds we'll move Spend visibility + savings by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: Data Owner in Procurement
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is Data Owner's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: AI Strategy Lead in Procurement
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is AI Strategy Lead's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
- **Persona**: InfoSec / CISO Office in Procurement
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Procurement dashboards
- **KPI moved**: Spend visibility + savings
...
- **Talking points**: "This is InfoSec / CISO Office's view of Procurement. In 90 seconds we'll move Spend visibility + savings by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/procurement` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/procurement` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/procurement/<role>` | Single role demo detail |
