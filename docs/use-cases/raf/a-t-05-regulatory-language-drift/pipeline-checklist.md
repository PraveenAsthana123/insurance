# Pipeline Checklist · regulatory-language-drift
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `regulatory_language_drift_raw` |
| Cleaned | Postgres | `regulatory_language_drift_clean` |
| Features | Feast / Postgres | `regulatory_language_drift_features` |
| Predictions | Postgres | `regulatory_language_drift_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/regulatory-language-drift/` |
...
1. Input → `regulatory_language_drift_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `regulatory_language_drift_predictions`
4. Log → ELK / Loki
