# Semantic Search AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `semantic-search-ai` · **Kind**: `spec_only`
**Accuracy**: 0.9683 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for semantic search ai tasks in
...
**FR-1**: System MUST accept input matching `semantic-search-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/semantic-search-ai/predict
        │
        ▼
...
- `predictions_semantic-search-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/semantic-search-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/semantic-search-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/semantic-search-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/semantic-search-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `semantic-search-ai_embeddings`
...
**Fairness metrics** (from data/fairness/semantic-search-ai.json):
