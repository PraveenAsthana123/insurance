# Pipeline Checklist · doc-template-image-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `doc_template_image_rec_raw` |
| Cleaned | Postgres | `doc_template_image_rec_clean` |
| Features | Feast / Postgres | `doc_template_image_rec_features` |
| Predictions | Postgres | `doc_template_image_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/doc-template-image-rec/` |
...
1. Input → `doc_template_image_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `doc_template_image_rec_predictions`
4. Log → ELK / Loki
