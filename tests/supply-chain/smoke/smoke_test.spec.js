// Tier 8 — Smoke / E2E test for supply-chain.
//
// Recommended: playwright (per global §64.42).
//
// Run with:  npx playwright test tests/supply-chain/smoke/
//
// Per global §64.30 tier 8: top 20 critical endpoints + UI routes per deploy.

const { test, expect } = require('@playwright/test');

test.describe('supply-chain smoke tests', () => {
  test('INSUR nav page for supply-chain loads', async ({ page }) => {
    await page.goto('http://localhost:3000/insur/supply-chain');
    await expect(page).toHaveURL(/\/insur\/supply-chain/);
