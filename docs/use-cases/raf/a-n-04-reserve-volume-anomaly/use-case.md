# Anomaly-Numerical-4 · reserve-volume-anomaly
...
Dept 10 Actuarial · streaming z-score + EWMA
...
|---|---|---|---|
| `INSUR-RESERVE_VOLUME_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RESERVE_VOLUME_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RESERVE_VOLUME_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RESERVE_VOLUME_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-RESERVE_VOLUME_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-RESERVE_VOLUME_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
