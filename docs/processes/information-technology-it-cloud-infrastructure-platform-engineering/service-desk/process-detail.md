...
> Department 19 · Information Technology (IT), Cloud, Infrastructure & Platform Engineering · slug `service-desk` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/service-desk/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/19/<domain>/service-desk?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/19/<domain>/service-desk?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/service-desk.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/information-technology-it-cloud-infrastructure-platform-engineering/service-desk/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-SERVICE_DESK-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-SERVICE_DESK-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-SERVICE_DESK-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-SERVICE_DESK-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-SERVICE_DESK-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-SERVICE_DESK-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-SERVICE_DESK-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/service-desk/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-SERVICE_DESK-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/service-desk/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/service-desk/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/service-desk/stats/`
