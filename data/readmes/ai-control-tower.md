# AI Control Tower · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-control-tower` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for ai control tower tasks in
...
**FR-1**: System MUST accept input matching `ai-control-tower` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-control-tower/predict
        │
        ▼
...
- `predictions_ai-control-tower`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-control-tower/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-control-tower/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-control-tower/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-control-tower/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-control-tower_embeddings`
...
**Fairness metrics** (from data/fairness/ai-control-tower.json):
