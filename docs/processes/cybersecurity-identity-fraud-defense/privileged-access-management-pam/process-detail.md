...
> Department 20 · Cybersecurity, Identity & Fraud Defense · slug `privileged-access-management-pam` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/privileged-access-management-pam/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/20/<domain>/privileged-access-management-pam?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/20/<domain>/privileged-access-management-pam?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/cybersecurity-identity-fraud-defense/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/privileged-access-management-pam.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/cybersecurity-identity-fraud-defense/privileged-access-management-pam/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/cybersecurity-identity-fraud-defense/privileged-access-management-pam/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-PRIVILEGED_ACCESS_MANAGEMENT_PAM-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/cybersecurity-identity-fraud-defense/privileged-access-management-pam/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/cybersecurity-identity-fraud-defense/privileged-access-management-pam/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/cybersecurity-identity-fraud-defense/privileged-access-management-pam/stats/`
