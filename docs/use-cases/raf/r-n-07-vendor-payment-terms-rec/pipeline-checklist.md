# Pipeline Checklist · vendor-payment-terms-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `vendor_payment_terms_rec_raw` |
| Cleaned | Postgres | `vendor_payment_terms_rec_clean` |
| Features | Feast / Postgres | `vendor_payment_terms_rec_features` |
| Predictions | Postgres | `vendor_payment_terms_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/vendor-payment-terms-rec/` |
...
1. Input → `vendor_payment_terms_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `vendor_payment_terms_rec_predictions`
4. Log → ELK / Loki
