# ReAct AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `react-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for react ai tasks in
...
**FR-1**: System MUST accept input matching `react-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/react-ai/predict
        │
        ▼
...
- `predictions_react-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/react-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/react-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/react-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/react-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `react-ai_embeddings`
...
**Fairness metrics** (from data/fairness/react-ai.json):
