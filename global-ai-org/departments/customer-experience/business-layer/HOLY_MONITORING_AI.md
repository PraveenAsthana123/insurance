# HOLY Beverage — Customer Experience — Monitoring AI
...
> Persona: **CX Manager**. Focus: **sentiment AI freshness + ticket-deflection accuracy**.
...
Tab: `/holy/customer-experience/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/customer-experience` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/customer-experience/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/customer-experience/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #customer-experience-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #customer-experience-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/customer-experience` returns 200 + job list
