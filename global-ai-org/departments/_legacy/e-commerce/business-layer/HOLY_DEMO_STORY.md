# HOLY Beverage — E Commerce — Demo User Story
...
The persona needs to: **raise conversion from 1.8% to 3.5%, cut cart abandonment 40%, AOV +20%**

Main KPI being moved: **Conversion rate, cart abandonment, AOV, fraud chargeback rate**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/e-commerce` (the HOLY navigation page for this dept).
...
> "Today Nina spends 80% of their week on manual data
...
> counterfactual. We don't replace Nina — we replace
...
|---|---|---|---|
| 1 | Open `/holy/e-commerce` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/e-commerce` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_e_commerce.py` (planned).
