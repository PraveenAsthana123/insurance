...
WHAT broke?      Check `/data/metrics/eeg-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/eeg-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/eeg-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/eeg-ai/model.joblib.backup models/eeg-ai/model.joblib`
