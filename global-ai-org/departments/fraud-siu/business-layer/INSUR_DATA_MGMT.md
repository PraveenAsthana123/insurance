# Data Management — Fraud / Special Investigations Unit (SIU)
...
| --- | --- | --- | --- |
| vehicle_claim_fraud | `shivamb/vehicle-claim-fraud-detection` | Vehicle fraud detection | `data/insurance/fraud-siu/vehicle_claim_fraud/` |
| creditcard_fraud | `mlg-ulb/creditcardfraud` | Credit-card fraud (Kaggle classic) | `data/insurance/fraud-siu/creditcard_fraud/` |
| auto_insurance_fraud | `buntyshah/auto-insurance-claims-data` | Auto claims fraud flags | `data/insurance/fraud-siu/auto_insurance_fraud/` |
| ieee_fraud | `ieee-fraud-detection/ieee-fraud-detection` | IEEE-CIS fraud transactions | `data/insurance/fraud-siu/ieee_fraud/` |

## Download Commands

```bash
# Per global §36 — Kaggle CLI is globally installed.
kaggle datasets download -d shivamb/vehicle-claim-fraud-detection -p data/insurance/fraud-siu/vehicle_claim_fraud/ --unzip
kaggle datasets download -d mlg-ulb/creditcardfraud -p data/insurance/fraud-siu/creditcard_fraud/ --unzip
kaggle datasets download -d buntyshah/auto-insurance-claims-data -p data/insurance/fraud-siu/auto_insurance_fraud/ --unzip
kaggle datasets download -d ieee-fraud-detection/ieee-fraud-detection -p data/insurance/fraud-siu/ieee_fraud/ --unzip
```
...
```
data/eval/fraud-siu/<pipeline-name>/<run_id>/plots/
