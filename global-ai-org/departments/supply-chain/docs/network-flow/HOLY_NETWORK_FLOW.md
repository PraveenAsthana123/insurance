# HOLY Beverage — Supply Chain Network Flow
...
```
Supply Chain API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Supply Chain API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          supply-chain_user_events       analytics-svc
Backend API       ──►          supply-chain_audit              audit-store
ML Worker         ──►          supply-chain_predictions        prediction-store
Drift detector    ──►          supply-chain_alerts             on-call PagerDuty
...
|---|---|---|---|
| supply-chain-api | postgresql | 5432 | TCP |
| supply-chain-api | redis | 6379 | TCP |
| supply-chain-api | ollama | 11434 | TCP |
| supply-chain-api | otel-collector | 4317 | gRPC |
| supply-chain-api | (other dept APIs) | 8000 | HTTP via service mesh |
