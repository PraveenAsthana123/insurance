# Content-Based Recommendation AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `content-based-recommendation-ai` · **Kind**: `spec_only`
**Accuracy**: 0.955 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for content-based recommendation ai tasks in
...
**FR-1**: System MUST accept input matching `content-based-recommendation-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/content-based-recommendation-ai/predict
        │
        ▼
...
- `predictions_content-based-recommendation-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/content-based-recommendation-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/content-based-recommendation-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/content-based-recommendation-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/content-based-recommendation-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `content-based-recommendation-ai_embeddings`
...
**Fairness metrics** (from data/fairness/content-based-recommendation-ai.json):
