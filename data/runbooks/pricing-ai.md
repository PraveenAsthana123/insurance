...
WHAT broke?      Check `/data/metrics/pricing-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/pricing-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/pricing-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/pricing-ai/model.joblib.backup models/pricing-ai/model.joblib`
