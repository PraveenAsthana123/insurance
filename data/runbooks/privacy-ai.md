...
WHAT broke?      Check `/data/metrics/privacy-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/privacy-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/privacy-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/privacy-ai/model.joblib.backup models/privacy-ai/model.joblib`
