# HOLY Beverage — Security Operations — Simulation Tab
...
under `REFERENCE_PROCESSES[("security-operations", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/security-operations/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/security-operations/{process}/runs | List past runs |
| GET /api/v1/holy/sim/security-operations/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/security-operations/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for security-operations in `REFERENCE_PROCESSES` dict
