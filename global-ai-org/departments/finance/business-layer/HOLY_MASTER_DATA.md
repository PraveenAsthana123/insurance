# HOLY Beverage — Finance — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **GL-accounts + cost-centers + tax-codes + currency-rates**.
...
This dept (**finance**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/finance` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/finance/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/finance/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/finance/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/finance/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/finance` returns 200 + entity catalog
