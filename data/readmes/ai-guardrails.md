# AI Guardrails · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `ai-guardrails` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for ai guardrails tasks in
...
**FR-1**: System MUST accept input matching `ai-guardrails` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/ai-guardrails/predict
        │
        ▼
...
- `predictions_ai-guardrails`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/ai-guardrails/predict` · body: features → prediction
- `GET /api/v1/ai-types/ai-guardrails/metrics` → live metrics.json
- `GET /api/v1/ai-types/ai-guardrails/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/ai-guardrails/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `ai-guardrails_embeddings`
...
**Fairness metrics** (from data/fairness/ai-guardrails.json):
