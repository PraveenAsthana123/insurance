# Fraud-Numerical-2 · soft-fraud-claim-inflation
...
Dept 8 SIU · LightGBM + peer percentile
...
|---|---|---|---|
| `INSUR-SOFT_FRAUD_CLAIM_INFLATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SOFT_FRAUD_CLAIM_INFLATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SOFT_FRAUD_CLAIM_INFLATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SOFT_FRAUD_CLAIM_INFLATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SOFT_FRAUD_CLAIM_INFLATION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SOFT_FRAUD_CLAIM_INFLATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
