# HOLY Beverage — E Commerce — Team Member Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **team-member** in **e-commerce** uses.
...
- **Portal**: every report retrievable at `/holy/e-commerce/reports?role=team-member&id=<report_id>`
...
- `request_id`, `actor_role=team-member`, `dept=e-commerce`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/e-commerce/team-member
  → list of standard reports

POST /api/v1/holy/reports/e-commerce/team-member/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/e-commerce/team-member/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/e-commerce/reports?role=team-member`
