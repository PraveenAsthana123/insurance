# HOLY Beverage — Finance — Agentic Stack
...
> actions against and the required scope for each. The Finance manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Finance)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Finance agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Finance agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Finance and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/finance.yaml](../../../../config/scopes/finance.yaml)
...
| Action irreversible (deletion, signing) | Manager (Finance) approval |
| Confidence < 0.6 | Manager (Finance) review queue |
...
will enforce every grant in `config/scopes/finance.yaml` has a matching audit
