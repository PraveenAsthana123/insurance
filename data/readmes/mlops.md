# MLOps · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `mlops` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for mlops tasks in
...
**FR-1**: System MUST accept input matching `mlops` schema
...
- `predictions_mlops`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/mlops/predict` · body: features → prediction
- `GET /api/v1/ai-types/mlops/metrics` → live metrics.json
- `GET /api/v1/ai-types/mlops/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/mlops/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/mlops.json):
