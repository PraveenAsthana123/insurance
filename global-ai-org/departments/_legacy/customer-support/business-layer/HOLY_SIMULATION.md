# HOLY Beverage — Customer Support — Simulation Tab
...
under `REFERENCE_PROCESSES[("customer-support", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/customer-support/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/customer-support/{process}/runs | List past runs |
| GET /api/v1/holy/sim/customer-support/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/customer-support/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for customer-support in `REFERENCE_PROCESSES` dict
