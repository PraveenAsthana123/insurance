...
WHAT broke?      Check `/data/metrics/document-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/document-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/document-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/document-ai/model.joblib.backup models/document-ai/model.joblib`
