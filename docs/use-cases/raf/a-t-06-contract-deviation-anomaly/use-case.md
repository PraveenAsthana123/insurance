# Anomaly-Text-6 · contract-deviation-anomaly
...
Dept 13 Legal · LayoutLM + clause outlier
...
|---|---|---|---|
| `INSUR-CONTRACT_DEVIATION_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CONTRACT_DEVIATION_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CONTRACT_DEVIATION_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CONTRACT_DEVIATION_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-CONTRACT_DEVIATION_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-CONTRACT_DEVIATION_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
