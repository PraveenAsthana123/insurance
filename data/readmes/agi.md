# AGI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `agi` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for agi tasks in
...
**FR-1**: System MUST accept input matching `agi` schema
...
- `predictions_agi`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/agi/predict` · body: features → prediction
- `GET /api/v1/ai-types/agi/metrics` → live metrics.json
- `GET /api/v1/ai-types/agi/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/agi/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/agi.json):
