# Recruitment AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `recruitment-ai` · **Kind**: `vertical_spec`
**Accuracy**: 0.9317 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for recruitment ai tasks in
...
**FR-1**: System MUST accept input matching `recruitment-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/recruitment-ai/predict
        │
        ▼
...
- `predictions_recruitment-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/recruitment-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/recruitment-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/recruitment-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/recruitment-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `recruitment-ai_embeddings`
...
**Fairness metrics** (from data/fairness/recruitment-ai.json):
