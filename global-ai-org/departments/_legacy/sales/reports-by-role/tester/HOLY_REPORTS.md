# HOLY Beverage — Sales — Tester Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **tester** in **sales** uses.
...
- **Portal**: every report retrievable at `/holy/sales/reports?role=tester&id=<report_id>`
...
- `request_id`, `actor_role=tester`, `dept=sales`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/sales/tester
  → list of standard reports

POST /api/v1/holy/reports/sales/tester/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/sales/tester/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/sales/reports?role=tester`
