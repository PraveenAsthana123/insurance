# Fraud-Text-8 · phone-script-fraud
...
Dept 9 CS · Whisper + BERT + sentiment
...
|---|---|---|---|
| `INSUR-PHONE_SCRIPT_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PHONE_SCRIPT_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PHONE_SCRIPT_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PHONE_SCRIPT_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PHONE_SCRIPT_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PHONE_SCRIPT_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
