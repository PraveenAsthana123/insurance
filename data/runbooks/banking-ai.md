...
WHAT broke?      Check `/data/metrics/banking-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/banking-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/banking-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/banking-ai/model.joblib.backup models/banking-ai/model.joblib`
