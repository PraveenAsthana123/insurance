...
Consolidated technical architecture for the Claims dept. Pairs with
...
| Domain agents | 11 | Python + Ollama / LLM-router |
...
- L2 processes: 12
- L3 sub-processes: 49
- Total addressable for automation: 49
...
- ADR — Ollama vs OpenAI for Claims (cost + latency + data residency)
- ADR — Chunking strategy for Claims RAG corpus (fixed / sentence / semantic-paragraph)
- ADR — Tenant isolation: row-level vs column-level
- ADR — Decision audit retention (7y regulated vs 1y default)
- ADR — Idempotency key TTL (Claims retry storm pattern)
