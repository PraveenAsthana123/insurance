// Tier 8 — Smoke / E2E test for manufacturing.
//
// Recommended: playwright (per global §64.42).
//
// Run with:  npx playwright test tests/manufacturing/smoke/
//
// Per global §64.30 tier 8: top 20 critical endpoints + UI routes per deploy.

const { test, expect } = require('@playwright/test');

test.describe('manufacturing smoke tests', () => {
  test('INSUR nav page for manufacturing loads', async ({ page }) => {
    await page.goto('http://localhost:3000/insur/manufacturing');
    await expect(page).toHaveURL(/\/insur\/manufacturing/);
