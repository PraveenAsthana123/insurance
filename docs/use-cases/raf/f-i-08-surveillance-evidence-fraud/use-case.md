# Fraud-Image-8 · surveillance-evidence-fraud
...
Dept 20 Cyber · temporal CNN + chain-hash
...
|---|---|---|---|
| `INSUR-SURVEILLANCE_EVIDENCE_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SURVEILLANCE_EVIDENCE_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SURVEILLANCE_EVIDENCE_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SURVEILLANCE_EVIDENCE_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SURVEILLANCE_EVIDENCE_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SURVEILLANCE_EVIDENCE_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
