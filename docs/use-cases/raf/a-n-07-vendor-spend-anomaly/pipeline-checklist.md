# Pipeline Checklist · vendor-spend-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `vendor_spend_anomaly_raw` |
| Cleaned | Postgres | `vendor_spend_anomaly_clean` |
| Features | Feast / Postgres | `vendor_spend_anomaly_features` |
| Predictions | Postgres | `vendor_spend_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/vendor-spend-anomaly/` |
...
1. Input → `vendor_spend_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `vendor_spend_anomaly_predictions`
4. Log → ELK / Loki
