...
WHAT broke?      Check `/data/metrics/physical-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/physical-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/physical-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/physical-ai/model.joblib.backup models/physical-ai/model.joblib`
