...
WHAT broke?      Check `/data/metrics/voice-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/voice-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/voice-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/voice-ai/model.joblib.backup models/voice-ai/model.joblib`
