# HOLY Beverage — Procurement — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Vendor scoring + spend classifications + negotiation drafts**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/procurement` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/procurement?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/procurement?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/procurement?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/procurement/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/procurement` returns 200 + chronological feed
