# HOLY Beverage — Customer Support — Graph AI
...
> Anchor flow: **ticket ─ classified by intent ─ resolved via rag_pipeline**.
...
```
ticket ─ classified by intent ─ resolved via rag_pipeline
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in customer-support touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/customer-support` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/customer-support/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/customer-support/neighbors/<node_id>` | 1-hop neighbors of a node |
