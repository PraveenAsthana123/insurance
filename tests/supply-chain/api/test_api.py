"""Tier 3 — API tests for supply-chain.
...
def test_supply_chain_list_returns_200():
    ...
    body = r.json()
    assert "supply-chain" in body.get("departments", []), f"supply-chain not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_supply_chain_invalid_input_returns_400():
