# Fairness AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `fairness-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for fairness ai tasks in
...
**FR-1**: System MUST accept input matching `fairness-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/fairness-ai/predict
        │
        ▼
...
- `predictions_fairness-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/fairness-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/fairness-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/fairness-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/fairness-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `fairness-ai_embeddings`
...
**Fairness metrics** (from data/fairness/fairness-ai.json):
