# HOLY Beverage — Legal — Ai Reviewer Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **ai-reviewer** in **legal** uses.
...
- **Portal**: every report retrievable at `/holy/legal/reports?role=ai-reviewer&id=<report_id>`
...
- `request_id`, `actor_role=ai-reviewer`, `dept=legal`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/legal/ai-reviewer
  → list of standard reports

POST /api/v1/holy/reports/legal/ai-reviewer/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/legal/ai-reviewer/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/legal/reports?role=ai-reviewer`
