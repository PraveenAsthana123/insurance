# Foundation Models · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `foundation-models` · **Kind**: `spec_only`
**Accuracy**: 0.9167 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for foundation models tasks in
...
**FR-1**: System MUST accept input matching `foundation-models` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/foundation-models/predict
        │
        ▼
...
- `predictions_foundation-models`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/foundation-models/predict` · body: features → prediction
- `GET /api/v1/ai-types/foundation-models/metrics` → live metrics.json
- `GET /api/v1/ai-types/foundation-models/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/foundation-models/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `foundation-models_embeddings`
...
**Fairness metrics** (from data/fairness/foundation-models.json):
