# Recommender-Numerical-3 · repair-shop-recommendation
...
|---|---|---|---|
| `INSUR-REPAIR_SHOP_RECOMMENDATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-REPAIR_SHOP_RECOMMENDATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-REPAIR_SHOP_RECOMMENDATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-REPAIR_SHOP_RECOMMENDATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-REPAIR_SHOP_RECOMMENDATION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-REPAIR_SHOP_RECOMMENDATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
