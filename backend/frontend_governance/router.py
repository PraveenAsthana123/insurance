...
SECTIONS = {
    "1_state_mgmt": [
        {"item": "Workflow state persisted in DB", "status": "⚠️",
         "where": "agent_invocation · no workflow_run table yet"},
        {"item": "Agent state queried on mount", "status": "✅",
         "where": "AgenticHubPage Live Activity tab (Iter 52)"},
        {"item": "UI state in localStorage", "status": "⚠️",
         "where": "some tabs use it · not standardized"},
        {"item": "Filter state per user", "status": "⚠️",
         "where": "AllAgentsNetworkPanel filter exists · not persisted"},
        {"item": "Tab state in sessionStorage", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Form state auto-save (draft to DB)", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "User session restorable", "status": "⚠️",
         "where": "JWT session · no draft state recovery"},
    ],
    "2_f12_security": [
        ...
        {"item": "No internal passwords in bundle",
         ...
         "where": "scanned frontend/src"},
        {"item": "console.log stripped in prod build", "status": "⚠️",
         "where": "Vite default · need terser drop_console=true"},
        {"item": "Source maps not shipped to prod", "status": "⚠️",
         "where": "Vite default builds .map · need build.sourcemap=false"},
        {"item": "Trace/correlation IDs allowed in F12", "status": "✅",
         "where": "Iter 43 trace_id surfaced"},
    ],
    "3_refresh_recovery": [
        {"item": "Workflow recovers from URL", "status": "⚠️",
         "where": "URL has run_id · render needs work"},
        {"item": "Current prompt restored", "status": "❌",
         "where": "NOT WIRED · no prompt draft DB"},
        {"item": "Uploaded files persist", "status": "❌",
         "where": "NOT WIRED · file_storage missing"},
        ...
        {"item": "Filter state from localStorage", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Last output rerendered", "status": "⚠️",
         "where": "ProductionPipeline shows · others vary"},
    ],
    "4_background": [
    ...
    "5_error_handling": [
        {"item": "API failure shows retry button", "status": "⚠️",
         "where": "some views · not standardized"},
        {"item": "Timeout shows progress", "status": "⚠️",
         "where": "needs review"},
        {"item": "Validation errors field-level", "status": "⚠️",
         "where": "Zod schemas exist · binding TBD"},
        {"item": "Agent failure shows status banner", "status": "✅",
         "where": "Live Activity color-codes status"},
        {"item": "File upload error · resume", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Network error · auto-reconnect", "status": "⚠️",
         "where": "fetch retry policy missing"},
        {"item": "Session expired · re-login modal", "status": "⚠️",
         "where": "401 thrown · modal TBD"},
        {"item": "Permission error · explanation", "status": "✅",
         "where": "rbac_middleware returns clear JSON"},
    ],
    "6_color_loading": [
        {"item": "Skeleton on initial render", "status": "❌",
         "where": "NOT WIRED · returns 'Loading…' text"},
        {"item": "Spinner during API call", "status": "⚠️",
         "where": "some tabs use it"},
        ...
        {"item": "Error boundary with explanation", "status": "⚠️",
         "where": "ErrorBoundary needs review"},
        {"item": "No blank white screen ever", "status": "⚠️",
         "where": "loading text · not skeleton"},
    ],
    "7_monitoring": [
        {"item": "LCP page-load metric", "status": "❌",
         "where": "NOT WIRED · web-vitals lib needed"},
        {"item": "Per-API call time", "status": "⚠️",
         "where": "backend has it · frontend doesn't track"},
        {"item": "Component error capture", "status": "❌",
         "where": "NOT WIRED · Sentry/OpenReplay needed"},
        {"item": "User click tracking", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Failed request tracking", "status": "⚠️",
         "where": "backend audit · no frontend telemetry"},
        {"item": "Refresh count signal", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Browser/device/resolution", "status": "❌",
         "where": "NOT WIRED · header capture missing"},
    ],
    "8_sync": [
        {"item": "WebSocket /ws/workflow/{id}", "status": "⚠️",
         "where": "Iter 34 /ws/activity · not per-workflow"},
        {"item": "SSE alternative available", "status": "❌",
         "where": "NOT WIRED"},
        {"item": "Polling every 2s+", "status": "✅",
         "where": "Iter 52 5s · acceptable"},
        {"item": "All 8 workflow states render", "status": "⚠️",
         "where": "Iter 41 + Iter 56 · standardize"},
        {"item": "Frontend never shows stale status", "status": "⚠️",
         "where": "polling makes 5s lag possible · acceptable"},
    ],
    "9_testing": [
        ...
        {"item": "Component tests (RTL)", "status": "⚠️",
         "where": "some tests exist · not all components"},
        {"item": "Integration tests", "status": "⚠️",
         "where": "Playwright e2e tests in /e2e"},
        {"item": "E2E tests (Playwright)", "status": "✅",
         "where": "frontend/e2e/* (Iter 22)"},
        {"item": "Browser compat", "status": "❌",
         "where": "NOT WIRED · Selenium Grid"},
        {"item": "Mobile tests", "status": "❌",
         "where": "NOT WIRED · Maestro"},
        {"item": "Accessibility (Axe)", "status": "⚠️",
         "where": "axe-core mentioned · not enforced"},
        {"item": "Performance (Lighthouse)", "status": "⚠️",
         "where": "available · no CI gate"},
        {"item": "Security (ZAP)", "status": "❌",
         "where": "NOT WIRED · ZAP scheduled"},
    ],
    "10_status_banner": [
        ...
        {"item": "Progress % visible", "status": "⚠️",
         "where": "implied by stage count · not %"},
    ...
    "11_audit": [
        {"item": "Login event tracked", "status": "⚠️",
         "where": "JWT issued · audit_chain logs"},
        ...
        {"item": "Export tracked", "status": "⚠️",
         "where": "outbound_audit captures · UI rarely fires"},
        {"item": "Download tracked", "status": "⚠️",
         "where": "outbound_audit · not UI-bound"},
        {"item": "Error tracked", "status": "⚠️",
         "where": "console errors · not all to backend"},
    ],
    "12_forgotten_issues": [
        {"item": "Browser refresh loses workflow",       "status": "⚠️"},
        {"item": "Multi-tab state divergence",           "status": "❌"},
        {"item": "Stale cache after deploy",             "status": "⚠️"},
        {"item": "Old JS bundle requested",              "status": "⚠️"},
        {"item": "Wrong state after retry",              "status": "⚠️"},
        {"item": "File upload interrupted",              "status": "❌"},
        {"item": "WebSocket disconnect not recovered",   "status": "⚠️"},
        {"item": "Large table freeze (no virtualize)",   "status": "❌"},
        {"item": "Infinite spinner (no timeout)",        "status": "⚠️"},
        {"item": "White screen of death",                "status": "⚠️"},
        {"item": "Memory leak in browser",               "status": "❌"},
        {"item": "Session timeout no warning",           "status": "❌"},
        {"item": "Slow rendering on big lists",          "status": "⚠️"},
        {"item": "Duplicate submit button",              "status": "⚠️"},
        {"item": "User double-click race",               "status": "⚠️"},
    ],
}
