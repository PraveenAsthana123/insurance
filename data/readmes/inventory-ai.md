# Inventory AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `inventory-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for inventory ai tasks in
...
**FR-1**: System MUST accept input matching `inventory-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/inventory-ai/predict
        │
        ▼
...
- `predictions_inventory-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/inventory-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/inventory-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/inventory-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/inventory-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `inventory-ai_embeddings`
...
**Fairness metrics** (from data/fairness/inventory-ai.json):
