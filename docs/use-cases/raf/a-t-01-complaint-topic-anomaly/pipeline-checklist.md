# Pipeline Checklist · complaint-topic-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `complaint_topic_anomaly_raw` |
| Cleaned | Postgres | `complaint_topic_anomaly_clean` |
| Features | Feast / Postgres | `complaint_topic_anomaly_features` |
| Predictions | Postgres | `complaint_topic_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/complaint-topic-anomaly/` |
...
1. Input → `complaint_topic_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `complaint_topic_anomaly_predictions`
4. Log → ELK / Loki
