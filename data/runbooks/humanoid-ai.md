...
WHAT broke?      Check `/data/metrics/humanoid-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/humanoid-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/humanoid-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/humanoid-ai/model.joblib.backup models/humanoid-ai/model.joblib`
