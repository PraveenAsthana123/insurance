...
WHAT broke?      Check `/data/metrics/sales-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/sales-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/sales-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/sales-ai/model.joblib.backup models/sales-ai/model.joblib`
