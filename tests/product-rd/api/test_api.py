"""Tier 3 — API tests for product-rd.
...
def test_product_rd_list_returns_200():
    ...
    body = r.json()
    assert "product-rd" in body.get("departments", []), f"product-rd not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_product_rd_invalid_input_returns_400():
