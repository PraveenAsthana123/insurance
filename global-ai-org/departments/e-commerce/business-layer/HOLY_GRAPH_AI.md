# HOLY Beverage — E Commerce — Graph AI
...
> Anchor flow: **customer ─ adds to cart ─ scored by abandonment_model**.
...
```
customer ─ adds to cart ─ scored by abandonment_model
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in e-commerce touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/e-commerce` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/e-commerce/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/e-commerce/neighbors/<node_id>` | 1-hop neighbors of a node |
