...
graph LR
    ...
    features --> model[NLP, RPA, Churn / Retention ML, Fairness AI, Recommendation AI]
...
- Decision lineage: every API call writes a row to `hr_audit` (see LLD §2)

## L6 — Observability

- Logs: structured JSON with `correlation_id`, `tenant_id`, `user_id`, `model_version`, `prompt_version`
- Metrics: `hr_request_count`, `hr_latency_p95`, `hr_model_confidence`, `hr_hallucination_rate`
