# Video Analytics AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `video-analytics-ai` · **Kind**: `cv_pretrained`
...
**Business goal**: Reduce manual effort + improve accuracy for video analytics ai tasks in
...
**FR-1**: System MUST accept input matching `video-analytics-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/video-analytics-ai/predict
        │
        ▼
...
- `predictions_video-analytics-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/video-analytics-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/video-analytics-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/video-analytics-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/video-analytics-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `video-analytics-ai_embeddings`
...
**Fairness metrics** (from data/fairness/video-analytics-ai.json):
