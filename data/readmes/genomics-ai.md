# Genomics AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `genomics-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for genomics ai tasks in
...
**FR-1**: System MUST accept input matching `genomics-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/genomics-ai/predict
        │
        ▼
...
- `predictions_genomics-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/genomics-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/genomics-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/genomics-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/genomics-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `genomics-ai_embeddings`
...
**Fairness metrics** (from data/fairness/genomics-ai.json):
