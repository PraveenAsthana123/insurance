...
WHAT broke?      Check `/data/metrics/legal-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/legal-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/legal-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/legal-ai/model.joblib.backup models/legal-ai/model.joblib`
