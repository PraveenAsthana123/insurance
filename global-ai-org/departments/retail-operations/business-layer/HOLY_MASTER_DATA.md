# HOLY Beverage — Retail Operations — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **store-master + planogram + assortment + shrinkage-codes**.
...
This dept (**retail-operations**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/retail-operations` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/retail-operations/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/retail-operations/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/retail-operations/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/retail-operations/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/retail-operations` returns 200 + entity catalog
