# HOLY Beverage — Hr — Automated Pipelines
...
|---|---|---|---|
| 1 | Attrition Prediction | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Attrition Prediction + Resume Parsing + Skills Gap Analysis**._
...
**Audit prefix:** `pipeline.hr.attrition_prediction`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream hr systems | `outcome=data_received` |
...
| 5 Report | Hr manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/hr` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/hr/<process_id>` | Full 5-phase spec for one process |
