# Recommender-Text-2 · regulation-recommender
...
Dept 12 Compliance · DPR + cross-encoder
...
|---|---|---|---|
| `INSUR-REGULATION_RECOMMENDER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-REGULATION_RECOMMENDER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-REGULATION_RECOMMENDER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-REGULATION_RECOMMENDER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-REGULATION_RECOMMENDER-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-REGULATION_RECOMMENDER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
