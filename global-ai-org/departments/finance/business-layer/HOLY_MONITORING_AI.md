# HOLY Beverage — Finance — Monitoring AI
...
> Persona: **Finance Manager**. Focus: **fraud-detection precision + cash-flow forecast drift**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/finance` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/finance/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/finance/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #finance-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #finance-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/finance` returns 200 + job list
