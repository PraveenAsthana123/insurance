# Recommender-Numerical-8 · hitl-adjudicator-rec
...
Dept 7 Claims · skill collab + load bandit
...
|---|---|---|---|
| `INSUR-HITL_ADJUDICATOR_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-HITL_ADJUDICATOR_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-HITL_ADJUDICATOR_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-HITL_ADJUDICATOR_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-HITL_ADJUDICATOR_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-HITL_ADJUDICATOR_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
