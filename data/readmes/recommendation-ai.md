# Recommendation AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `recommendation-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for recommendation ai tasks in
...
**FR-1**: System MUST accept input matching `recommendation-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/recommendation-ai/predict
        │
        ▼
...
- `predictions_recommendation-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/recommendation-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/recommendation-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/recommendation-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/recommendation-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `recommendation-ai_embeddings`
...
**Fairness metrics** (from data/fairness/recommendation-ai.json):
