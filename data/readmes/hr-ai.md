# HR AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `hr-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for hr ai tasks in
...
**FR-1**: System MUST accept input matching `hr-ai` schema
...
- `predictions_hr-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/hr-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/hr-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/hr-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/hr-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/hr-ai.json):
