# HOLY Beverage — Manufacturing — Graph AI
...
> Anchor flow: **material ─ assembled into product ─ inspected by CV model**.
...
```
material ─ assembled into product ─ inspected by CV model
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in manufacturing touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/manufacturing` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/manufacturing/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/manufacturing/neighbors/<node_id>` | 1-hop neighbors of a node |
