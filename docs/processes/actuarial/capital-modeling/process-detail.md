...
> Department 10 · Actuarial · slug `capital-modeling` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/capital-modeling/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/10/<domain>/capital-modeling?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/10/<domain>/capital-modeling?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/capital-modeling.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/actuarial/capital-modeling/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-CAPITAL_MODELING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-CAPITAL_MODELING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-CAPITAL_MODELING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-CAPITAL_MODELING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-CAPITAL_MODELING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-CAPITAL_MODELING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-CAPITAL_MODELING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/actuarial/capital-modeling/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-CAPITAL_MODELING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/actuarial/capital-modeling/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/actuarial/capital-modeling/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/actuarial/capital-modeling/stats/`
