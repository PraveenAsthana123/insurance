# Anomaly-Numerical-10 · sensor-telemetry-anomaly
...
|---|---|---|---|
| `INSUR-SENSOR_TELEMETRY_ANOMALY-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SENSOR_TELEMETRY_ANOMALY-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SENSOR_TELEMETRY_ANOMALY-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SENSOR_TELEMETRY_ANOMALY-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SENSOR_TELEMETRY_ANOMALY-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SENSOR_TELEMETRY_ANOMALY-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
