# Neuro-Symbolic AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `neuro-symbolic-ai` · **Kind**: `agent_engine`
**Accuracy**: 0.9367 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for neuro-symbolic ai tasks in
...
**FR-1**: System MUST accept input matching `neuro-symbolic-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/neuro-symbolic-ai/predict
        │
        ▼
...
- `predictions_neuro-symbolic-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/neuro-symbolic-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/neuro-symbolic-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/neuro-symbolic-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/neuro-symbolic-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `neuro-symbolic-ai_embeddings`
...
**Fairness metrics** (from data/fairness/neuro-symbolic-ai.json):
