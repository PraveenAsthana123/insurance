# Diagnostic AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `diagnostic-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for diagnostic ai tasks in
...
**FR-1**: System MUST accept input matching `diagnostic-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/diagnostic-ai/predict
        │
        ▼
...
- `predictions_diagnostic-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/diagnostic-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/diagnostic-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/diagnostic-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/diagnostic-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `diagnostic-ai_embeddings`
...
**Fairness metrics** (from data/fairness/diagnostic-ai.json):
