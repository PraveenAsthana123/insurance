# Pipeline Checklist · damage-photo-authenticity
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `damage_photo_authenticity_raw` |
| Cleaned | Postgres | `damage_photo_authenticity_clean` |
| Features | Feast / Postgres | `damage_photo_authenticity_features` |
| Predictions | Postgres | `damage_photo_authenticity_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/damage-photo-authenticity/` |
...
1. Input → `damage_photo_authenticity_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `damage_photo_authenticity_predictions`
4. Log → ELK / Loki
