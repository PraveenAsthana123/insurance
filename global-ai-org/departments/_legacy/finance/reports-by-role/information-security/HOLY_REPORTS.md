# HOLY Beverage — Finance — Information Security Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **information-security** in **finance** uses.
...
- **Portal**: every report retrievable at `/holy/finance/reports?role=information-security&id=<report_id>`
...
- `request_id`, `actor_role=information-security`, `dept=finance`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/finance/information-security
  → list of standard reports

POST /api/v1/holy/reports/finance/information-security/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/finance/information-security/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/finance/reports?role=information-security`
