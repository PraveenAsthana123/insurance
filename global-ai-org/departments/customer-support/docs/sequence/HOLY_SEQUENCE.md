# HOLY Beverage — Customer Support — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/customer-support
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **RAG-Backed Resolution**

_Question → intent → retrieve runbook chunks → LLM answer + cite_
...
When a user types a goal in `/holy/customer-support/agentic`:
