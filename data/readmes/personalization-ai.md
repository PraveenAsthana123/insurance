# Personalization AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `personalization-ai` · **Kind**: `spec_only`
**Accuracy**: 0.96 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for personalization ai tasks in
...
**FR-1**: System MUST accept input matching `personalization-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/personalization-ai/predict
        │
        ▼
...
- `predictions_personalization-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/personalization-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/personalization-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/personalization-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/personalization-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `personalization-ai_embeddings`
...
**Fairness metrics** (from data/fairness/personalization-ai.json):
