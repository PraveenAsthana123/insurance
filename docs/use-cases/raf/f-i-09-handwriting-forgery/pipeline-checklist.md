# Pipeline Checklist · handwriting-forgery
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `handwriting_forgery_raw` |
| Cleaned | Postgres | `handwriting_forgery_clean` |
| Features | Feast / Postgres | `handwriting_forgery_features` |
| Predictions | Postgres | `handwriting_forgery_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/handwriting-forgery/` |
...
1. Input → `handwriting_forgery_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `handwriting_forgery_predictions`
4. Log → ELK / Loki
