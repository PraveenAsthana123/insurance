...
any 3-5 to assemble a Legal board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Legal
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is Admin (System)'s view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: Dept Manager in Legal
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is Dept Manager's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: Team Member in Legal
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is Team Member's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Legal
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is QA / Tester's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: SecOps Engineer in Legal
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is SecOps Engineer's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Legal
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is DevOps's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: AI Reviewer in Legal
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is AI Reviewer's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: DT Lead in Legal
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is DT Lead's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: System Architect in Legal
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is System Architect's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: Test Architect in Legal
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is Test Architect's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: DB Architect in Legal
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is DB Architect's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: API Architect in Legal
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is API Architect's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: Data Owner in Legal
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is Data Owner's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: AI Strategy Lead in Legal
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is AI Strategy Lead's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
- **Persona**: InfoSec / CISO Office in Legal
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Legal dashboards
- **KPI moved**: Contract review cycle + risk-flag precision
...
- **Talking points**: "This is InfoSec / CISO Office's view of Legal. In 90 seconds we'll move Contract review cycle + risk-flag precision by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/legal` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/legal` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/legal/<role>` | Single role demo detail |
