...
WHAT broke?      Check `/data/metrics/finance-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/finance-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/finance-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/finance-ai/model.joblib.backup models/finance-ai/model.joblib`
