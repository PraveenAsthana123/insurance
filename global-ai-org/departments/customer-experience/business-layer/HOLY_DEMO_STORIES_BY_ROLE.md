...
any 3-5 to assemble a Customer Experience board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Customer Experience
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is Admin (System)'s view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: Dept Manager in Customer Experience
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is Dept Manager's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: Team Member in Customer Experience
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is Team Member's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Customer Experience
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is QA / Tester's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: SecOps Engineer in Customer Experience
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is SecOps Engineer's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Customer Experience
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is DevOps's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: AI Reviewer in Customer Experience
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is AI Reviewer's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: DT Lead in Customer Experience
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is DT Lead's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: System Architect in Customer Experience
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is System Architect's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: Test Architect in Customer Experience
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is Test Architect's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: DB Architect in Customer Experience
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is DB Architect's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: API Architect in Customer Experience
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is API Architect's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: Data Owner in Customer Experience
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is Data Owner's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: AI Strategy Lead in Customer Experience
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is AI Strategy Lead's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
- **Persona**: InfoSec / CISO Office in Customer Experience
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Customer Experience dashboards
- **KPI moved**: CSAT + ticket deflection
...
- **Talking points**: "This is InfoSec / CISO Office's view of Customer Experience. In 90 seconds we'll move CSAT + ticket deflection by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-experience` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/customer-experience` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/customer-experience/<role>` | Single role demo detail |
