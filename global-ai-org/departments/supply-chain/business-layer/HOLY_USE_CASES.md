# HOLY Beverage — Supply Chain — AI Use-Case Catalog
...
> use cases for **supply-chain**. Implementation status tracked per current
> codebase state.
...
|---|---|---|
| ✅ Implemented | **15** of 32 | Working module exists in `backend/ml/reference/*.py` (drill-locked) |
| 🟡 Partial | **11** of 32 | Existing module applies; dept-specific stub deferred |
...
## 2. Supply Chain-Specific Use Cases

| Use Case | Category | Data Types | AI Area | Business Value | Status |
|---|---|---|---|---|---|
| Demand Forecasting | Supply Chain | sales time-series | Forecasting | Lower inventory cost | ✅ timeseries_lifecycle.py (Rossmann smoke) |
| Inventory Optimization | Supply Chain | inventory data | OR + Forecasting | Higher fill rate | 🟡 timeseries_lifecycle.py covers forecast; OR layer deferred |
| Route Optimization (VRP) | Logistics | GPS/orders | OR-tools | Less fuel | ❌ deferred (OR-tools) |
| Supply Chain Risk AI | Risk AI | vendor/geo | Risk AI | Less disruption | 🟡 simulation_engine + anomaly_lifecycle apply |
| Shipment Delay Prediction | Logistics | tracking events | Forecasting | Better ETAs | 🟡 timeseries_lifecycle.py applies |
