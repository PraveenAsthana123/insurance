# HOLY Beverage — Marketing — Monitoring AI
...
> Persona: **Marketing Lead**. Focus: **channel-attribution + content-recommender drift**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/marketing` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/marketing/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/marketing/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #marketing-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #marketing-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/marketing` returns 200 + job list
