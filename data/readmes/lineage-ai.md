# Lineage AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `lineage-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for lineage ai tasks in
...
**FR-1**: System MUST accept input matching `lineage-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/lineage-ai/predict
        │
        ▼
...
- `predictions_lineage-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/lineage-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/lineage-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/lineage-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/lineage-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `lineage-ai_embeddings`
...
**Fairness metrics** (from data/fairness/lineage-ai.json):
