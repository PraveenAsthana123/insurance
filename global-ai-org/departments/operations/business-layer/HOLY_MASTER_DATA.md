# HOLY Beverage — Operations — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **process-catalog + RACI-matrix + cost-center**.
...
This dept (**operations**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/operations` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/operations/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/operations/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/operations/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/operations/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/operations` returns 200 + entity catalog
