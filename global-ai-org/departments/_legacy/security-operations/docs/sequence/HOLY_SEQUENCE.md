# HOLY Beverage — Security Operations — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/security-operations
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Threat Classifier on Alert**

_SIEM alert → ML classifier → enrich w/ threat-intel → ticket_
...
When a user types a goal in `/holy/security-operations/agentic`:
