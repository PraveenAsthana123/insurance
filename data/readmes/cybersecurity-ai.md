# Cybersecurity AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `cybersecurity-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for cybersecurity ai tasks in
...
**FR-1**: System MUST accept input matching `cybersecurity-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/cybersecurity-ai/predict
        │
        ▼
...
- `predictions_cybersecurity-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/cybersecurity-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/cybersecurity-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/cybersecurity-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/cybersecurity-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `cybersecurity-ai_embeddings`
...
**Fairness metrics** (from data/fairness/cybersecurity-ai.json):
