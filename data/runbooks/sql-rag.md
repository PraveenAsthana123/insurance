...
WHAT broke?      Check `/data/metrics/sql-rag.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/sql-rag.json`
WHO touched?     `git log --since=<date> data/ai_types/sql-rag.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/sql-rag/model.joblib.backup models/sql-rag/model.joblib`
