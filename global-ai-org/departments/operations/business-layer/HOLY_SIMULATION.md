# HOLY Beverage — Operations — Simulation Tab
...
under `REFERENCE_PROCESSES[("operations", "<process>")]`.
...
| Endpoint | Purpose |
|---|---|
| POST /api/v1/holy/sim/operations/{process}/run | Trigger simulation |
| GET /api/v1/holy/sim/operations/{process}/runs | List past runs |
| GET /api/v1/holy/sim/operations/{process}/runs/{sim_id}/manifest | Per-run summary |
| GET /api/v1/holy/sim/operations/{process}/runs/{sim_id}/events | Event stream |

## TODO

- [ ] Define ≥ 1 process for operations in `REFERENCE_PROCESSES` dict
