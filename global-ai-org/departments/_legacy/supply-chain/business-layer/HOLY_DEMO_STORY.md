# HOLY Beverage — Supply Chain — Demo User Story
...
**Linda, Head of Supply Chain Planning**
...
The persona needs to: **cut stock-outs by 60% and reduce safety stock holding cost 30%**

Main KPI being moved: **Stock-out rate, inventory turns, on-time-in-full (OTIF)**

## 3. Walkthrough (what they see + do)

1. Persona lands on `/holy/supply-chain` (the HOLY navigation page for this dept).
...
> "Today Linda spends 80% of their week on manual data
...
> counterfactual. We don't replace Linda — we replace
...
|---|---|---|---|
| 1 | Open `/holy/supply-chain` | Left sidebar with N processes | "Every process this dept owns, surfaced as navigable artifacts" |
...
**Cross-reference**: This story powers the demo at `/holy/supply-chain` in the
HOLY frontend. The literal click-by-click in §5 is also the drill
`tests/drills/drill_dept_demo_supply_chain.py` (planned).
