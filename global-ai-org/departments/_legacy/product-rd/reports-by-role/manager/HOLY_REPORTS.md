# HOLY Beverage — Product Rd — Manager Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **manager** in **product-rd** uses.
...
- **Portal**: every report retrievable at `/holy/product-rd/reports?role=manager&id=<report_id>`
...
- `request_id`, `actor_role=manager`, `dept=product-rd`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/product-rd/manager
  → list of standard reports

POST /api/v1/holy/reports/product-rd/manager/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/product-rd/manager/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/product-rd/reports?role=manager`
