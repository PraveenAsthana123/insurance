# HOLY Beverage — Manufacturing — Monitoring AI
...
> Persona: **Plant Manager**. Focus: **OEE prediction + defect-detection CV pipeline health**.
...
Tab: `/holy/manufacturing/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/manufacturing` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/manufacturing/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/manufacturing/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #manufacturing-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #manufacturing-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/manufacturing` returns 200 + job list
