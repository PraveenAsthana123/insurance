# Fraud-Numerical-1 · premium-rate-evasion
...
Dept 8 SIU · XGB + adversarial validation
...
|---|---|---|---|
| `INSUR-PREMIUM_RATE_EVASION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PREMIUM_RATE_EVASION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PREMIUM_RATE_EVASION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PREMIUM_RATE_EVASION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PREMIUM_RATE_EVASION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PREMIUM_RATE_EVASION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
