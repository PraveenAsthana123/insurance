# Recommender-Numerical-2 · premium-tier-upgrade-rec
...
|---|---|---|---|
| `INSUR-PREMIUM_TIER_UPGRADE_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PREMIUM_TIER_UPGRADE_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PREMIUM_TIER_UPGRADE_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PREMIUM_TIER_UPGRADE_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PREMIUM_TIER_UPGRADE_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PREMIUM_TIER_UPGRADE_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
