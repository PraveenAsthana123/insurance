...
> Department 6 · Billing & Collections · slug `refund-processing` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/refund-processing/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/6/<domain>/refund-processing?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/6/<domain>/refund-processing?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/refund-processing.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/billing-collections/refund-processing/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-REFUND_PROCESSING-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-REFUND_PROCESSING-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-REFUND_PROCESSING-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-REFUND_PROCESSING-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-REFUND_PROCESSING-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-REFUND_PROCESSING-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-REFUND_PROCESSING-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/billing-collections/refund-processing/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-REFUND_PROCESSING-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/billing-collections/refund-processing/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/billing-collections/refund-processing/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/billing-collections/refund-processing/stats/`
