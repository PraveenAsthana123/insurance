# HOLY Beverage — Executive Leadership — Manager Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **manager** in **executive-leadership**.

## Persona

The **manager** at Executive Leadership. They use this dashboard
...
Dept KPIs + ROI + approval queue + team performance
...
- KPI trend (12 weeks)
- Approval-queue funnel
- Team performance ranking
- Cost waterfall

Library dispatch per chart (recharts / plotly / echarts / d3 / vega-lite)
returned by backend `GET /api/v1/holy/dashboards/executive-leadership/manager/charts/<chart_id>`.
...
Inline actions the **manager** can take from this dashboard:
...
```
GET /api/v1/holy/dashboards/executive-leadership/manager
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/executive-leadership/manager/charts/<chart_id>
  → {library, type, data, baseline, threshold, drill_url, refreshed_at}
```

## Related Artifacts

- `../../reports-by-role/manager/HOLY_REPORTS.md` — paired standard reports
...
- Frontend route: `/holy/executive-leadership/dashboard?role=manager`
