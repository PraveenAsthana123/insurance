# Recommender-Numerical-1 · next-best-product-cross-sell
...
```mermaid
graph LR
    Input["Numerical Input"] --> Preprocess[G1-G5 preprocessing]
...
|---|---|---|---|
| `INSUR-NEXT_BEST_PRODUCT_CROSS_SELL-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-NEXT_BEST_PRODUCT_CROSS_SELL-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-NEXT_BEST_PRODUCT_CROSS_SELL-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-NEXT_BEST_PRODUCT_CROSS_SELL-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-NEXT_BEST_PRODUCT_CROSS_SELL-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-NEXT_BEST_PRODUCT_CROSS_SELL-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
