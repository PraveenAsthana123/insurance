# Pipeline Checklist · damage-photo-manipulation
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `damage_photo_manipulation_raw` |
| Cleaned | Postgres | `damage_photo_manipulation_clean` |
| Features | Feast / Postgres | `damage_photo_manipulation_features` |
| Predictions | Postgres | `damage_photo_manipulation_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/damage-photo-manipulation/` |
...
1. Input → `damage_photo_manipulation_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `damage_photo_manipulation_predictions`
4. Log → ELK / Loki
