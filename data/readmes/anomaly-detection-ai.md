# Anomaly Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `anomaly-detection-ai` · **Kind**: `tabular_ml`
**Accuracy**: 0.963 · **Algorithm**: GradientBoostingClassifier

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for anomaly detection ai tasks in
...
**FR-1**: System MUST accept input matching `anomaly-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/anomaly-detection-ai/predict
        │
        ▼
...
- `predictions_anomaly-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/anomaly-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/anomaly-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/anomaly-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/anomaly-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `anomaly-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/anomaly-detection-ai.json):
