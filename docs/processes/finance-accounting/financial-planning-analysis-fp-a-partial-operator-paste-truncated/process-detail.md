# Financial Planning & Analysis (FP&A) [partial — operator paste truncated]

> Department 14 · Finance & Accounting · slug `financial-planning-analysis-fp-a-partial-operator-paste-truncated` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/financial-planning-analysis-fp-a-partial-operator-paste-truncated/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/14/<domain>/financial-planning-analysis-fp-a-partial-operator-paste-truncated?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/14/<domain>/financial-planning-analysis-fp-a-partial-operator-paste-truncated?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/finance-accounting/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/financial-planning-analysis-fp-a-partial-operator-paste-truncated.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/finance-accounting/financial-planning-analysis-fp-a-partial-operator-paste-truncated/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/finance-accounting/financial-planning-analysis-fp-a-partial-operator-paste-truncated/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-FINANCIAL_PLANNING_ANALYSIS_FP_A_PARTIAL_OPERATOR_PASTE_TRUNCATED-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/finance-accounting/financial-planning-analysis-fp-a-partial-operator-paste-truncated/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/finance-accounting/financial-planning-analysis-fp-a-partial-operator-paste-truncated/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/finance-accounting/financial-planning-analysis-fp-a-partial-operator-paste-truncated/stats/`
