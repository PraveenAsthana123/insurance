# HOLY Beverage — Customer Experience — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/customer-experience
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Ticket Triage + Auto-Reply**

_Inbound msg → intent classifier → KB-RAG → autoreply or human-route_
...
When a user types a goal in `/holy/customer-experience/agentic`:
