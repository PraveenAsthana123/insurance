...
any 3-5 to assemble a Customer Support board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Customer Support
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is Admin (System)'s view of Customer Support. In 90 seconds we'll move FCR + AHT by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: Dept Manager in Customer Support
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is Dept Manager's view of Customer Support. In 90 seconds we'll move FCR + AHT by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: Team Member in Customer Support
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is Team Member's view of Customer Support. In 90 seconds we'll move FCR + AHT by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Customer Support
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is QA / Tester's view of Customer Support. In 90 seconds we'll move FCR + AHT by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: SecOps Engineer in Customer Support
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is SecOps Engineer's view of Customer Support. In 90 seconds we'll move FCR + AHT by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Customer Support
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is DevOps's view of Customer Support. In 90 seconds we'll move FCR + AHT by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: AI Reviewer in Customer Support
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is AI Reviewer's view of Customer Support. In 90 seconds we'll move FCR + AHT by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: DT Lead in Customer Support
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is DT Lead's view of Customer Support. In 90 seconds we'll move FCR + AHT by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: System Architect in Customer Support
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is System Architect's view of Customer Support. In 90 seconds we'll move FCR + AHT by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: Test Architect in Customer Support
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is Test Architect's view of Customer Support. In 90 seconds we'll move FCR + AHT by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: DB Architect in Customer Support
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is DB Architect's view of Customer Support. In 90 seconds we'll move FCR + AHT by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: API Architect in Customer Support
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is API Architect's view of Customer Support. In 90 seconds we'll move FCR + AHT by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: Data Owner in Customer Support
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is Data Owner's view of Customer Support. In 90 seconds we'll move FCR + AHT by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: AI Strategy Lead in Customer Support
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is AI Strategy Lead's view of Customer Support. In 90 seconds we'll move FCR + AHT by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
- **Persona**: InfoSec / CISO Office in Customer Support
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Customer Support dashboards
- **KPI moved**: FCR + AHT
...
- **Talking points**: "This is InfoSec / CISO Office's view of Customer Support. In 90 seconds we'll move FCR + AHT by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/customer-support` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/customer-support` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/customer-support/<role>` | Single role demo detail |
