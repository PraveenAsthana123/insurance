...
WHAT broke?      Check `/data/metrics/metadata-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/metadata-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/metadata-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/metadata-ai/model.joblib.backup models/metadata-ai/model.joblib`
