# HOLY Beverage — Procurement — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **vendor-master + material-group + contract + payment-terms**.
...
This dept (**procurement**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/procurement` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/procurement/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/procurement/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/procurement/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/procurement/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/procurement` returns 200 + entity catalog
