# RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `rag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for rag tasks in
...
**FR-1**: System MUST accept input matching `rag` schema
...
- `predictions_rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/rag/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/rag.json):
