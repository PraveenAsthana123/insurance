# Pipeline Checklist · prd-clause-recommender
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `prd_clause_recommender_raw` |
| Cleaned | Postgres | `prd_clause_recommender_clean` |
| Features | Feast / Postgres | `prd_clause_recommender_features` |
| Predictions | Postgres | `prd_clause_recommender_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/prd-clause-recommender/` |
...
1. Input → `prd_clause_recommender_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `prd_clause_recommender_predictions`
4. Log → ELK / Loki
