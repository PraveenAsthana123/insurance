# Recommender-Numerical-6 · reserve-tier-recommendation
...
Dept 10 Actuarial · Bayesian rec
...
|---|---|---|---|
| `INSUR-RESERVE_TIER_RECOMMENDATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RESERVE_TIER_RECOMMENDATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RESERVE_TIER_RECOMMENDATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RESERVE_TIER_RECOMMENDATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-RESERVE_TIER_RECOMMENDATION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-RESERVE_TIER_RECOMMENDATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
