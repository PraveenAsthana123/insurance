# Synthetic Data AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `synthetic-data-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for synthetic data ai tasks in
...
**FR-1**: System MUST accept input matching `synthetic-data-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/synthetic-data-ai/predict
        │
        ▼
...
- `predictions_synthetic-data-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/synthetic-data-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/synthetic-data-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/synthetic-data-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/synthetic-data-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `synthetic-data-ai_embeddings`
...
**Fairness metrics** (from data/fairness/synthetic-data-ai.json):
