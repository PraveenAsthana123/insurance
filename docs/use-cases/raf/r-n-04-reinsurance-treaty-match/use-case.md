# Recommender-Numerical-4 · reinsurance-treaty-match
...
Dept 11 Reinsurance · NSGA-II + content
...
|---|---|---|---|
| `INSUR-REINSURANCE_TREATY_MATCH-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-REINSURANCE_TREATY_MATCH-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-REINSURANCE_TREATY_MATCH-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-REINSURANCE_TREATY_MATCH-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-REINSURANCE_TREATY_MATCH-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-REINSURANCE_TREATY_MATCH-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
