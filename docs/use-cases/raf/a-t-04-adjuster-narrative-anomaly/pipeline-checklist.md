# Pipeline Checklist · adjuster-narrative-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `adjuster_narrative_anomaly_raw` |
| Cleaned | Postgres | `adjuster_narrative_anomaly_clean` |
| Features | Feast / Postgres | `adjuster_narrative_anomaly_features` |
| Predictions | Postgres | `adjuster_narrative_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/adjuster-narrative-anomaly/` |
...
1. Input → `adjuster_narrative_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `adjuster_narrative_anomaly_predictions`
4. Log → ELK / Loki
