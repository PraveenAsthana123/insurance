...
> Department 18 · Data, Analytics & Enterprise Intelligence · slug `data-compliance` · AI items: 3
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/data-compliance/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/18/<domain>/data-compliance?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/18/<domain>/data-compliance?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/data-compliance.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/data-analytics-enterprise-intelligence/data-compliance/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-DATA_COMPLIANCE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-DATA_COMPLIANCE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-DATA_COMPLIANCE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-DATA_COMPLIANCE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-DATA_COMPLIANCE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-DATA_COMPLIANCE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-DATA_COMPLIANCE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/data-analytics-enterprise-intelligence/data-compliance/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-DATA_COMPLIANCE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/data-analytics-enterprise-intelligence/data-compliance/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/data-analytics-enterprise-intelligence/data-compliance/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/data-analytics-enterprise-intelligence/data-compliance/stats/`
