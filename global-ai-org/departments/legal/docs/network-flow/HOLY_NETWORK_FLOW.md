# HOLY Beverage — Legal Network Flow
...
```
Legal API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Legal API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          legal_user_events       analytics-svc
Backend API       ──►          legal_audit              audit-store
ML Worker         ──►          legal_predictions        prediction-store
Drift detector    ──►          legal_alerts             on-call PagerDuty
...
|---|---|---|---|
| legal-api | postgresql | 5432 | TCP |
| legal-api | redis | 6379 | TCP |
| legal-api | ollama | 11434 | TCP |
| legal-api | otel-collector | 4317 | gRPC |
| legal-api | (other dept APIs) | 8000 | HTTP via service mesh |
