# SLM · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `slm` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for slm tasks in
...
**FR-1**: System MUST accept input matching `slm` schema
...
- `predictions_slm`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/slm/predict` · body: features → prediction
- `GET /api/v1/ai-types/slm/metrics` → live metrics.json
- `GET /api/v1/ai-types/slm/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/slm/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/slm.json):
