# Pipeline Checklist · uw-decision-drift
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `uw_decision_drift_raw` |
| Cleaned | Postgres | `uw_decision_drift_clean` |
| Features | Feast / Postgres | `uw_decision_drift_features` |
| Predictions | Postgres | `uw_decision_drift_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/uw-decision-drift/` |
...
1. Input → `uw_decision_drift_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `uw_decision_drift_predictions`
4. Log → ELK / Loki
