# Pipeline Checklist · email-content-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `email_content_anomaly_raw` |
| Cleaned | Postgres | `email_content_anomaly_clean` |
| Features | Feast / Postgres | `email_content_anomaly_features` |
| Predictions | Postgres | `email_content_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/email-content-anomaly/` |
...
1. Input → `email_content_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `email_content_anomaly_predictions`
4. Log → ELK / Loki
