...
WHAT broke?      Check `/data/metrics/rlhf.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/rlhf.json`
WHO touched?     `git log --since=<date> data/ai_types/rlhf.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/rlhf/model.joblib.backup models/rlhf/model.joblib`
