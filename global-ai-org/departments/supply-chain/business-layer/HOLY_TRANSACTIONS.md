# HOLY Beverage — Supply Chain — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Forecast re-runs + reorder recommendations + supplier scorecards**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/supply-chain` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/supply-chain?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/supply-chain?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/supply-chain?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/supply-chain/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/supply-chain` returns 200 + chronological feed
