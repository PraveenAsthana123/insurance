...
WHAT broke?      Check `/data/metrics/hpc-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/hpc-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/hpc-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/hpc-ai/model.joblib.backup models/hpc-ai/model.joblib`
