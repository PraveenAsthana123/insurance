...
This dept's `/holy/fraud-siu/simulation` tab specification. Manual-mode + Automatic-mode
side-by-side with 5 layers visible per run.

## Tab path
`/holy/fraud-siu/simulation?process=<L2-process-name>`
...
| Endpoint | Purpose |
|---|---|
| `POST /api/v1/holy/sim/fraud-siu/{process}/run` | Trigger sim; body: `{mode, inputs, seed}` |
| `GET /api/v1/holy/sim/fraud-siu/{process}/runs/{sim_id}/events` | SSE stream for live waterfall |
| `GET /api/v1/holy/sim/fraud-siu/{process}/runs/{sim_id}/manifest` | Per-layer summary + comparison report |
| `GET /api/v1/holy/sim/fraud-siu/{process}/runs/{sim_id}/replay` | Frame-by-frame replay of past run |
