# Recommender-Text-7 · caselaw-precedent-rec
...
|---|---|---|---|
| `INSUR-CASELAW_PRECEDENT_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-CASELAW_PRECEDENT_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-CASELAW_PRECEDENT_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-CASELAW_PRECEDENT_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-CASELAW_PRECEDENT_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-CASELAW_PRECEDENT_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
