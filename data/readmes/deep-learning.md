# Deep Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `deep-learning` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for deep learning tasks in
...
**FR-1**: System MUST accept input matching `deep-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/deep-learning/predict
        │
        ▼
...
- `predictions_deep-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/deep-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/deep-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/deep-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/deep-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `deep-learning_embeddings`
...
**Fairness metrics** (from data/fairness/deep-learning.json):
