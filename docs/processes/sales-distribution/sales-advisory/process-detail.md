...
> Department 3 · Sales & Distribution · slug `sales-advisory` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/sales-advisory/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/3/<domain>/sales-advisory?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/3/<domain>/sales-advisory?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/sales-advisory.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/sales-distribution/sales-advisory/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-SALES_ADVISORY-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-SALES_ADVISORY-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-SALES_ADVISORY-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-SALES_ADVISORY-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-SALES_ADVISORY-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-SALES_ADVISORY-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-SALES_ADVISORY-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/sales-distribution/sales-advisory/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-SALES_ADVISORY-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/sales-distribution/sales-advisory/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/sales-distribution/sales-advisory/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/sales-distribution/sales-advisory/stats/`
