# Fraud-Text-2 · phishing-email-detect
...
Dept 20 Cyber · DistilBERT + URL features
...
|---|---|---|---|
| `INSUR-PHISHING_EMAIL_DETECT-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PHISHING_EMAIL_DETECT-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PHISHING_EMAIL_DETECT-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PHISHING_EMAIL_DETECT-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PHISHING_EMAIL_DETECT-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PHISHING_EMAIL_DETECT-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
