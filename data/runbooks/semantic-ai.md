...
WHAT broke?      Check `/data/metrics/semantic-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/semantic-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/semantic-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/semantic-ai/model.joblib.backup models/semantic-ai/model.joblib`
