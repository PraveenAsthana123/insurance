# Contract AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `contract-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for contract ai tasks in
...
**FR-1**: System MUST accept input matching `contract-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/contract-ai/predict
        │
        ▼
...
- `predictions_contract-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/contract-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/contract-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/contract-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/contract-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `contract-ai_embeddings`
...
**Fairness metrics** (from data/fairness/contract-ai.json):
