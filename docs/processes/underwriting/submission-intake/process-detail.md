...
> Department 4 · Underwriting · slug `submission-intake` · AI items: 5
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/submission-intake/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/4/<domain>/submission-intake?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/4/<domain>/submission-intake?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/submission-intake.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/underwriting/submission-intake/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-SUBMISSION_INTAKE-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-SUBMISSION_INTAKE-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-SUBMISSION_INTAKE-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-SUBMISSION_INTAKE-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-SUBMISSION_INTAKE-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-SUBMISSION_INTAKE-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-SUBMISSION_INTAKE-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/underwriting/submission-intake/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-SUBMISSION_INTAKE-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/underwriting/submission-intake/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/underwriting/submission-intake/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/underwriting/submission-intake/stats/`
