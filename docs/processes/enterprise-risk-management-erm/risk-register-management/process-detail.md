...
> Department 15 · Enterprise Risk Management (ERM) · slug `risk-register-management` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/risk-register-management/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/15/<domain>/risk-register-management?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/15/<domain>/risk-register-management?tab=simulation`
...
|---|---|---|---|---|
| 1 | Input dataset | data | `data/insurance/enterprise-risk-management-erm/` | yes (file picker) |
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/risk-register-management.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/enterprise-risk-management-erm/risk-register-management/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-RISK_REGISTER_MANAGEMENT-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-RISK_REGISTER_MANAGEMENT-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-RISK_REGISTER_MANAGEMENT-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-RISK_REGISTER_MANAGEMENT-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-RISK_REGISTER_MANAGEMENT-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-RISK_REGISTER_MANAGEMENT-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-RISK_REGISTER_MANAGEMENT-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/enterprise-risk-management-erm/risk-register-management/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-RISK_REGISTER_MANAGEMENT-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/enterprise-risk-management-erm/risk-register-management/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/enterprise-risk-management-erm/risk-register-management/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/enterprise-risk-management-erm/risk-register-management/stats/`
