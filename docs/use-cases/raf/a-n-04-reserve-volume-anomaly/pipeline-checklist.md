# Pipeline Checklist · reserve-volume-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `reserve_volume_anomaly_raw` |
| Cleaned | Postgres | `reserve_volume_anomaly_clean` |
| Features | Feast / Postgres | `reserve_volume_anomaly_features` |
| Predictions | Postgres | `reserve_volume_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/reserve-volume-anomaly/` |
...
1. Input → `reserve_volume_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `reserve_volume_anomaly_predictions`
4. Log → ELK / Loki
