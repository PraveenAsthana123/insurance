# World Models · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `world-models` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for world models tasks in
...
**FR-1**: System MUST accept input matching `world-models` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/world-models/predict
        │
        ▼
...
- `predictions_world-models`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/world-models/predict` · body: features → prediction
- `GET /api/v1/ai-types/world-models/metrics` → live metrics.json
- `GET /api/v1/ai-types/world-models/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/world-models/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `world-models_embeddings`
...
**Fairness metrics** (from data/fairness/world-models.json):
