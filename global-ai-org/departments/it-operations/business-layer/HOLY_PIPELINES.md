# HOLY Beverage — It Operations — Automated Pipelines
...
|---|---|---|---|
| 1 | Log Anomaly | Tabular ML (default — extend per dept-specific lifecycle) | Dept manager + AI reviewer |

_Additional candidate pipelines (not yet detailed): **Log Anomaly + Incident Classifier + Auto-Remediation**._
...
**Audit prefix:** `pipeline.it_operations.log_anomaly`

| Phase | What | Audit-row outcome |
|---|---|---|
| 1 Input | Per-process inputs from upstream it-operations systems | `outcome=data_received` |
...
| 5 Report | It Operations manager dashboard per §64.37 | `outcome=report_published` + visible_to[] |
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/pipelines/it-operations` | Pipeline catalog above as JSON |
| `GET /api/v1/holy/pipelines/it-operations/<process_id>` | Full 5-phase spec for one process |
