...
WHAT broke?      Check `/data/metrics/spatial-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/spatial-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/spatial-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/spatial-ai/model.joblib.backup models/spatial-ai/model.joblib`
