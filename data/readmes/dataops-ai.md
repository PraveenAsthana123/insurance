# DataOps AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `dataops-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for dataops ai tasks in
...
**FR-1**: System MUST accept input matching `dataops-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/dataops-ai/predict
        │
        ▼
...
- `predictions_dataops-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/dataops-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/dataops-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/dataops-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/dataops-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `dataops-ai_embeddings`
...
**Fairness metrics** (from data/fairness/dataops-ai.json):
