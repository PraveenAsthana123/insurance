# Banking AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `banking-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for banking ai tasks in
...
**FR-1**: System MUST accept input matching `banking-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/banking-ai/predict
        │
        ▼
...
- `predictions_banking-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/banking-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/banking-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/banking-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/banking-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `banking-ai_embeddings`
...
**Fairness metrics** (from data/fairness/banking-ai.json):
