# LLM · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `llm` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for llm tasks in
...
**FR-1**: System MUST accept input matching `llm` schema
...
- `predictions_llm`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/llm/predict` · body: features → prediction
- `GET /api/v1/ai-types/llm/metrics` → live metrics.json
- `GET /api/v1/ai-types/llm/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/llm/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/llm.json):
