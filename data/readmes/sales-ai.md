# Sales AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `sales-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for sales ai tasks in
...
**FR-1**: System MUST accept input matching `sales-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/sales-ai/predict
        │
        ▼
...
- `predictions_sales-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/sales-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/sales-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/sales-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/sales-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `sales-ai_embeddings`
...
**Fairness metrics** (from data/fairness/sales-ai.json):
