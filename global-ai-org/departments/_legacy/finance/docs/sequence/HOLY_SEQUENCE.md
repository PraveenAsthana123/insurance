# HOLY Beverage — Finance — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/finance
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Fraud Detection on Txn**

_Txn arrives → rule engine → ML score → decision tier → approve/review/reject_
...
When a user types a goal in `/holy/finance/agentic`:
