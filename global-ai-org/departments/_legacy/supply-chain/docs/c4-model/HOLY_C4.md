# HOLY Beverage — Supply Chain C4 Model
...
graph TB
    ...
    holy[HOLY Supply Chain Platform]
...
graph LR
    ...
    features --> model[Forecasting ML, Optimization AI, IoT AI, Anomaly Detection]
...
- Decision lineage: every API call writes a row to `supply_chain_audit` (see LLD §2)

## L6 — Observability

- Logs: structured JSON with `correlation_id`, `tenant_id`, `user_id`, `model_version`, `prompt_version`
- Metrics: `supply-chain_request_count`, `supply-chain_latency_p95`, `supply-chain_model_confidence`, `supply-chain_hallucination_rate`
