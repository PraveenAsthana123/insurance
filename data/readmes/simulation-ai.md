# Simulation AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `simulation-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for simulation ai tasks in
...
**FR-1**: System MUST accept input matching `simulation-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/simulation-ai/predict
        │
        ▼
...
- `predictions_simulation-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/simulation-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/simulation-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/simulation-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/simulation-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `simulation-ai_embeddings`
...
**Fairness metrics** (from data/fairness/simulation-ai.json):
