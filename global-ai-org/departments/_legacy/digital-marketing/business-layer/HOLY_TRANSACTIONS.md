# HOLY Beverage — Digital Marketing — Transactional History

> Per operator 2026-05-23 — every dept maintains a unified chronological
> feed of every meaningful event. Focus: **Campaign attribution decisions + creative-rec scoring runs**.
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/transactions/digital-marketing` | Most recent N events across all sources, newest first |
| `GET /api/v1/holy/transactions/digital-marketing?source=cron` | Filtered to cron events only |
| `GET /api/v1/holy/transactions/digital-marketing?since_epoch=<ts>` | Events since a unix timestamp |
| `GET /api/v1/holy/transactions/digital-marketing?event_type=ml.*` | Wildcard event-type filter |
| `GET /api/v1/holy/transactions/digital-marketing/{event_id}` | Full single-event detail |
...
1. (+) `/api/v1/holy/transactions/digital-marketing` returns 200 + chronological feed
