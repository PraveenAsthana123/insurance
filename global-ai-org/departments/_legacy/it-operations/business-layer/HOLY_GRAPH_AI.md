# HOLY Beverage — It Operations — Graph AI
...
> Anchor flow: **log_event ─ classified by anomaly_model ─ opens incident**.
...
```
log_event ─ classified by anomaly_model ─ opens incident
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in it-operations touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/it-operations` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/it-operations/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/it-operations/neighbors/<node_id>` | 1-hop neighbors of a node |
