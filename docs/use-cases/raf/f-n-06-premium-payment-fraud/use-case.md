# Fraud-Numerical-6 · premium-payment-fraud
...
Dept 6 Billing · streaming GBM + IsoForest
...
|---|---|---|---|
| `INSUR-PREMIUM_PAYMENT_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PREMIUM_PAYMENT_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PREMIUM_PAYMENT_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PREMIUM_PAYMENT_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PREMIUM_PAYMENT_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PREMIUM_PAYMENT_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
