# HOLY Beverage — Hr — Sequence Flow
...
sequenceDiagram
    ...
    deactivate W
    D->>API: GET /api/v1/holy/monitoring/hr
    API->>FS: scan latest manifest
    FS-->>API: run summary
    API-->>D: status + age + readiness
```

## 3. Dept-primary AI flow — **Attrition Prediction Refresh**

_Employee snapshot → feature → predict → risk tier → manager dashboard_
...
When a user types a goal in `/holy/hr/agentic`:
