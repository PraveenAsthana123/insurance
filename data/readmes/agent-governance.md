# Agent Governance · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `agent-governance` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for agent governance tasks in
...
**FR-1**: System MUST accept input matching `agent-governance` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/agent-governance/predict
        │
        ▼
...
- `predictions_agent-governance`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/agent-governance/predict` · body: features → prediction
- `GET /api/v1/ai-types/agent-governance/metrics` → live metrics.json
- `GET /api/v1/ai-types/agent-governance/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/agent-governance/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `agent-governance_embeddings`
...
**Fairness metrics** (from data/fairness/agent-governance.json):
