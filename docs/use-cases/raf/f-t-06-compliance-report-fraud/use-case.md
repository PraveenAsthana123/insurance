# Fraud-Text-6 · compliance-report-fraud
...
Dept 12 Compliance · attention + RAG verify
...
|---|---|---|---|
| `INSUR-COMPLIANCE_REPORT_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-COMPLIANCE_REPORT_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-COMPLIANCE_REPORT_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-COMPLIANCE_REPORT_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-COMPLIANCE_REPORT_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-COMPLIANCE_REPORT_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
