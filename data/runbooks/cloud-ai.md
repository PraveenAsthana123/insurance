...
WHAT broke?      Check `/data/metrics/cloud-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/cloud-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/cloud-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/cloud-ai/model.joblib.backup models/cloud-ai/model.joblib`
