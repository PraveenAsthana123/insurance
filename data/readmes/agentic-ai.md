# Agentic AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `agentic-ai` · **Kind**: `agent_engine`
...
**Business goal**: Reduce manual effort + improve accuracy for agentic ai tasks in
...
**FR-1**: System MUST accept input matching `agentic-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/agentic-ai/predict
        │
        ▼
...
- `predictions_agentic-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/agentic-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/agentic-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/agentic-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/agentic-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `agentic-ai_embeddings`
...
**Fairness metrics** (from data/fairness/agentic-ai.json):
