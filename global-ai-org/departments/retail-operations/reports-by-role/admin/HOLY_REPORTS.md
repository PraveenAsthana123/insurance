# HOLY Beverage — Retail Operations — Admin Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **admin** in **retail-operations** uses.
...
- **Portal**: every report retrievable at `/holy/retail-operations/reports?role=admin&id=<report_id>`
...
- `request_id`, `actor_role=admin`, `dept=retail-operations`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/retail-operations/admin
  → list of standard reports

POST /api/v1/holy/reports/retail-operations/admin/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/retail-operations/admin/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/retail-operations/reports?role=admin`
