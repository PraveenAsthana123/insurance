...
WHAT broke?      Check `/data/metrics/finops-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/finops-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/finops-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/finops-ai/model.joblib.backup models/finops-ai/model.joblib`
