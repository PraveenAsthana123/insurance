# Fraud-Numerical-4 · synthetic-identity-fraud
...
|---|---|---|---|
| `INSUR-SYNTHETIC_IDENTITY_FRAUD-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-SYNTHETIC_IDENTITY_FRAUD-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-SYNTHETIC_IDENTITY_FRAUD-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-SYNTHETIC_IDENTITY_FRAUD-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-SYNTHETIC_IDENTITY_FRAUD-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-SYNTHETIC_IDENTITY_FRAUD-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
