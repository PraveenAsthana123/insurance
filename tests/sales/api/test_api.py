...
def test_sales_list_returns_200():
    ...
    body = r.json()
    assert "sales" in body.get("departments", []), f"sales not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_sales_invalid_input_returns_400():
