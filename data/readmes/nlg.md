# NLG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `nlg` · **Kind**: `nlp_baseline`
...
**Business goal**: Reduce manual effort + improve accuracy for nlg tasks in
...
**FR-1**: System MUST accept input matching `nlg` schema
...
- `predictions_nlg`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/nlg/predict` · body: features → prediction
- `GET /api/v1/ai-types/nlg/metrics` → live metrics.json
- `GET /api/v1/ai-types/nlg/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/nlg/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/nlg.json):
