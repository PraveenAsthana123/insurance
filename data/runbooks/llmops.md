...
WHAT broke?      Check `/data/metrics/llmops.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/llmops.json`
WHO touched?     `git log --since=<date> data/ai_types/llmops.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/llmops/model.joblib.backup models/llmops/model.joblib`
