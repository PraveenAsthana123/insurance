...
WHAT broke?      Check `/data/metrics/graphrag.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/graphrag.json`
WHO touched?     `git log --since=<date> data/ai_types/graphrag.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/graphrag/model.joblib.backup models/graphrag/model.joblib`
