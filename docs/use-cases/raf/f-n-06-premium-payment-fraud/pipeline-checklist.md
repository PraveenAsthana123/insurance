# Pipeline Checklist · premium-payment-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `premium_payment_fraud_raw` |
| Cleaned | Postgres | `premium_payment_fraud_clean` |
| Features | Feast / Postgres | `premium_payment_fraud_features` |
| Predictions | Postgres | `premium_payment_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/premium-payment-fraud/` |
...
1. Input → `premium_payment_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `premium_payment_fraud_predictions`
4. Log → ELK / Loki
