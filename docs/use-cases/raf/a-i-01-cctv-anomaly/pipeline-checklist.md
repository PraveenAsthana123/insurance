# Pipeline Checklist · cctv-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `cctv_anomaly_raw` |
| Cleaned | Postgres | `cctv_anomaly_clean` |
| Features | Feast / Postgres | `cctv_anomaly_features` |
| Predictions | Postgres | `cctv_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/cctv-anomaly/` |
...
1. Input → `cctv_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `cctv_anomaly_predictions`
4. Log → ELK / Loki
