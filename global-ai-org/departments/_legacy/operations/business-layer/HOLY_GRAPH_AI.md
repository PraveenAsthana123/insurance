# HOLY Beverage — Operations — Graph AI
...
> Anchor flow: **process_event ─ detected by anomaly_model ─ triggers RCA_LLM**.
...
```
process_event ─ detected by anomaly_model ─ triggers RCA_LLM
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in operations touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/operations` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/operations/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/operations/neighbors/<node_id>` | 1-hop neighbors of a node |
