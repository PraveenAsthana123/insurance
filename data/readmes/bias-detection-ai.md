# Bias Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `bias-detection-ai` · **Kind**: `ops_existing`
**Accuracy**: 0.9467 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for bias detection ai tasks in
...
**FR-1**: System MUST accept input matching `bias-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/bias-detection-ai/predict
        │
        ▼
...
- `predictions_bias-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/bias-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/bias-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/bias-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/bias-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `bias-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/bias-detection-ai.json):
