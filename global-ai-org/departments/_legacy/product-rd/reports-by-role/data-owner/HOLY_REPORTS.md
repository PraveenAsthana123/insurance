# HOLY Beverage — Product Rd — Data Owner Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **data-owner** in **product-rd** uses.
...
- **Portal**: every report retrievable at `/holy/product-rd/reports?role=data-owner&id=<report_id>`
...
- `request_id`, `actor_role=data-owner`, `dept=product-rd`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/product-rd/data-owner
  → list of standard reports

POST /api/v1/holy/reports/product-rd/data-owner/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/product-rd/data-owner/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/product-rd/reports?role=data-owner`
