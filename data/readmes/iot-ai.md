# IoT AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `iot-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for iot ai tasks in
...
**FR-1**: System MUST accept input matching `iot-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/iot-ai/predict
        │
        ▼
...
- `predictions_iot-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/iot-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/iot-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/iot-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/iot-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/iot-ai.json):
