...
WHAT broke?      Check `/data/metrics/ethical-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ethical-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/ethical-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ethical-ai/model.joblib.backup models/ethical-ai/model.joblib`
