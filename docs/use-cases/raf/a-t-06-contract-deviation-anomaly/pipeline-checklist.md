# Pipeline Checklist · contract-deviation-anomaly
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `contract_deviation_anomaly_raw` |
| Cleaned | Postgres | `contract_deviation_anomaly_clean` |
| Features | Feast / Postgres | `contract_deviation_anomaly_features` |
| Predictions | Postgres | `contract_deviation_anomaly_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/contract-deviation-anomaly/` |
...
1. Input → `contract_deviation_anomaly_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `contract_deviation_anomaly_predictions`
4. Log → ELK / Loki
