# Recommender-Text-8 · prd-clause-recommender
...
|---|---|---|---|
| `INSUR-PRD_CLAUSE_RECOMMENDER-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-PRD_CLAUSE_RECOMMENDER-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-PRD_CLAUSE_RECOMMENDER-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-PRD_CLAUSE_RECOMMENDER-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-PRD_CLAUSE_RECOMMENDER-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-PRD_CLAUSE_RECOMMENDER-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
