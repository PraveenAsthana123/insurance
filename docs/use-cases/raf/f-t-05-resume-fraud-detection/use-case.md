# Fraud-Text-5 · resume-fraud-detection
...
Dept 16 HR · BERT + entity match + RAG
...
|---|---|---|---|
| `INSUR-RESUME_FRAUD_DETECTION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-RESUME_FRAUD_DETECTION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-RESUME_FRAUD_DETECTION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-RESUME_FRAUD_DETECTION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-RESUME_FRAUD_DETECTION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-RESUME_FRAUD_DETECTION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
