...
any 3-5 to assemble a Supply Chain board demo.
...
- **demo_id**: `admin_demo`
- **Persona**: Admin (System) in Supply Chain
- **Scenario**: Demonstrate **Tenant onboarding + RBAC drift detection** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is Admin (System)'s view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Tenant onboarding + RBAC drift detection."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: Dept Manager in Supply Chain
- **Scenario**: Demonstrate **KPI surface + approval queue + team perf** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is Dept Manager's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by KPI surface + approval queue + team perf."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: Team Member in Supply Chain
- **Scenario**: Demonstrate **My-work queue + my SLA + personal metrics** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is Team Member's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by My-work queue + my SLA + personal metrics."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **demo_id**: `tester_demo`
- **Persona**: QA / Tester in Supply Chain
- **Scenario**: Demonstrate **Regression heatmap + flaky-test triage** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is QA / Tester's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Regression heatmap + flaky-test triage."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: SecOps Engineer in Supply Chain
- **Scenario**: Demonstrate **Alert volume + MTTD/MTTR + vuln backlog** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is SecOps Engineer's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Alert volume + MTTD/MTTR + vuln backlog."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **demo_id**: `devops_demo`
- **Persona**: DevOps in Supply Chain
- **Scenario**: Demonstrate **DORA metrics + deploy frequency + cost** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is DevOps's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by DORA metrics + deploy frequency + cost."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: AI Reviewer in Supply Chain
- **Scenario**: Demonstrate **Model drift + fairness gate + override rate** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is AI Reviewer's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Model drift + fairness gate + override rate."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: DT Lead in Supply Chain
- **Scenario**: Demonstrate **AS-IS vs TO-BE + automation % per process** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is DT Lead's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by AS-IS vs TO-BE + automation % per process."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: System Architect in Supply Chain
- **Scenario**: Demonstrate **Service health + dep graph + capacity** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is System Architect's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Service health + dep graph + capacity."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: Test Architect in Supply Chain
- **Scenario**: Demonstrate **Test pyramid health + coverage per service** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is Test Architect's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Test pyramid health + coverage per service."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: DB Architect in Supply Chain
- **Scenario**: Demonstrate **Slow-query list + schema drift + replication** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is DB Architect's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Slow-query list + schema drift + replication."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: API Architect in Supply Chain
- **Scenario**: Demonstrate **API p95 + version adoption + deprecation** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is API Architect's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by API p95 + version adoption + deprecation."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: Data Owner in Supply Chain
- **Scenario**: Demonstrate **Data quality + lineage + freshness SLA** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is Data Owner's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Data quality + lineage + freshness SLA."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: AI Strategy Lead in Supply Chain
- **Scenario**: Demonstrate **Automation backlog + ROI vs plan** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is AI Strategy Lead's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Automation backlog + ROI vs plan."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
- **Persona**: InfoSec / CISO Office in Supply Chain
- **Scenario**: Demonstrate **Compliance gates + CVE backlog + 3rd-party risk** on the Supply Chain dashboards
- **KPI moved**: Forecast MAPE + stockout rate
...
- **Talking points**: "This is InfoSec / CISO Office's view of Supply Chain. In 90 seconds we'll move Forecast MAPE + stockout rate by Compliance gates + CVE backlog + 3rd-party risk."
- **Success criteria** (drill-able):
  - Dashboard endpoint responds in p95 < 500ms
  - Action triggers an audit row visible in `/api/v1/holy/transactions/supply-chain` within 30s
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/demo-stories/supply-chain` | All 15 role demos |
| `GET /api/v1/holy/demo-stories/supply-chain/<role>` | Single role demo detail |
