# Pipeline Checklist · phishing-email-detect
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `phishing_email_detect_raw` |
| Cleaned | Postgres | `phishing_email_detect_clean` |
| Features | Feast / Postgres | `phishing_email_detect_features` |
| Predictions | Postgres | `phishing_email_detect_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/phishing-email-detect/` |
...
1. Input → `phishing_email_detect_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `phishing_email_detect_predictions`
4. Log → ELK / Loki
