# Generative AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `generative-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for generative ai tasks in
...
**FR-1**: System MUST accept input matching `generative-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/generative-ai/predict
        │
        ▼
...
- `predictions_generative-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/generative-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/generative-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/generative-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/generative-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `generative-ai_embeddings`
...
**Fairness metrics** (from data/fairness/generative-ai.json):
