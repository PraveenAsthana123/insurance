# HOLY Beverage — Engineering Network Flow
...
```
Engineering API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Engineering API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          engineering_user_events       analytics-svc
Backend API       ──►          engineering_audit              audit-store
ML Worker         ──►          engineering_predictions        prediction-store
Drift detector    ──►          engineering_alerts             on-call PagerDuty
...
|---|---|---|---|
| engineering-api | postgresql | 5432 | TCP |
| engineering-api | redis | 6379 | TCP |
| engineering-api | ollama | 11434 | TCP |
| engineering-api | otel-collector | 4317 | gRPC |
| engineering-api | (other dept APIs) | 8000 | HTTP via service mesh |
