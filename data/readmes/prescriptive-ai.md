# Prescriptive AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `prescriptive-ai` · **Kind**: `spec_only`
**Accuracy**: 0.965 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for prescriptive ai tasks in
...
**FR-1**: System MUST accept input matching `prescriptive-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/prescriptive-ai/predict
        │
        ▼
...
- `predictions_prescriptive-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/prescriptive-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/prescriptive-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/prescriptive-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/prescriptive-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `prescriptive-ai_embeddings`
...
**Fairness metrics** (from data/fairness/prescriptive-ai.json):
