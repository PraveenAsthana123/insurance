# Active Learning · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `active-learning` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for active learning tasks in
...
**FR-1**: System MUST accept input matching `active-learning` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/active-learning/predict
        │
        ▼
...
- `predictions_active-learning`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/active-learning/predict` · body: features → prediction
- `GET /api/v1/ai-types/active-learning/metrics` → live metrics.json
- `GET /api/v1/ai-types/active-learning/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/active-learning/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `active-learning_embeddings`
...
**Fairness metrics** (from data/fairness/active-learning.json):
