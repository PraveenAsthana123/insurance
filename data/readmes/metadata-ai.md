# Metadata AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `metadata-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for metadata ai tasks in
...
**FR-1**: System MUST accept input matching `metadata-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/metadata-ai/predict
        │
        ▼
...
- `predictions_metadata-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/metadata-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/metadata-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/metadata-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/metadata-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `metadata-ai_embeddings`
...
**Fairness metrics** (from data/fairness/metadata-ai.json):
