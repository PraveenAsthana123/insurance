# Drift Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `drift-detection-ai` · **Kind**: `ops_existing`
**Accuracy**: 0.9633 · **Algorithm**: GradientBoostingClassifier (per-type baseline)

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for drift detection ai tasks in
...
**FR-1**: System MUST accept input matching `drift-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/drift-detection-ai/predict
        │
        ▼
...
- `predictions_drift-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/drift-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/drift-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/drift-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/drift-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `drift-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/drift-detection-ai.json):
