# TinyML · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `tinyml` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for tinyml tasks in
...
**FR-1**: System MUST accept input matching `tinyml` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/tinyml/predict
        │
        ▼
...
- `predictions_tinyml`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/tinyml/predict` · body: features → prediction
- `GET /api/v1/ai-types/tinyml/metrics` → live metrics.json
- `GET /api/v1/ai-types/tinyml/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/tinyml/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/tinyml.json):
