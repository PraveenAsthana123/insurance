# PII Detection AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `pii-detection-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for pii detection ai tasks in
...
**FR-1**: System MUST accept input matching `pii-detection-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/pii-detection-ai/predict
        │
        ▼
...
- `predictions_pii-detection-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/pii-detection-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/pii-detection-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/pii-detection-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/pii-detection-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `pii-detection-ai_embeddings`
...
**Fairness metrics** (from data/fairness/pii-detection-ai.json):
