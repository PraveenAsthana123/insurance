# HOLY Beverage — Legal — Agentic Stack
...
> actions against and the required scope for each. The Legal manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Legal)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Legal agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| **Harness Agent** | Cross-agent sync between Legal and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/legal.yaml](../../../../config/scopes/legal.yaml)
...
| Action irreversible (deletion, signing) | Manager (Legal) approval |
| Confidence < 0.6 | Manager (Legal) review queue |
...
will enforce every grant in `config/scopes/legal.yaml` has a matching audit
