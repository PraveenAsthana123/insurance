...
WHAT broke?      Check `/data/metrics/ontology-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/ontology-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/ontology-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/ontology-ai/model.joblib.backup models/ontology-ai/model.joblib`
