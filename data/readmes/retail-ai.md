# Retail AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `retail-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for retail ai tasks in
...
**FR-1**: System MUST accept input matching `retail-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/retail-ai/predict
        │
        ▼
...
- `predictions_retail-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/retail-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/retail-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/retail-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/retail-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `retail-ai_embeddings`
...
**Fairness metrics** (from data/fairness/retail-ai.json):
