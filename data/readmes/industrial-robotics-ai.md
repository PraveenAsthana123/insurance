# Industrial Robotics AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `industrial-robotics-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for industrial robotics ai tasks in
...
**FR-1**: System MUST accept input matching `industrial-robotics-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/industrial-robotics-ai/predict
        │
        ▼
...
- `predictions_industrial-robotics-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/industrial-robotics-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/industrial-robotics-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/industrial-robotics-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/industrial-robotics-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `industrial-robotics-ai_embeddings`
...
**Fairness metrics** (from data/fairness/industrial-robotics-ai.json):
