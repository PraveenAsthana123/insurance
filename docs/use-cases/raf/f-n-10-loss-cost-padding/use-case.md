# Fraud-Numerical-10 · loss-cost-padding
...
Dept 10 Actuarial · hierarchical Bayes
...
|---|---|---|---|
| `INSUR-LOSS_COST_PADDING-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-LOSS_COST_PADDING-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-LOSS_COST_PADDING-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-LOSS_COST_PADDING-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-LOSS_COST_PADDING-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-LOSS_COST_PADDING-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
