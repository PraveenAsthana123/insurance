# VR AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `vr-ai` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for vr ai tasks in
...
**FR-1**: System MUST accept input matching `vr-ai` schema
...
- `predictions_vr-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/vr-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/vr-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/vr-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/vr-ai/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/vr-ai.json):
