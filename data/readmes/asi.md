# ASI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `asi` · **Kind**: `spec_only`
...
**Business goal**: Reduce manual effort + improve accuracy for asi tasks in
...
**FR-1**: System MUST accept input matching `asi` schema
...
- `predictions_asi`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/asi/predict` · body: features → prediction
- `GET /api/v1/ai-types/asi/metrics` → live metrics.json
- `GET /api/v1/ai-types/asi/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/asi/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/asi.json):
