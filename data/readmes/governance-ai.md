# Governance AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `governance-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for governance ai tasks in
...
**FR-1**: System MUST accept input matching `governance-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/governance-ai/predict
        │
        ▼
...
- `predictions_governance-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/governance-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/governance-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/governance-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/governance-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `governance-ai_embeddings`
...
**Fairness metrics** (from data/fairness/governance-ai.json):
