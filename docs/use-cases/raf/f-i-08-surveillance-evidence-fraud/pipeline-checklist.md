# Pipeline Checklist · surveillance-evidence-fraud
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `surveillance_evidence_fraud_raw` |
| Cleaned | Postgres | `surveillance_evidence_fraud_clean` |
| Features | Feast / Postgres | `surveillance_evidence_fraud_features` |
| Predictions | Postgres | `surveillance_evidence_fraud_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/surveillance-evidence-fraud/` |
...
1. Input → `surveillance_evidence_fraud_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `surveillance_evidence_fraud_predictions`
4. Log → ELK / Loki
