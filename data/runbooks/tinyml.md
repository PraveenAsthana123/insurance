...
WHAT broke?      Check `/data/metrics/tinyml.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/tinyml.json`
WHO touched?     `git log --since=<date> data/ai_types/tinyml.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/tinyml/model.joblib.backup models/tinyml/model.joblib`
