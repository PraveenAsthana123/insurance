# Recommender-Text-4 · kb-article-rec
...
|---|---|---|---|
| `INSUR-KB_ARTICLE_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-KB_ARTICLE_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-KB_ARTICLE_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-KB_ARTICLE_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-KB_ARTICLE_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-KB_ARTICLE_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
