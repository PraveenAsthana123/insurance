# HOLY Beverage — Engineering — Monitoring AI
...
> Persona: **Eng Manager**. Focus: **code-review LLM + CI flaky-test detector accuracy**.
...
Tab: `/holy/engineering/monitoring`
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/monitoring/engineering` | Per-job last-run summary + health status |
| `GET /api/v1/holy/monitoring/engineering/jobs/<job>/runs?limit=20` | Recent runs with manifest summaries |
| `GET /api/v1/holy/monitoring/engineering/jobs/<job>/runs/<run_id>` | Full manifest of a single run |
...
| Job error rate spike | > 10% errors over 1h window | Slack #engineering-ops |
| Drift PSI > 0.2 | accuracy gate breach | Slack #engineering-ai-reviewer |
...
1. (+) `/api/v1/holy/monitoring/engineering` returns 200 + job list
