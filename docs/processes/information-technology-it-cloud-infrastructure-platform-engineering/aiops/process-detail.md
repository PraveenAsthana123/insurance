...
> Department 19 · Information Technology (IT), Cloud, Infrastructure & Platform Engineering · slug `aiops` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/aiops/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/19/<domain>/aiops?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/19/<domain>/aiops?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/aiops.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/information-technology-it-cloud-infrastructure-platform-engineering/aiops/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-AIOPS-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-AIOPS-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-AIOPS-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-AIOPS-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-AIOPS-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-AIOPS-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-AIOPS-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/aiops/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-AIOPS-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/aiops/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/aiops/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/information-technology-it-cloud-infrastructure-platform-engineering/aiops/stats/`
