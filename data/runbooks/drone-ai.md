...
WHAT broke?      Check `/data/metrics/drone-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/drone-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/drone-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/drone-ai/model.joblib.backup models/drone-ai/model.joblib`
