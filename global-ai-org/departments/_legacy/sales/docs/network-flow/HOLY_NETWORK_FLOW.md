# HOLY Beverage — Sales Network Flow
...
```
Sales API ──HTTP──► Auth Service           (RBAC + ABAC checks)
...
```
Sales API ──HTTPS──► Vendor APIs            (Salesforce / SAP / etc.)
...
─────────────                  ───────────────          ──────────
Frontend UI       ──►          sales_user_events       analytics-svc
Backend API       ──►          sales_audit              audit-store
ML Worker         ──►          sales_predictions        prediction-store
Drift detector    ──►          sales_alerts             on-call PagerDuty
...
|---|---|---|---|
| sales-api | postgresql | 5432 | TCP |
| sales-api | redis | 6379 | TCP |
| sales-api | ollama | 11434 | TCP |
| sales-api | otel-collector | 4317 | gRPC |
| sales-api | (other dept APIs) | 8000 | HTTP via service mesh |
