# Pipeline Checklist · oem-part-image-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `oem_part_image_rec_raw` |
| Cleaned | Postgres | `oem_part_image_rec_clean` |
| Features | Feast / Postgres | `oem_part_image_rec_features` |
| Predictions | Postgres | `oem_part_image_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/oem-part-image-rec/` |
...
1. Input → `oem_part_image_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `oem_part_image_rec_predictions`
4. Log → ELK / Loki
