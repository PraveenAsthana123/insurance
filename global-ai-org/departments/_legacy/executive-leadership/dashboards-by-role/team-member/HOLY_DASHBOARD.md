# HOLY Beverage — Executive Leadership — Team Member Dashboard

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have a
> role-scoped dashboard. This is the dashboard for **team-member** in **executive-leadership**.

## Persona

The **team-member** at Executive Leadership. They use this dashboard
...
Personal queue + my tickets/leads/tasks + my SLA + my pipeline output

## KPIs Surfaced (Tiles)

- My open items
- My SLA hit-rate
- My this-week vs last
- My next action
- Inbox unread
- Mentions / replies
...
- My-work-burn-down
- My SLA trend
- My task-type distribution (pie)

Library dispatch per chart (recharts / plotly / echarts / d3 / vega-lite)
returned by backend `GET /api/v1/holy/dashboards/executive-leadership/team-member/charts/<chart_id>`.
...
Inline actions the **team-member** can take from this dashboard:
...
```
GET /api/v1/holy/dashboards/executive-leadership/team-member
  → {tiles: [...], charts: [...], filters: {...}, refreshed_at}

GET /api/v1/holy/dashboards/executive-leadership/team-member/charts/<chart_id>
  → {library, type, data, baseline, threshold, drill_url, refreshed_at}
```

## Related Artifacts

- `../../reports-by-role/team-member/HOLY_REPORTS.md` — paired standard reports
...
- Frontend route: `/holy/executive-leadership/dashboard?role=team-member`
