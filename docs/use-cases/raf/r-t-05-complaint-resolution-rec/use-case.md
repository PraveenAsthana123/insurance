# Recommender-Text-5 · complaint-resolution-rec
...
|---|---|---|---|
| `INSUR-COMPLAINT_RESOLUTION_REC-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-COMPLAINT_RESOLUTION_REC-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-COMPLAINT_RESOLUTION_REC-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-COMPLAINT_RESOLUTION_REC-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-COMPLAINT_RESOLUTION_REC-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-COMPLAINT_RESOLUTION_REC-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
