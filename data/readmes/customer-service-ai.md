# Customer Service AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `customer-service-ai` · **Kind**: `vertical_spec`
...
**Business goal**: Reduce manual effort + improve accuracy for customer service ai tasks in
...
**FR-1**: System MUST accept input matching `customer-service-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/customer-service-ai/predict
        │
        ▼
...
- `predictions_customer-service-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/customer-service-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/customer-service-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/customer-service-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/customer-service-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `customer-service-ai_embeddings`
...
**Fairness metrics** (from data/fairness/customer-service-ai.json):
