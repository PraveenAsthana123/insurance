# HOLY Beverage — Supply Chain — Graph AI
...
> Anchor flow: **sku ─ stored at warehouse ─ shipped to customer**.
...
```
sku ─ stored at warehouse ─ shipped to customer
       │
       ▼
...
2. **Onboarding** — "Show me everything a `manager` in supply-chain touches."
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/graph/supply-chain` | Full graph (nodes + edges) as JSON |
| `GET /api/v1/holy/graph/supply-chain/nodes?type=role` | Filtered node list |
| `GET /api/v1/holy/graph/supply-chain/neighbors/<node_id>` | 1-hop neighbors of a node |
