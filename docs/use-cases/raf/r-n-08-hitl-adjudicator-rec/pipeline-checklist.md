# Pipeline Checklist · hitl-adjudicator-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `hitl_adjudicator_rec_raw` |
| Cleaned | Postgres | `hitl_adjudicator_rec_clean` |
| Features | Feast / Postgres | `hitl_adjudicator_rec_features` |
| Predictions | Postgres | `hitl_adjudicator_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/hitl-adjudicator-rec/` |
...
1. Input → `hitl_adjudicator_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `hitl_adjudicator_rec_predictions`
4. Log → ELK / Loki
