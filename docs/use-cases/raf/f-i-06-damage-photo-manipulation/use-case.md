# Fraud-Image-6 · damage-photo-manipulation
...
Dept 7 Claims · ResNet + manipulation cls
...
|---|---|---|---|
| `INSUR-DAMAGE_PHOTO_MANIPULATION-INFERENCE` | per request OR `*/5 * * * *` | run model | predictions table |
| `INSUR-DAMAGE_PHOTO_MANIPULATION-DRIFT-CHECK` | hourly | PSI / KS drift | drift_metrics |
| `INSUR-DAMAGE_PHOTO_MANIPULATION-RETRAIN` | `0 3 * * 1` | weekly retrain | MLflow run |
| `INSUR-DAMAGE_PHOTO_MANIPULATION-VECTOR-INGEST` | `*/15 * * * *` | embed → vector DB | vector_db |
| `INSUR-DAMAGE_PHOTO_MANIPULATION-HITL-AUDIT` | `0 9 * * *` | sample overrides | hitl_audit |
| `INSUR-DAMAGE_PHOTO_MANIPULATION-FAIRNESS-AUDIT` | `0 9 * * 1` | per-cohort metrics | fairness_audit |
