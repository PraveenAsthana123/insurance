...
WHAT broke?      Check `/data/metrics/email-ai.json` for accuracy regression
WHEN broke?      Compare `trained_at` vs current · check `/data/drift/email-ai.json`
WHO touched?     `git log --since=<date> data/ai_types/email-ai.json`
WHY broke?       Check feature_importance shift vs baseline
HOW rollback?    `cp models/email-ai/model.joblib.backup models/email-ai/model.joblib`
