# Data Governance AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `data-governance-ai` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for data governance ai tasks in
...
**FR-1**: System MUST accept input matching `data-governance-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/data-governance-ai/predict
        │
        ▼
...
- `predictions_data-governance-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/data-governance-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/data-governance-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/data-governance-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/data-governance-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `data-governance-ai_embeddings`
...
**Fairness metrics** (from data/fairness/data-governance-ai.json):
