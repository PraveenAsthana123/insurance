"""Tier 3 — API tests for procurement.
...
def test_procurement_list_returns_200():
    ...
    body = r.json()
    assert "procurement" in body.get("departments", []), f"procurement not in /depts response"


@pytest.mark.skip(reason="placeholder — replace with real endpoint contract test")
def test_procurement_invalid_input_returns_400():
