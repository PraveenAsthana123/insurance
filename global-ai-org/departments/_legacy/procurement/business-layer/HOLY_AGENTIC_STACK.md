# HOLY Beverage — Procurement — Agentic Stack
...
> actions against and the required scope for each. The Procurement manager owns
> reviews; AI-Strategy owns scope grants.

## Owner

**Manager (Procurement)** + **AI-Strategy** + **Information Security**.

## 10-layer execution flow (§64.40)

Every Procurement agent action MUST traverse layers 1 → 10 in order. Skipping any
...
| OS | What it gives Procurement agents |
|---|---|
...
| **Harness Agent** | Cross-agent sync between Procurement and adjacent depts (handoffs) |
...
co-approval. The grant lives in [config/scopes/procurement.yaml](../../../../config/scopes/procurement.yaml)
...
| Action irreversible (deletion, signing) | Manager (Procurement) approval |
| Confidence < 0.6 | Manager (Procurement) review queue |
...
will enforce every grant in `config/scopes/procurement.yaml` has a matching audit
