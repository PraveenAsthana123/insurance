# HOLY Beverage — Executive Leadership — Digital Transformation Standard Reports

> Per global CLAUDE.md §64.37 — every (dept, role) pair MUST have ≥ 3 standard
> reports. These are the reports the **digital-transformation** in **executive-leadership** uses.
...
- **Portal**: every report retrievable at `/holy/executive-leadership/reports?role=digital-transformation&id=<report_id>`
...
- `request_id`, `actor_role=digital-transformation`, `dept=executive-leadership`, `report_id`, `recipients[]`
- `data_hash` (so we can prove what was sent vs what was claimed)

## Backend Contract

```
GET  /api/v1/holy/reports/executive-leadership/digital-transformation
  → list of standard reports

POST /api/v1/holy/reports/executive-leadership/digital-transformation/<report_id>/run
  → trigger report; returns {run_id, status, artifact_url}

GET  /api/v1/holy/reports/executive-leadership/digital-transformation/<report_id>/runs/<run_id>
...
- Frontend route: `/holy/executive-leadership/reports?role=digital-transformation`
