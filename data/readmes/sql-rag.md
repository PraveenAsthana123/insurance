# SQL RAG · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `sql-rag` · **Kind**: `rag_variant`
...
**Business goal**: Reduce manual effort + improve accuracy for sql rag tasks in
...
**FR-1**: System MUST accept input matching `sql-rag` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/sql-rag/predict
        │
        ▼
...
- `predictions_sql-rag`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/sql-rag/predict` · body: features → prediction
- `GET /api/v1/ai-types/sql-rag/metrics` → live metrics.json
- `GET /api/v1/ai-types/sql-rag/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/sql-rag/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/sql-rag.json):
