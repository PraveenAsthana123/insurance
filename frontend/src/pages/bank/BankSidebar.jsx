  ...
  return (
    <aside style={{
      ...
      <div style={{ padding: '10px 0', borderBottom: '1px solid #1e40af', marginBottom: 10 }}>
        ...
        {[
          { to: '/bank/workspace?module=eai-os',    icon: '🏢', label: '§144 · Enterprise AI OS (9 layers)' },
          { to: '/bank/workspace?module=itsm',       icon: '🎫', label: '§143 · ITSM (incidents · L2 RCA · P1)' },
          { to: '/bank/workspace?module=prompts',    icon: '💬', label: '§145 · Conversation log' },
          { to: '/bank/workspace?module=platform',   icon: '🗺️', label: '§147 · Platform Explorer (all APIs)' },
          { to: '/bank/workspace?module=ai-types',   icon: '🤖', label: '§148 · AI Types catalog (200)' },
          { to: '/bank/workspace?module=processes',  icon: '🛡️', label: '§150 · Process Resilience (live)' },
          { to: '/bank/workspace?module=chatgroup',  icon: '💬', label: 'ChatGroup · human + agent rooms' },
          { to: '/bank/workspace?module=control-tower', icon: '🏗', label: '§144 · AI Control Tower (12 dashboards)' },
          { to: '/bank/workspace?module=stt',        icon: '🎙', label: 'Speech-to-Text · audio + video' },
          { to: '/bank/workspace?module=tts',        icon: '🔊', label: 'Text-to-Speech (§F01)' },
          { to: '/bank/workspace?module=notifications', icon: '🔔', label: 'Notification Center (§F02)' },
          { to: '/bank/workspace?module=feature-flags', icon: '🎛', label: 'Feature Flags (§F03)' },
          { to: '/bank/workspace?module=workspace-demo', icon: '🧪', label: '§149.2 Layout Demo (drag · glass · flow)' },
          { to: '/bank/workspace?module=eaos-dept',    icon: '🏢', label: '🆕 EAOS Department (10 sub-menu · 12 tabs each)' },
          { to: '/bank/workspace?module=eaos',         icon: '🏢', label: '§EAOS Top-10 Scoreboard' },
          { to: '/bank/workspace?module=command-center', icon: '🏛', label: 'Enterprise AI Command Center' },
          { to: '/bank/workspace?module=promptops',    icon: '📝', label: 'PromptOps' },
          { to: '/bank/workspace?module=evalops',      icon: '🧪', label: 'EvaluationOps' },
          { to: '/bank/workspace?module=governance-om',icon: '⚖️', label: 'Governance Operating Model (§EAOS-3)' },
          { to: '/bank/workspace?module=agent-lifecycle', icon: '🔁', label: 'Agent Lifecycle (§EAOS-5)' },
          { to: '/bank/workspace?module=audit-explorer', icon: '📜', label: 'Audit Log Explorer (§F04)' },
          { to: '/bank/workspace?module=cost',           icon: '💰', label: 'Cost Optimizer (§F05)' },
          { to: '/bank/workspace?module=drift',          icon: '📉', label: 'Drift Monitor (§F06)' },
          { to: '/bank/workspace?module=prompt-playground', icon: '🧪', label: 'Prompt Playground (§F07)' },
          { to: '/bank/workspace?module=model-compare',  icon: '🆚', label: 'Model Comparison (§F09)' },
          { to: '/bank/workspace?module=datasets',       icon: '📦', label: 'Datasets (§F13)' },
          { to: '/bank/workspace?module=finetune',       icon: '🛠', label: 'Fine-tune Jobs (§F12)' },
          { to: '/bank/workspace?module=webhook-debug',  icon: '🪝', label: 'Webhook Debug (§F15)' },
          { to: '/bank/workspace?module=sse-stream',     icon: '📡', label: 'SSE Stream (§F16)' },
          { to: '/bank/workspace?module=aeo',            icon: '🧠', label: '🆕 AEO · Autonomous Enterprise Orchestrator (Layer 10)' },
          { to: '/bank/workspace?module=digital-twin',   icon: '🎮', label: 'Digital Twin (Layer 8)' },
          { to: '/bank/workspace?module=investment-portfolio', icon: '💎', label: 'Investment Portfolio (Layer 7)' },
          { to: '/bank/workspace?module=cost-portfolio', icon: '💰', label: 'Cost Portfolio (Layer 6)' },
          { to: '/bank/workspace?module=risk-portfolio', icon: '⚠️', label: 'Risk Portfolio (Layer 5)' },
          { to: '/bank/workspace?module=model-portfolio',icon: '🧠', label: 'Model Portfolio (Layer 4)' },
          { to: '/bank/workspace?module=translate',      icon: '🌐', label: 'Translation (§F10)' },
          { to: '/bank/workspace?module=ocr',            icon: '👁', label: 'Functional OCR (§F11)' },
          { to: '/bank/workspace?module=embeddings',     icon: '🔢', label: 'Embedding Playground (§F14)' },
          { to: '/bank/workspace?module=vectors',        icon: '🗄', label: 'Vector DB Browser (§F08)' },
        ].map((m) => (
      ...
      <div style={{ padding: '10px 0', borderBottom: '1px solid #1e40af', marginBottom: 10 }}>
        ...
        <Link to="/bank/workspace?module=ai-types" style={{
        ...
        {[
          ...
          <Link key={d.domain} to={`/bank/workspace?module=ai-types&domain=${d.domain}`} style={{
            display: 'flex', alignItems: 'center', gap: 8,
            minHeight: 38,
            padding: '8px 30px',
            color: '#dbeafe', textDecoration: 'none',
