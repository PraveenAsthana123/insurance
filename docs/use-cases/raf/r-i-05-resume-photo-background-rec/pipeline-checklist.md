# Pipeline Checklist · resume-photo-background-rec
...
| Stage | Storage | Table |
|---|---|---|
| Raw | Postgres | `resume_photo_background_rec_raw` |
| Cleaned | Postgres | `resume_photo_background_rec_clean` |
| Features | Feast / Postgres | `resume_photo_background_rec_features` |
| Predictions | Postgres | `resume_photo_background_rec_predictions` |
| Embeddings | Vector DB | source = predictions |
| Audit | Postgres | `audit_rows` (§38.3) |
| Models | MLflow | registry |
| Explanations | S3/MinIO | `/explanations/resume-photo-background-rec/` |
...
1. Input → `resume_photo_background_rec_raw` (redacted)
2. Process trace → OTel span tree
3. Output → `resume_photo_background_rec_predictions`
4. Log → ELK / Loki
