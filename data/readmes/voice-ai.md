# Voice AI · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `voice-ai` · **Kind**: `audio_baseline`
**Accuracy**: 0.935 · **Algorithm**: GradientBoost on synthetic mel-spectrogram features

---

## 1. BRD · Business Requirements Document

**Business goal**: Reduce manual effort + improve accuracy for voice ai tasks in
...
**FR-1**: System MUST accept input matching `voice-ai` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/voice-ai/predict
        │
        ▼
...
- `predictions_voice-ai`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/voice-ai/predict` · body: features → prediction
- `GET /api/v1/ai-types/voice-ai/metrics` → live metrics.json
- `GET /api/v1/ai-types/voice-ai/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/voice-ai/model.joblib]
            ↘ [preprocessor.py]
...
- collection: `voice-ai_embeddings`
...
**Fairness metrics** (from data/fairness/voice-ai.json):
