# HOLY Beverage — Sales — Master Data

> Per operator 2026-05-24 — every dept publishes its master + reference
> data here. Focus: **customer-master + sales-org + price-list + discount-conditions**.
...
This dept (**sales**) operates against the slice of the structure
...
| Endpoint | Returns |
|---|---|
| `GET /api/v1/holy/master-data/sales` | Schema catalog + per-entity owner |
| `GET /api/v1/holy/master-data/sales/customer?limit=20` | Customer master sample |
| `GET /api/v1/holy/master-data/sales/vendor?limit=20` | Vendor master sample |
| `GET /api/v1/holy/master-data/sales/product?limit=20` | Product master sample |
| `GET /api/v1/holy/master-data/sales/org-structure` | Org tree slice for this dept |
...
1. (+) `/api/v1/holy/master-data/sales` returns 200 + entity catalog
