...
> Department 7 · Claims · slug `customer-communication` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/customer-communication/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/7/<domain>/customer-communication?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/7/<domain>/customer-communication?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/customer-communication.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/claims/customer-communication/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-CUSTOMER_COMMUNICATION-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-CUSTOMER_COMMUNICATION-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-CUSTOMER_COMMUNICATION-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-CUSTOMER_COMMUNICATION-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-CUSTOMER_COMMUNICATION-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-CUSTOMER_COMMUNICATION-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-CUSTOMER_COMMUNICATION-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/claims/customer-communication/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-CUSTOMER_COMMUNICATION-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/claims/customer-communication/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/claims/customer-communication/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/claims/customer-communication/stats/`
