...
> Department 15 · Enterprise Risk Management (ERM) · slug `climate-risk` · AI items: 4
...
| Item | What | Where |
|---|---|---|
| Test data | TODO scrubbed sample | `data/demo/climate-risk/` |
...
- **One-liner**: TODO
- **Demo URL**: `http://192.168.1.88:3210/insurance/15/<domain>/climate-risk?tab=user-demo`
- **Recording**: TODO
...
Simulation URL: `http://192.168.1.88:3210/insurance/15/<domain>/climate-risk?tab=simulation`
...
| 2 | Feature schema | data spec | `backend/ml/insurance/schemas/climate-risk.yaml` | view-only |
...
Release summary: auto-gen `docs/processes/enterprise-risk-management-erm/climate-risk/release-summary-vX.Y.md` covering §1 problem · §4 impact deltas · §10 phase scorecard · §11 test pass/fail · top 3 failure modes · sign-off · git SHA + image digest.

## 12. Cron jobs + DB writes

| Cron tag | Schedule | Purpose | DB updated |
|---|---|---|---|
| `INSUR-CLIMATE_RISK-DAILY-REFRESH` | daily 02:00 | refresh model on new data | MLflow + audit |
| `INSUR-CLIMATE_RISK-DRIFT-CHECK` | hourly | data + concept drift | drift_metrics |
| `INSUR-CLIMATE_RISK-HITL-AUDIT` | daily 09:00 | sample HITL overrides for retrain | hitl_audit |
| `INSUR-CLIMATE_RISK-RAG-REINDEX` | weekly Mon 03:00 | re-embed source docs | vector_db |
| `INSUR-CLIMATE_RISK-COST-REPORT` | weekly Mon 08:00 | dollar impact reconciliation | cost_report |
| `INSUR-CLIMATE_RISK-EVAL-DEEPEVAL` | weekly Mon 04:00 | Ragas + DeepEval regression | eval_metrics |
| `INSUR-CLIMATE_RISK-VECTOR-INGEST` | */15 * * * * | move new data into vector DB | vector_db |
...
Gold set: `data/eval/enterprise-risk-management-erm/climate-risk/gold_set.jsonl` (100-500 queries · stratified · monthly refresh).
...
Every data artifact embedded automatically · per `INSUR-CLIMATE_RISK-VECTOR-INGEST` cron above.
...
→ Tab: `?tab=resai` · evidence files: `data/eval/enterprise-risk-management-erm/climate-risk/resai/`
...
→ Tab: `?tab=expai` · artifacts: `data/eval/enterprise-risk-management-erm/climate-risk/expai/`
...
→ Tab: `?tab=analysis` · evidence: `data/eval/enterprise-risk-management-erm/climate-risk/stats/`
