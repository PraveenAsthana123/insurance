# LLMOps · Comprehensive README · §137

**Effective**: 2026-06-11 · **Slug**: `llmops` · **Kind**: `ops_existing`
...
**Business goal**: Reduce manual effort + improve accuracy for llmops tasks in
...
**FR-1**: System MUST accept input matching `llmops` schema
...
Caller (frontend/agent)
        │
        ▼
  /api/v1/ai-types/llmops/predict
        │
        ▼
...
- `predictions_llmops`: id · input_jsonb · output · confidence · timestamp

**Endpoints**:
- `POST /api/v1/ai-types/llmops/predict` · body: features → prediction
- `GET /api/v1/ai-types/llmops/metrics` → live metrics.json
- `GET /api/v1/ai-types/llmops/explain/{prediction_id}` → SHAP
...
```
[router.py] → [model_loader.py] → [/models/llmops/model.joblib]
            ↘ [preprocessor.py]
...
**Fairness metrics** (from data/fairness/llmops.json):
